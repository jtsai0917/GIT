					...
					while($row = mssql_fetch_array($result))
					{
						...
						switch ($str)
						{
							...
							$query="SELECT  PROD_TANK, PDD_PROD_NO
									FROM      TANK_DATA1
									WHERE   (PDD_PROD_NO LIKE '%".substr($_SESSION['AND_GOODS'],0,4)."%') ";
	
							$result=mssql_query($query);
							$numRows=mssql_num_rows($result);
							$a='<select name="TANK_NO" id="TANK_NO" onchange="set_date_session(this.name,this.value)">';
							while($row=mssql_fetch_array($result)){
								if($_SESSION['TANK_NO']==$row['PROD_TANK']){$str= ' selected';$value=$M.$row['PROD_TANK'];}else{$str='';}
								$a.= '<option value="'.$row['PROD_TANK'].'" '.$str.'>'.$row['PROD_TANK'].'</option>';
							}
