...
<form action="<?php echo $loginFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
  ...
  <table width="1200" border="1">
    <tr class="centet">
      ...
      <input name="LotNo" type="text" id="LotNo" value="<?php echo $_GET['lot_no']?>" size="20" />
      (A,B,C,D)</td>
...
($row['COLUMN_NAME']!='SerialNo') && ($row['COLUMN_NAME']!='TestDate'))
	...
	{echo '<tr class="centet">';
		...
		elseif($stdu=='' and $usl<>'' and $_SESSION[$row['COLUMN_NAME']]<=$usl){echo ' style="background-color:white;"';}
	  	elseif(($_SESSION[$row['COLUMN_NAME']]<=$usl and $_SESSION[$row['COLUMN_NAME']]>$stdu) or ($_SESSION[$row['COLUMN_NAME']]>=$lsl and $_SESSION[$row['COLUMN_NAME']]<$stdl)) {echo ' style="background-color:yellow;"';}
	  	elseif((($_SESSION[$row['COLUMN_NAME']]>$usl) and ($_SESSION[$row['COLUMN_NAME']]<>'NULL') and ($spec<>NULL)) or (($_SESSION[$row['COLUMN_NAME']]<$lsl) and ($_SESSION[$row['COLUMN_NAME']]<>'NULL') and ($spec<>NULL))){echo ' style="background-color:red;"';}
...
<span class="a18">檢驗報告非上傳人員無法刪除</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;請選擇ABCD(上傳報告送出前選擇即可)
<?php
	echo '<select name="ABCD" id="ABCD">';
...
FROM              FILE_REPORTS
WHERE          (FILE_LOT_NO like '".$_GET['lot_no']."%') AND (ANI_GROUP = '".$_GET['ani_groupname']."')";
$result = mssql_query($query);
//echo $query;
while($row = mssql_fetch_array($result)){
	echo '<tr>';
...
{  if($_POST['ABCD']=='Z')
	...
	if($_GET['ani_groupname']=='M13' or $_GET['ani_groupname']=='P')
	{	$_SESSION['input3']='Y';
		$redir=substr($_SESSION['redir'],0,-1);
		
		scriptconfirm1("是否上傳數據?",$redir."M13",$_SESSION['redir']."1");
	}
	else
	{
