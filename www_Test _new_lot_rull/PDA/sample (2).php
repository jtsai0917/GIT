...
$query="select count(*) from sample where SMP_LOT='".$_SESSION['lid']."'";
	...
	echo '</select><BR>';
		echo '樣品瓶號：<input type="text" name="smpid" id="smpid" width="10" value="'.$_SESSION['smpid'].'" onKeyPress="return SubmitEnter(this,event)" onchange="set_date_session(this.name,this.value)"/>';
...
if(isset($_POST['add'])) {
	if($_POST['selected_group']==5){$isrework='0';}
	else{$isrework=NULL;}
...
$query="INSERT INTO dbo.Sample_All
							...
							NULL,NULL,NULL,'".$_POST['selected_group']."',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL)";
...
if($result = mssql_query($query)){echo "Sample updated...<BR>";};
	...
	{
		scriptconfirm("無此瓶號，建立新瓶號?","[".$_POST['sma_id']."] 無此樣品瓶號","insert_smp_.php?pid=".$pid."&smp_id=".$_POST['smpid']."&sn=".$_POST['selected_group']);
	}
refresh();
}
$i=1;
