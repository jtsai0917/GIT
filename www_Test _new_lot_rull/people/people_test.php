				...
				/*		if(trim($row['ani_group'])=="TT")
						{									
								...
								while ($rowTT = mssql_fetch_array($resultTT))
								{
									if(trim($rowTT['In'])<>'')
									{
										//$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+($row1['hrs']/$numRows1);$alltime1=$alltime1+($row1['hrs']/$numRows1);
															$TTIN="select hrs
															from WORKTIME
															where ANI_FULLNAME='TTlndium' and PDD_CHEMICAL='H2SO4'";
															$resultTTIN = mssql_query($TTIN);
															while ($rowTTIN = mssql_fetch_array($resultTTIN))
															{
												
												$alltime=$alltime+$rowTTIN['hrs']/$numRows1;
																
															}									
									}
									if(trim($rowTT['Pt'])<>''){//$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$row1['hrs']/$numRows1;$alltime1=$alltime1+($row1['hrs']/$numRows1);
									$TTPT="select hrs
															from WORKTIME
															where ANI_FULLNAME='Platium' and PDD_CHEMICAL='H2SO4' ";
															$resultTTPT = mssql_query($TTPT);
															while ($rowTTPT = mssql_fetch_array($resultTTPT))
															{
												
												$alltime=$alltime+$rowTTPT['hrs']/$numRows1;
															}
									}
									if(trim($rowTT['Se'])<>''){$TTSE="select hrs
															from WORKTIME
															where ANI_FULLNAME='Selenium' and PDD_CHEMICAL='H2SO4' ";
															$resultTTSE = mssql_query($TTSE);
															while ($rowTTSE = mssql_fetch_array($resultTTSE))
															{
												
												$alltim1=$alltime+$rowTTSE['hrs']/$numRows1;
															}
									}
									if(trim($rowTT['Hg'])<>''){$TTHG="select hrs
															from WORKTIME
															where ANI_FULLNAME='Mercury' and PDD_CHEMICAL='H2SO4' ";
															$resultTTHG = mssql_query($TTHG);
															while ($rowTTHG = mssql_fetch_array($resultTTHG))
															{
												
												$alltime=$alltime+$rowTTHG['hrs']/$numRows1;
															}	
									}
									
									if(trim($rowTT['Pd'])<>''){$TTPD="select hrs
															from WORKTIME
															where ANI_FULLNAME='Platium' and PDD_CHEMICAL='H2SO4' ";
															$resultTTPD = mssql_query($TTPD);
															while ($rowTTPD = mssql_fetch_array($resultTTPD))
															{
												
												$alltime=$alltime+$rowTTPD['hrs']/$numRows1;
															}	
...
/////////////////////////////////////////////////////////////////////前處理計算////////////////////////////////////
				if(trim($row['first'])<>'')
				{
					/*if(trim($row['chemical'])=='H2SO4' and  trim($row['ani_group'])=='TT')
						{
							$TT="select top 1 [In],[Pt],[Se],[Hg],[P],[Pd] 
								from TLEQA9110103
								where lotno='".$row['lot_no']."'";
								//echo $TT;
								$_SESSION['TT']=$_SESSION['TT']."/".$TT;
								$resultTT = mssql_query($TT);
								while ($rowTT = mssql_fetch_array($resultTT))
								{	
								if(trim($rowTT['Pt'])<>''){
															...
															while ($rowTTPT = mssql_fetch_array($resultTTPT))
															{
																$firsttime=$firsttime+$rowTTPT['hrs'];
									...
									if(trim($rowTT['Se'])<>''){
															...
															while ($rowTTSE = mssql_fetch_array($resultTTSE))
															{
																$firsttime=$firsttime+$rowTTSE['hrs'];echo "+10";
									...
									if(trim($rowTT['Hg'])<>''){
															...
															while ($rowTTHG = mssql_fetch_array($resultTTHG))
															{
																$firsttime=$firsttime+$rowTTHG['hrs'];
															}
...
/////////////////////////////////////////////////////////算工時大部分/////////////////////////////////////////////		
	...
	$objPHPExcel = PHPExcel_IOFactory::load("./report.xlsx");
			...
			$objPHPExcel->getActiveSheet()->setCellValue("H".$p, iconv("big5","utf-8","實際工作天數"));
			$GLOBALS['p']++;
...
/////////////////////////////////////////////////////////////////////前處理計算////////////////////////////////////
				...
				/*	if(trim($row['chemical'])=='H2SO4' and  trim($row['ani_group'])=='TT')
						{		
								...
								$TT="select top 1 [In],[Pt],[Se],[Hg],[P],[Pd] 
								from TLEQA9110103
								where lotno='".$row['lot_no']."' and AnalyzeTime='".$row['create_time']."' ";
								//echo $TT;
								$_SESSION['TT']=$_SESSION['TT']."/".$TT;
								$resultTT = mssql_query($TT);
								while ($rowTT = mssql_fetch_array($resultTT))
								...
								if(trim($rowTT['Pt'])<>''){
									...
									if(trim($rowTT['Hg'])<>''){
															...
															while ($rowTTHG = mssql_fetch_array($resultTTHG))
															{
																$first1[$row['first']]=$first1[$row['first']]+$rowTTHG['hrs'];
																$first2=$first2+$rowTTHG['hrs'];
															}
									...
									if(trim($rowTT['Au'])<>''){
															$TTAU="select hrs
															from WORKTIME
															where ANI_FULLNAME='AU'";
															$resultTTAU = mssql_query($TTAU);
															while ($rowTTAU = mssql_fetch_array($resultTTAU))
															{
																$first1[$row['first']]=$first1[$row['first']]+$rowTTAU['hrs'];
																$first2=$first2+$rowTTAU['hrs'];
															}
									}
							}
						}//TT IF
								...
								{	
									if(trim($rowTT['In'])<>'')
									{
										//$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+($row1['hrs']/$numRows1);$alltime1=$alltime1+($row1['hrs']/$numRows1);
															...
															while ($rowTTIN = mssql_fetch_array($resultTTIN))
															{
												$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$rowTTIN['hrs']/$numRows1;
												$alltime1=$alltime1+$rowTTIN['hrs']/$numRows1;
									...
									$TTPT="select hrs
															...
															while ($rowTTPT = mssql_fetch_array($resultTTPT))
															{
												$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$rowTTPT['hrs']/$numRows1;
												$alltime1=$alltime1+$rowTTPT['hrs']/$numRows1;
															}
									}
									if(trim($rowTT['Se'])<>''){$TTSE="select hrs
															from WORKTIME
															...
															while ($rowTTSE = mssql_fetch_array($resultTTSE))
															{
												$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$rowTTSE['hrs']/$numRows1;
												$alltime1=$alltime1+$rowTTSE['hrs']/$numRows1;
															}
									}
									if(trim($rowTT['Hg'])<>''){$TTHG="select hrs
															from WORKTIME
															...
															while ($rowTTHG = mssql_fetch_array($resultTTHG))
															{
												$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$rowTTHG['hrs']/$numRows1;
												$alltime1=$alltime1+$rowTTHG['hrs']/$numRows1;
															}	
									}
									...
									if(trim($rowTT['Pd'])<>''){$TTPD="select hrs
															...
															while ($rowTTPD = mssql_fetch_array($resultTTPD))
															{
												$alltime[$rowPP['create_user']]=$alltime[$rowPP['create_user']]+$rowTTPD['hrs']/$numRows1;
												$alltime1=$alltime1+$rowTTPD['hrs']/$numRows1;
															}	
			 ...
			 and (AF.create_time<='".$date2."083000')";
				...
				while ($rowP1 = mssql_fetch_array($resultP1))
				{
						...
						if($num[$rowP1['create_user']]>0 )
						{
							...
							$objPHPExcel->getActiveSheet()->setCellValue("H".$p,HR());
							$GLOBALS['p']++;
						}
				}
  ...
  <tr class="GridviewScrollHeader">';
	//	echo '<table border="1" width="1024">';
		//echo '<tr height="">';
			...
			$query="select AF.first,AF.lot_no,AF.create_time,AF.chemical,AF.ani_group,AF.create_user,ED1.EMP_NAME as ed1
			 ,ED2.EMP_NAME as ed2,AF.need_no,AF.ps
			 from analyze_first1 as AF
			 left join EMPLOYEE_DATA as ED1 on AF.create_user=ED1.EMP_NO
			 left join EMPLOYEE_DATA as ED2 on AF.first=ED2.EMP_NO
	...
	$query=$query."and ani_group!='PMS' and ani_group!='RION' order by AF.create_time";
	//echo $query;
	  $result = mssql_query($query);
			$numRows=mssql_num_rows($result);	
...
function HR(){
	...
	$query="select max(time1) as new from
		(
		SELECT 
		...
		convert(varchar, [recorddatetime], 108) as time2,*
		  ...
		  where empno like '%".trim($_SESSION['create_user'])."%' and readername like '%考勤%' or ((readername like '%補打卡%' or readername like '%出差%') and SAG_FINISHED_TIME!='')  
		  )  as OOXX
			where time1 >='".$A."' and time1 <= '".$B."' group by time1";
			//echo $query;
			$result = mssql_query($query);
			$numRows=mssql_num_rows($result);
			while($row=mssql_fetch_array($result));
