...
//	echo $query."<BR>";
	...
	if($_POST['btk']==''){$btk='NULL';}else{$btk="'".$_POST['btk']."'";}
		$query="Insert Into LORRY_FILL_CHECK 	(create_date, creator, FDM_LOT_NO,  LFC_W_LY_NO,  LFC_B_REMAIN_QTY,  LFC_B_TROUGH_TOP,  LFC_B_CHK_COUP_CLR,  LFC_B_CHK_COUP,  LFC_B_CHK_COUP_LINK,  LFC_B_CHK_EXHAUST,  
				...
				VALUES ('".date("YmdHis")."','".$_SESSION['uid']."','".$_SESSION['lid']."','".trim($lorry_no)."',".$remnant.",".$m3.",'Y','Y','Y','Y',".$qty.",".$flow_speed.",".$smp_cnt.",".$btk.",".$qty.",'".$_POST['etq']."','Y','Y','Y','Y','Y','Y','Y','".$_SESSION['uid']."',".$_POST['sti0'].",'".$_POST['sti1']."','".$_POST['sti2']."',".$_POST['mega_check'].",'Y','".$tank_no."','".$_POST['bm3']."','".$_POST['em3']."')";
//		$_SESSION['opp']= $query ;
//		break;
		$result1 = mssql_query($query);
...
function sm1(){
	...
	echo '充填量設定： <input type="text"  autocomplete="off" name="qty" id="qty"  onKeyPress="return SubmitEnter("this",event)"/></br>';
...
function tank(){
	...
	select_Spot($pid);
	echo '</td></tr></table>';	
	echo '</br></br><input type="submit"  name="finish" value="   下 一 步   " /><input type="submit" name="submit3" value="上一步" /></br>';
}
