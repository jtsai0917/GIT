...
if((isset($_POST["mm_insert"])) && ($_POST["mm_insert"] == "form1")){
	...
	$query="INSERT INTO TLOQA9100310A
                            (TestDate, CHK1, CHK2, CHK3, CHK4, LotNo, SerialNo, SampleNo, value1, Ok, Tester, Operator, AnaManager, 
                            AnalyzeTime)";

	$query.=" VALUES          ('".$_POST['TestDate']."',1,1,1,1,'".$_POST['lot_no']."','".$_POST['serialno']."','".$_POST['sampleno']."',".$_POST['value1'].",".$ok.",'".$_SESSION['uname']."','".$_POST['operator2']."','','".$_POST['analyzetime']."')";
...
$query="SELECT          TLOQA9100310A.*
FROM              TLOQA9100310A INNER JOIN
                            AnalyzeDesign ON TLOQA9100310A.LotNo = AnalyzeDesign.AND_LOT_NO
...
?>
  ...
  <p>項目：H2O2 NH4 ； 測試  單位：ppb ； 社內規格 ：?~10 ；再分析值/定量下限：?~10 </p>
  <table width="1200" border="1">
    <tr class="centet">
  <td width="83">ICS1100分析方法</td>
      <td width="308">
        輸入: 
          <input name="value1" type="text" id="value1" value="0" size="6" />
        ppb 
...
<td><label for="lotno"></label>
  ...
  <input type="text" name="operator" id="operator" value="<?php echo iconv("utf-8","big5",$_GET['operator']);?>" readonly="readonly"></td>
    ...
    <tr class="centet">
      ...
      <td width="200"> 值 :
        <input name="qty11" type="text" id="qty11" value="<?php echo $row['value1']?>" size="10"readonly="readonly"></td>
      <td width="200">合否判定
