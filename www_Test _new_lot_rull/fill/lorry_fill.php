...
if(isset($_POST['enter'])) {
	...
	$query="SELECT  FILL_INDICATE.FID_FILL_STATUS, FILL_INDICATE.FID_FILL_BEGIN_DATE, FILLPLAN_OUT_DECIDE.FDM_LY_NO
			FROM      FILL_INDICATE INNER JOIN FILLPLAN_OUT_DECIDE ON FILL_INDICATE.FDM_LOT_NO = FILLPLAN_OUT_DECIDE.FDM_LOT_NO
	...
	if($row1[0]==2){my_msg("此 LOT 已經充填完成","fill.php");}
...
if(isset($_POST['end'])) 
{
	...
	$query="UPDATE  FILL_INDICATE SET FID_FILL_STATUS = 2, FID_FILL_END_DATE = '".date("YmdHis")."' WHERE   (FDM_LOT_NO = '".$_SESSION['lid']."') ";
	...
	$query="Insert Into LORRY_FILL_CHECK 	(FDM_LOT_NO,  LFC_W_LY_NO,  LFC_B_REMAIN_QTY,  LFC_B_TROUGH_TOP,  LFC_B_CHK_COUP_CLR,  LFC_B_CHK_COUP,  LFC_B_CHK_COUP_LINK,  LFC_B_CHK_EXHAUST,  
			...
			VALUES ('".$_SESSION['lid']."','".trim($_SESSION['lorry_no'])."',".$_POST['remnant'].",".$_POST['m32'].",'Y','Y','Y','Y',".$_POST['qty'].","._flow($_POST['qty']).",".$smp_cnt.",'".$_POST['btk']."',".$_POST['m3'].",'".$_POST['m32']."','Y','Y','Y','Y','Y','Y','Y','".$_SESSION['uid']."',".$_POST['sti0'].",'".$_POST['sti1']."','".$_POST['sti0']."','1','Y','".$_POST['rd']."')";
//	echo "<BR>".$query."<BR>";
	$result1 = mssql_query($query);
...
function sm1(){
	...
	(Select Max(PRA_OUT_COUNT) as MAX_OUT_COUNT  From LORRY_EXAMINE_LIST Where LEL_LOT_NO ='".$_SESSION['lid']."')";}
                            ...
                            (LEL_LOT_NO, PRA_OUT_COUNT, PDD_PROD_NO, LEL_LY_NO, LEL_REMNANT_QTY) 
...
function tank(){
	...
	echo '<input type="hidden" name="purge_qty" id="qty" value="'.$_POST['purge_qty'].'" />';
	echo '<input type="hidden" name="flow_speed" value="'.$_POST['flow_speed'].'" />';
	echo '充填來源：<select name="tank_no" id="tank_no"><option value=""></option>';
...
// 8. 封槽管理項目
	...
	$query="Update FILL_INDICATE Set FID_FILL_BEGIN_DATE = '".date("YmdHis")."', FID_FILL_STATUS=1, FID_QTY = ".$_SESSION['qty'].", FID_WASHED_COUNT = 0, FID_OPERATOR = '".$_SESSION['uid']."'   Where FDM_LOT_NO = '".$_SESSION['lid']."'"; 
//	echo "<BR>".$query."<BR>";
	...
	if($numr==0){
		...
		$query="INSERT INTO lorry_fill_tmp (lot_no, remnant, m3, btk, [mega-check], purge_qty, tank_no, LFC_TANK, flow_speed, smp_cnt, update_time, lorry_no, qty)
			VALUES  (N'".$_SESSION['lid']."', ".$_POST['remnant'].", ".$_POST['m3'].", '".$_POST['btk']."', ".$_POST['mega_check'].",".$_POST['purge_qty'].", N'".$_POST['tank_no']."', N'".$_POST['tank_no']."', ".$_POST['flow_speed'].", ".$_SESSION['smp_cnt'].", N'".date("YmdHis")."', '".$_SESSION['lorry_no']."',".$_POST['qty'].")";
//		echo "<BR>".$query."<BR>";
		$result=mssql_query($query);
	}
	...
	echo '氣密測試(1.9kg/cm^2 10分鐘): <input type="text"  autocomplete="off" name="sti0" id="sti0"  style="font-size:'.($_SESSION['font_size']).'px" size="12"  value="10" /><br>';
