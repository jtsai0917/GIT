...
function items()
{
	if($this->rull_status!=1)
	{
		...
		elseif($this->Total_Monthly_1st=='Y')
		{
			...
			if(($this->eachday=='N') and ($this->eachday_first=='N') and ($this->Total_Duration_times == NULL)){$this->item=$_SESSION['Reg'];}
	...
	function read()
	{
		...
		{	  ///read excel2007	
							...
							if($v=='Sample Name')
								{
									...
									//		my_msg("上傳文件的LOT NO 錯誤",$_SESSION['lasturl']);
											fun_confirm('Lot NO 錯誤...繼續上傳',$_SESSION['lasturl']);				
										}
									}
								}
    		...
    		else
			{
				...
				$_SESSION['sSSSSSSSSSSSSSSSSSSSSSSSSSS']=$query;
				$result=mssql_query($query);
				$numrows=mssql_num_rows($result);
				if($numrows>0)
				{	
