...
datepick();
if(isset($_POST['lid'])){
	if(substr(($_POST['lid']),-4,1)=='B' or (substr(($_POST['lid']),-4,1)=='b')){
		fun_alert("請改用20L PE充填/洗淨 作業");
		jumpto("index.php");
	}
$_POST['lid']=strtoupper(trim($_POST['lid']));
$_SESSION['lid']=trim($_POST['lid']);}
$query="select * from fill_indicate where fdm_lot_no='".$_SESSION['lid']."'";
$result=mssql_query($query);
$numr=mssql_num_rows($result);
// if($numr==0){	my_msg($_POST['lid']." 沒有充填計畫","wash_drum_f1.php");}
...
$query="SELECT	DRUM_WASH_UPDATE.*	FROM	DRUM_WASH_UPDATE WHERE	(FDM_LOT_NO = '".$_SESSION['lid']."')";
// $_SESSION['que']=$query;
$result=mssql_query($query);
$numrows=mssql_num_rows($result);
if($numrows==0){
	jumpto("wash_drum_first.php");
}
...
$query="SELECT count(EWD_DRUM) as cnt FROM EL_WASH_DRUM	WHERE   (FDM_LOT_NO = '".$_SESSION['lid']."')";
// echo "<BR>".$query."<BR>";
$result=mssql_query($query);
$row=mssql_fetch_row($result);
$_SESSION['cnt']=$row[0];

$query="SELECT    top(1)      A.FOD_UNI, A.CTD_CUST_NO, B.CTD_CUST_NAME, A.FDM_SERIAL_NO, A.PDD_PROD_NO, C.PDD_PROD_NAME, C.PDD_DRUM_KG,
...
WHERE          (A.FDM_LOT_NO = '".$_SESSION['lid']."') ORDER BY   FOD_UNI DESC";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
...
if($_SESSION['cnt'] >=$_SESSION['fdm_qty_drum']){
	echo '<font size="14" color="red">超過充填桶數</font>';
	echo '<p><strong><a href="edit_drum_cnt.php?lid='.$_SESSION['lid'].'">查看/編輯'.$_SESSION['lid'].'桶號</a></strong></p>';
	break;
//	my_msg("超過充填桶數","wash_drum_f1.php");
}
	...
	if($_SESSION['reset_dmno']==1){
		echo '輸入桶號:<input type="text" style="font-size:25px" size="12" name="dm_no"  autocomplete="off" value="" autofocus onchange="set_date_session(this.name,this.value)"/><br>';
		echo '製造Lot:<input type="text" style="font-size:30px" size="12" name="made_lot"  autocomplete="off" value="'.$_SESSION['made_lot'].'"/><br>';
		echo '<input type="submit" hidden="hidden" autofocus name="sm1" value="   下 一 步   " />';
	}
	else{
		echo '輸入桶號:<input type="text" style="font-size:30px" size="12" name="dm_no"  autocomplete="off" value="'.$_SESSION['dm_no'].'" autofocus onchange="set_date_session(this.name,this.value)"/><br>';
		echo '製造Lot:<input type="text" style="font-size:30px" size="12" name="made_lot"  autocomplete="off" value="'.$_SESSION['made_lot'].'"/><br>';
		echo '<input type="submit" hidden="hidden" autofocus name="sm1" value="   下 一 步   " />'; 
	}
	unset($_SESSION['reset_dmno']);
	echo '<input type="button" name="X" id="X" style="width:120px;height:40px;border:2px orange double;" value="上 一 步" onClick="window.open('."'wash_drum_f1.php ', '_self'".');">
	<input type="button" name="X1" id="X1" style="width:120px;height:40px;border:2px orange double;" value=" 結 束 " onClick="window.open('."'fill.php ', '_self'".');"></br></form>';
