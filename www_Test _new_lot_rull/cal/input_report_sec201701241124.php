...
($row['COLUMN_NAME']!='SerialNo') && ($row['COLUMN_NAME']!='TestDate'))
	...
	{echo '<tr class="centet">';
		...
		echo '<td><font color="#0000FF">'.$showitem.'</font>：</td><td>';
      	echo '<input name="'.$row['COLUMN_NAME'].'" id="'.$row['COLUMN_NAME'].'" type="text"   onchange="set_date_session(this.name,this.value)"';
		if(substr($clo->excel,0,1)=='=')
		{
	  	...
	  	if(($_SESSION[$row['COLUMN_NAME']]>$stdu) and ($_SESSION[$row['COLUMN_NAME']]<$spec)) {echo ' style="background-color:yellow;"';}
	  	elseif(($_SESSION[$row['COLUMN_NAME']]>$spec) and ($_SESSION[$row['COLUMN_NAME']]<>'NULL') and ($spec<>NULL)){echo ' style="background-color:red;"';}
...
while ($row=mssql_fetch_array($result))
{
	...
	if((($_SESSION['uname']==$row['Tester']) or ($auth->keyin_ok=='1')) and (($row['AnalyzeTime']+3000000)>date("YmdHis"))){echo '<td width="30"><a href="index.php?'.$urllast.'&smpno='.$row['SampleNo'].'&Ok='.$row['Ok'].'&ani_time='.$row['AnalyzeTime'].'&id='.$id.'&url=edit_report" target="new">編輯</a></td>';}
	else{echo '<td></td>';}
...
while($row = mssql_fetch_array($result)){
	if(($row['COLUMN_NAME']=='B') and $_GET['pdd_chemical']=='H2O'){
		write_ttb(1,$_POST['SampleNo'],$_POST['Ok'],$_POST['Tester'],$_GET['lot_no'],"TLEQA9110101",$_POST['B'],$_POST['Operator'],$_SESSION['createtime']);
		}
