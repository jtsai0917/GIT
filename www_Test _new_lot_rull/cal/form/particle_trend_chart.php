...
include $path."/../PHPEXCEL18/Classes/PHPExcel.php";
include $path."/../PHPEXCEL18/Classes/PHPExcel/IOFactory.php";
$particle_trend_chart_st3=$path."/form/particle_trend_chart_st3_xlxs.xlsx";
...
FROM              QC_LotData INNER JOIN
                            Fill_Flow_Chart ON QC_LotData.LotNo = Fill_Flow_Chart.Lot_No
WHERE          (QC_LotData.TestDate >= '".gts($_POST['datepicker1'])." 00:00:00') AND (QC_LotData.TestDate <= '".gts($_POST['datepicker2'])." 23:59:59') AND (ProdNo = 'P006-000') 
                            (QC_LotData.Chemical = 'H2SO4') AND (Fill_Flow_Chart.flow <> N'') and (Fill_Flow_Chart.flow like '%".trim($row1['fl'])."%') ORDER BY   QC_LotData.LotNo";
	$result=mssql_query($query);
	while($row=mssql_fetch_array($result)){
		$objPHPExcel->setActiveSheetIndex($j)->setCellValue("A".($i+4),$row['CustNo']);
