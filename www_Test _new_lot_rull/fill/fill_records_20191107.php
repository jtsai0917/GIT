...
//		echo "AL=". $row['AL']; 
		...
		$web[$j][5]=$row['F5'];
		$web[$j][6]=$row['F6'];
		$web[$j][7]=substr($row['F7'],0,2).":".substr($row['F7'],2,2).":".substr($row['F7'],4,2);
		$web[$j][8]=substr($row['F8'],0,2).":".substr($row['F8'],2,2).":".substr($row['F8'],4,2);
		$web[$j][9]=LEL_remnant($row['F4']);
		$web[$j][10]=$Value1;
		$web[$j][11]=$Value2;
		$web[$j][12]=$row['充填作業.充填來源TANK'];
		$web[$j][13]=fill_flow($row['F4']);
		$web[$j][14]=fill_source($row['F4']);
		$web[$j][15]=get_uname($row['fill_operator']);
		$web[$j][16]=$row['F11'];
		$web[$j][17]=$row['取樣作業.取樣數'];
		$web[$j][18]=$row['取樣作業.送出數'];
		$web[$j][19]=$row['取樣作業.送出時間'];
		$web[$j][20]=$row['分析作業.取得時間'];
		$web[$j][21]=$row['分析作業.取得數量'];
		$web[$j][22]=$row['F12'];
		$web[$j][23]=$row['分析作業.判定'];
		$web[$j][24]=$row['F13'];
		$web[$j][25]=$row['F14'];
		$web[$j][26]=$row['F15'];
		...
		if($row['PTYPE']=='DM' or $row['PTYPE']=='BTL' or $row['PTYPE']=='TOTO'){$qty=round($row['F10']);}
...
if(isset($_POST['webexcel'])){
	...
	for($n=0;$n<$XX;$n++)		
	{
		$ENG='A';
		for($i=0;$i<=26;$i++){
		$obj->setCellValue($ENG.$I,iconv('big5','utf-8',$web[$n][$i]));
		$ENG++;
		}
		$I++;
	}
...
if(isset($_POST['ax_submit'])){
	...
	while($row=mssql_fetch_array($result)){
		...
		$PDD1->pid_target=$row['PNO'];
		$PDD1->cx();
		$source_lot=chk_source($row['F4']);
		$qty=round(($row['TYSQTY']-$row['TYSQTY1']),2);
		if($row['UNITP']=='L')/// 藥品單位為公升的
		{   
				$qtyfill=round($qty/$row['LKG'],0);			
		}
		else{
			$qtyfill=round($qty,0);	
		}
...
*/
		...
		elseif((trim($row['PTYPE'])=='DM' or trim($row['PTYPE'])=='BTL')){
			$c5=$qtyfill;
			$dd=stabb($row['F5']);
			$obj
...
function stabc($ds){   //daytime-style  20150902010203=>2015/09/02 01:02:03
