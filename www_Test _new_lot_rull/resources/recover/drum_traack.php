...
回收判斷(Y/N): <input name="chk1" type="text" id="chk1" size="10"  />
		...
		if(isset($_POST['submit']))
		{
			...
			$query="select DHN_USED_COUNT,DHN_MAKE_DATE,DHN_DRUM_NO
			,DHN_DISCARD_W_DATE,DHN_DISCARD_DATE,DHN_DISCARD_MAN,DHN_TRANS_DATE,DHN_CF_DISCARD_DATE,
			CD.CTD_CUST_SHORT_NAME as CD11
			,PD1.PDD_PROD_NAME as PD11
			,[CTD_CUST_NO1]
			,[PDD_PROD_NO1]
			,[DHN_WASHED_DATE1]
			,[DHN_FILLED_DATE1]
			...
			,CD1.CTD_CUST_SHORT_NAME as CD22
			,PD2.PDD_PROD_NAME as PD22
			,[PDD_PROD_NO2]
			,[DHN_WASHED_DATE2]
			,[DHN_FILLED_DATE2]
			,[DHN_LOT_NO2]
			...
			,CD2.CTD_CUST_SHORT_NAME as CD33
			,PD3.PDD_PROD_NAME as PD33
			,[PDD_PROD_NO3]
			,[DHN_WASHED_DATE3]
			,[DHN_FILLED_DATE3]
			,[DHN_LOT_NO3]
			...
			inner join CUSTOMER_DATA as CD on DRUM_HISTORY_NORMAL.CTD_CUST_NO1=CD.CTD_CUST_NO
			left join  	CUSTOMER_DATA as CD1 on DRUM_HISTORY_NORMAL.CTD_CUST_NO2=CD1.CTD_CUST_NO
			left join 	CUSTOMER_DATA as CD2 on DRUM_HISTORY_NORMAL.CTD_CUST_NO3=CD2.CTD_CUST_NO
			inner join PRODUCT_DATA as PD1 on 	DRUM_HISTORY_NORMAL.PDD_PROD_NO1=PD1.PDD_PROD_NO
			left join PRODUCT_DATA as PD2 on 	DRUM_HISTORY_NORMAL.PDD_PROD_NO2=PD2.PDD_PROD_NO
			left join PRODUCT_DATA as PD3 on 	DRUM_HISTORY_NORMAL.PDD_PROD_NO3=PD3.PDD_PROD_NO		
			where  DHN_DRUM_NO!='' ";
			if($_SESSION['datepicker1']<>''){$query=$query." and  DHN_OUT_DATE1>='".$date1."000000' 
															and DHN_OUT_DATE1<='".$date2."999999' ";}
			...
			$query=$query." order by CD.CTD_CUST_SHORT_NAME,PDD_PROD_NO1,DHN_OUT_DATE1";
			//echo $query;
  ...
  <tr class="GridviewScrollHeader">';
			...
			while($row=mssql_fetch_array($result))
			{
				...
				echo  '<td>'.$row['DHN_USED_COUNT'].'</td>';
			  echo  '<td>'.$row['DHN_MAKE_DATE'].'</td>';
			   echo  '<td>'.$row['DHN_DRUM_NO'].'</td>';
			   $objPHPExcel->getActiveSheet()->setCellValue("A".$p,$row['DHN_USED_COUNT']);
			   $objPHPExcel->getActiveSheet()->setCellValue("B".$p,$row['DHN_MAKE_DATE']);
			   $objPHPExcel->getActiveSheet()->setCellValue("C".$p,$row['DHN_DRUM_NO']);
			   
			   
			  echo  '<td>'.$row['CD11'].'</td>';
			  echo  '<td>'.$row['PD11'].'</td>';
			   echo  '<td>'.ttd($row['DHN_WASHED_DATE1']).'</td>';
			 echo  '<td>'.ttd($row['DHN_FILLED_DATE1']).'</td>';
			  echo  '<td>'.$row['DHN_LOT_NO1'].'</td>';
			   echo  '<td>'.ttd($row['DHN_OUT_DATE1']).'</td>';		   
			  echo  '<td>'.$row['DHN_VALIDATE1'].'</td>';
			  echo  '<td>'.ttd($row['DHN_WL_RCV_DATE1']).'</td>';
			  echo  '<td>'.ttd($row['DHN_TYS_RCV_DATE1']).'</td>';      
			   echo  '<td>'.$row['DHN_RCV_DECIDE1'].'</td>';
			   echo  '<td>'.ttd($row['DHN_REMAIN_DATE1']).'</td>';
			   echo  '<td>'.$row['DHN_REMAIN_DECIDE1'].'</td>';
			    $objPHPExcel->getActiveSheet()->setCellValue("D".$p, iconv("big5","utf-8",$row['CD11']));
			$objPHPExcel->getActiveSheet()->setCellValue("E".$p,iconv("big5","utf-8",$row['PD11']));
			$objPHPExcel->getActiveSheet()->setCellValue("F".$p,ttd($row['DHN_WASHED_DATE1']));
			$objPHPExcel->getActiveSheet()->setCellValue("G".$p,ttd($row['DHN_FILLED_DATE1']));
			$objPHPExcel->getActiveSheet()->setCellValue("H".$p,$row['DHN_LOT_NO1']);
			$objPHPExcel->getActiveSheet()->setCellValue("I".$p,$row['DHN_OUT_DATE1']);
			$objPHPExcel->getActiveSheet()->setCellValue("J".$p,$row['DHN_VALIDATE1']);
			$objPHPExcel->getActiveSheet()->setCellValue("K".$p,ttd($row['DHN_WL_RCV_DATE1']));
			$objPHPExcel->getActiveSheet()->setCellValue("L".$p,ttd($row['DHN_TYS_RCV_DATE1']));
			$objPHPExcel->getActiveSheet()->setCellValue("M".$p,$row['DHN_RCV_DECIDE1']);
			$objPHPExcel->getActiveSheet()->setCellValue("N".$p,ttd($row['DHN_REMAIN_DATE1']));
			$objPHPExcel->getActiveSheet()->setCellValue("O".$p,$row['DHN_REMAIN_DECIDE1']);
			  
			   echo  '<td>'.$row['CD22'].'</td>';
			  echo  '<td>'.$row['PD22'].'</td>';
			   echo  '<td>'.ttd($row['DHN_WASHED_DATE2']).'</td>';
			 echo  '<td>'.ttd($row['DHN_FILLED_DATE2']).'</td>';
			  echo  '<td>'.$row['DHN_LOT_NO2'].'</td>';
			   echo  '<td>'.ttd($row['DHN_OUT_DATE2']).'</td>';		   
			  echo  '<td>'.$row['DHN_VALIDATE2'].'</td>';
			  echo  '<td>'.ttd($row['DHN_WL_RCV_DATE2']).'</td>';
			  echo  '<td>'.ttd($row['DHN_TYS_RCV_DATE2']).'</td>';      
			   echo  '<td>'.$row['DHN_RCV_DECIDE2'].'</td>';
			   echo  '<td>'.ttd($row['DHN_REMAIN_DATE2']).'</td>';
			   echo  '<td>'.$row['DHN_REMAIN_DECIDE2'].'</td>';
			     $objPHPExcel->getActiveSheet()->setCellValue("P".$p, iconv("big5","utf-8",$row['CD22']));
			$objPHPExcel->getActiveSheet()->setCellValue("Q".$p,iconv("big5","utf-8",$row['PD22']));
			$objPHPExcel->getActiveSheet()->setCellValue("R".$p,ttd($row['DHN_WASHED_DATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("S".$p,ttd($row['DHN_FILLED_DATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("T".$p,$row['DHN_LOT_NO2']);
			$objPHPExcel->getActiveSheet()->setCellValue("U".$p,ttd($row['DHN_OUT_DATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("V".$p,ttd($row['DHN_VALIDATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("W".$p,$row['DHN_WL_RCV_DATE2']);
			$objPHPExcel->getActiveSheet()->setCellValue("X".$p,ttd($row['DHN_TYS_RCV_DATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("Y".$p,$row['DHN_RCV_DECIDE2']);
			$objPHPExcel->getActiveSheet()->setCellValue("Z".$p,ttd($row['DHN_REMAIN_DATE2']));
			$objPHPExcel->getActiveSheet()->setCellValue("AA".$p,$row['DHN_REMAIN_DECIDE2']);
			   
			    echo  '<td>'.$row['CD33'].'</td>';
			  echo  '<td>'.$row['PD33'].'</td>';
			   echo  '<td>'.ttd($row['DHN_WASHED_DATE3']).'</td>';
			 echo  '<td>'.ttd($row['DHN_FILLED_DATE3']).'</td>';
			  echo  '<td>'.$row['DHN_LOT_NO3'].'</td>';
			   echo  '<td>'.$row['DHN_OUT_DATE3'].'</td>';		   
			  echo  '<td>'.$row['DHN_VALIDATE3'].'</td>';
			  echo  '<td>'.ttd($row['DHN_WL_RCV_DATE3']).'</td>';
			  echo  '<td>'.ttd($row['DHN_TYS_RCV_DATE3']).'</td>';      
			   echo  '<td>'.$row['DHN_RCV_DECIDE3'].'</td>';
			   echo  '<td>'.ttd($row['DHN_REMAIN_DATE3']).'</td>';
			   echo  '<td>'.$row['DHN_REMAIN_DECIDE3'].'</td>';
			   
			   $objPHPExcel->getActiveSheet()->setCellValue("AB".$p, iconv("big5","utf-8",$row['CD33']));
			$objPHPExcel->getActiveSheet()->setCellValue("AC".$p,iconv("big5","utf-8",$row['PD33']));
			$objPHPExcel->getActiveSheet()->setCellValue("AD".$p,ttd($row['DHN_WASHED_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AE".$p,ttd($row['DHN_FILLED_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AF".$p,$row['DHN_LOT_NO3']);
			$objPHPExcel->getActiveSheet()->setCellValue("AG".$p,ttd($row['DHN_OUT_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AH".$p,ttd($row['DHN_VALIDATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AI".$p,ttd($row['DHN_WL_RCV_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AJ".$p,ttd($row['DHN_TYS_RCV_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AK".$p,$row['DHN_RCV_DECIDE3']);
			$objPHPExcel->getActiveSheet()->setCellValue("AL".$p,ttd($row['DHN_REMAIN_DATE3']));
			$objPHPExcel->getActiveSheet()->setCellValue("AM".$p,ttd($row['DHN_REMAIN_DECIDE3']));
			  
			   echo  '<td>'.$row['DHN_DISCARD_W_DATE'].'</td>';
			  echo  '<td>'.$row['DHN_DISCARD_DATE'].'</td>';
			  echo  '<td>'.$row['DHN_DISCARD_MAN'].'</td>';
			   echo  '<td>'.$row['DHN_TRANS_DATE'].'</td>';
			    echo  '<td>'.$row['DHN_CF_DISCARD_DATE'].'</td>';
				echo '</tr>';
				$objPHPExcel->getActiveSheet()->setCellValue("AN".$p,$row['DHN_DISCARD_W_DATE']);
			$objPHPExcel->getActiveSheet()->setCellValue("AO".$p,$row['DHN_DISCARD_DATE']);
			$objPHPExcel->getActiveSheet()->setCellValue("AP".$p,$row['DHN_DISCARD_MAN']);
			$objPHPExcel->getActiveSheet()->setCellValue("AQ".$p,$row['DHN_TRANS_DATE']);
			$objPHPExcel->getActiveSheet()->setCellValue("AR".$p,$row['DHN_CF_DISCARD_DATE']);
			$p++;
