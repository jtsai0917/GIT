		...
		while($row = mssql_fetch_array($result))
			{
				...
				//	$objPHPExcel->getActiveSheet()->setCellValue("A3","test1");
					...
					$objDrawing->setPath('../tmp/'.$fileurl.$n.".png");
					$objDrawing->setHeight(20);
					$objDrawing->setWidth(250);
					$objDrawing->setCoordinates('D'.($n+23));
				...
				// 	$objPHPExcel->setActiveSheetIndex(0)->getCell('E28')->setCellValue('E28', 'http://localhost/tmp/'.$barcode.".png");
					$objPHPExcel->setActiveSheetIndex(0)
								...
								->setCellValue('B'.($n+2),iconv("big5","utf-8","Lot NO:"))
								->setCellValue('B'.($n+1),iconv("big5","utf-8","Lorry NO:"))
								->setCellValue('P'.$n,iconv("big5","utf-8","出荷先:"))
								->setCellValue('P'.($n+2),iconv("big5","utf-8","出荷決定書:"))
								->setCellValue('P'.($n+1),iconv("big5","utf-8","出荷日期:"))
								->setCellValue('F'.$n,iconv("big5","utf-8",get_prod_name($pid)))
								->setCellValue("T".$n,iconv("big5","utf-8",$cname))
								->setCellValue('F3',$OCL_LY_NO)
								->setCellValue('T'.($n+1),ddd($OCL_OUT_DATE))
								->setCellValue('T'.($n+2),"(".$OTD_NO.")")
								->setCellValue('F'.($n+2),$OCL_LOT_NO)
								->setCellValue('H'.($n+5),$OCL_CHK_BAR)
								->setCellValue("H".($n+6),$OCL_CHK_LOT_PASTED)
								->setCellValue('H'.($n+7),$OCL_CHK_SPEC_LINK)
								...
								->setCellValue('H'.($n+10),$OCL_CHK_UPCAP)
								->setCellValue('H'.($n+11),$OCL_CHK_HOSE)
								->setCellValue('A'.($n+19),$start)
								->setCellValue('G'.($n+19),$end)
								//->setCellValue('E28',$barcode)
					...
					$objPHPExcel->setActiveSheetIndex(0)->getStyle('A'.($n+15).':AE'.($n+15))->getFont()->setSize(9);
					for($i=$n;$i<=$n+28;$i++){$objPHPExcel->getActiveSheet()->getRowDimension($i)->setRowHeight(25.5);}
					$objPHPExcel->getActiveSheet()->getStyle('A'.($n+29))->getAlignment()->setWrapText(true);
					$n=$n+30;
				} // end if order=2		
		}  // end while
