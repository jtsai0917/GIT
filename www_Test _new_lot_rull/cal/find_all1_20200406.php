	...
	{	
		...
		/*
			$query1="	SELECT           TL.*, Sample_All.SMA_DRUMNO AS Expr1, 
                            Sample_All.SMA_SMP AS Expr2, Sample_All.SMA_DRUMNO, AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME 
...
FROM              ".$table1." AS TL INNER JOIN
			...
			*/
				$query1="SELECT DISTINCT 
                            TL.*,  
                            AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, AF.first , sampler.sampler 
FROM              ".$table1." AS TL  INNER JOIN
                            AnalyzeDesign ON TL.LotNo = AnalyzeDesign.AND_LOT_NO LEFT OUTER JOIN
...
if(isset($_POST['submit10']))
{
	...
	//	echo 1;
		...
		$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8","充填人員"));$ENG++;
			...
			$query1="select EMPLOYEE_DATA.EMP_NAME, TL.*,AF.first from ".$table1."  AS TL INNER JOIN
                            Sample_All ON TL.SampleNo = Sample_All.SMA_ID AND TL.LotNo = Sample_All.SMA_LOT INNER JOIN
