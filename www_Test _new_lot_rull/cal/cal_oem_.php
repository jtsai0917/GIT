...
$query="SELECT          AnalyzeDesign.AND_REPORT_DATETIME, EMPLOYEE_DATA.EMP_NAME, AnalyzeDesign.AND_MEMO, 
...
WHERE          (AnalyzeDesign.AND_NEED_NO>449) and (AnalyzeDesign.AND_NEED_NO<550";
...
FROM              AnalyzeDesign INNER JOIN
                            PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO INNER JOIN
                            EMPLOYEE_DATA ON AnalyzeDesign.AND_PERSON = EMPLOYEE_DATA.EMP_NO
WHERE          (AnalyzeDesign.AND_NEED_NO<151) AND (AnalyzeDesign.AND_LOT_NO ='".$_POST['textfield4']."')";
}
...
$str6="UP001";
    ...
    echo '<td>'.iconv("big5","utf-8",$row['PDD_PROD_NAME']).'</td>';
    echo '<td>'.$row['PDD_TYPE'].'</td>';
    echo '<td>'.$row['AND_LOT_NO'].'</td>';
    echo '<td>'.$row['AND_MEMO'].'</td>';
    echo '<td>'.$row['AND_REPORT_DATETIME'].'</td>';
	analyzeitem($row['AND_ITEM'],$row['AND_LOT_NO'],iconv("big5","utf-8",$row['EMP_NAME']),$row['PDD_CHEMICAL'],$row['AND_GOODS']);
