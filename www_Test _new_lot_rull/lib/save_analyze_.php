...
if(($pid=='P001-200' || $pid=='P002-190' || $pid=='P006-330' || $pid=='P050-330') and ($cid<>'C16043' and $cid<>'C20701' and $cid<>'C20703' and $cid<>'C20704' and $cid<>'C20705' and $cid<>'C20707' and $cid<>'C20708' and $cid<>'C20709' and $cid<>'C20801' and $cid<>'C20901' and $cid<>'C20904' and $cid<>'C20942' and $cid<>'C20955' and $cid<>'C20956'))
	...
	if($numRows<1){
	 ...
	 $test_items."','',".$_GET['smp_cnt'].",0,'','".$_GET['out_date']."','".$_SESSION['uid']."','".$_GET['item_style']."','".get_test_item($_GET['pid'])."')";
	 $_SESSION['dun']=$query;
	 $result = mssql_query($query); 
	 }
	}
	else{
		$query="UPDATE        dbo.AnalyzeDesign	SET AND_ITEM_ORI='".$_SESSION['item_ori']."', AND_SELECT_TYPE = '".$_GET['item_style']."', AND_ITEM = '".$test_items."', AND_SMP_DATETIME = '".$_GET['datepicker1']."', AND_OUT_DATETIME = '".$_GET['out_date']."', AND_PERSON = '".$_SESSION['uid']."'	WHERE (AND_LOT_NO = '".$_GET['lot_no']."')";
	$result = mssql_query($query);
	if($dun==1){
		$query="UPDATE        dbo.AnalyzeDesign	SET AND_ITEM_ORI='".$_SESSION['item_ori']."', AND_SELECT_TYPE = '".$_GET['item_style']."', AND_ITEM = '".$test_items."', AND_SMP_DATETIME = '".$_GET['datepicker1']."', AND_OUT_DATETIME = '".$_GET['out_date']."', AND_PERSON = '".$_SESSION['uid']."'	WHERE (AND_LOT_NO = '".$_GET['lot_no']."_')";
	 $result = mssql_query($query); 
	 }
	}
