...
<table border="1" width="720">
    ...
    <td>
      ...
      <input type="submit" name="add1" id="add1" value="儲存"  />
...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_INDEX'] ?>" <?php echo testnotonick($row['ANI_INDEX'],$_GET['pid'],trim($_SESSION['ani_all']).",")?>/></td></tr>
<?php
}
?>
</table>
...
if(isset($_POST["normal"])){
	...
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result)){
	$_SESSION['ani_all'].= $row['ANI_INDEX'].",";
	}

$url="index.php?url=ani_all&id=".$_GET['id']."&pid=".$_GET['pid'];
jumpto($url);
}
...
if (isset($_POST["add1"])){
	if($_GET['id']<>''){
	$aa=$_POST['chkbox'];
    $_SESSION['ani_all']=implode(",", $aa);
	$query="UPDATE        dbo.Analyze_Rulls
	SET                  Rull_Total ='".$_SESSION['ani_all']."'
	WHERE         ([index] = ".$_GET['id'].")";
$result = mssql_query($query);
	}
