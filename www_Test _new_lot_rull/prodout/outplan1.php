...
if($_SESSION['datepicker1']==''){$_SESSION['datepicker1']=$_SESSION['datepicker2']=date("m/d/Y");}
	...
	echo '<td width="130">客戶</td><td width="50">訂單號碼</td><td width="100">狀況</td><td width="100">P/O No:</td><td width="100">出荷計劃日</td></tr>';
...
while($row = mssql_fetch_array($result))
{
	...
	if($sag->numrows==0){$lot_no="";}else{$lot_no="已轉為決定書";}
		...
		get_cust_name($row['OAF_DELI_CUST_NO']).'  ('.$row['OAF_DELI_CUST_NO'].')</a></td><td>'.$row['OPM_ORDER_NO'].'</td><td>'.$lot_no.'</td>
