...
$reader= PHPExcel_IOFactory::createReaderForFile("C:/upload_tmp/up.xlsx");
$reader->setReadDataOnly(true);
$excel= $reader->load("C:/upload_tmp/up.xlsx");	
...
$j=0;
echo '<form action="" method="post" name="form1"><input type="submit" name="submit" value="Update value"><table border="1"><tr><td>NO</td><td>主科目</td><td>來源部門</td><td>目標部門</td><td>分攤率</td></tr>';
for($i=3;$i<=$highestRows;$i++){
...
if(isset($_POST['submit'])){
	for($x=0;$x<$j;$x++){
		$query="UPDATE TYS_EXPENSESALLOCATIONSETUP SET PERCENTAGE = ".trim($_POST['D'.$x])."  WHERE (MAINACCOUNT = N'".trim($_POST['A'.$x])."') AND (FROM_DEPARTMENT = N'".trim($_POST['B'.$x])."') AND (TO_DEPARTMENT = N'".trim($_POST['C'.$x])."')";
