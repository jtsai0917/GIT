...
{	
	...
	$CHK="select BOA_NO,BOA_TIMES,BOA_BEGIN_DATE
	from BOARD_ALL
	where BOA_NO='".$_POST['name']."'";
	$resultCHK = mssql_query($CHK);
	$numrowsCHK = mssql_num_rows($resultCHK);
	while($rowCHK=mssql_fetch_array($resultCHK))
	{
		$times=$rowCHK['BOA_TIMES']+1;
	}
	if($numrowsCHK==0)
		{ assdasdas
			echo "棧板名稱錯誤";
		...
		if($numrowsCHK>0)
		{
			$NEW1="INSERt INTO BOARD_ALL(BOA_NO,BOA_BEGIN_DATE,BOA_TIMES)
			VALUES ('".$_POST['name']."','".$date1."','".$times."')";
			$resultNEW1 = mssql_query($NEW1);
			echo "更新";
		}
}*/
