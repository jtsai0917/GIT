...
//		echo $query;
	...
	include("../connections/conn.php");
		...
		for($j=1;$j<$i;$j++){			
			...
			//
			if(substr(trim($aa[$j][6]),-3)<>'000' and substr(trim($aa[$j][6]),-3,1)<>'0' and trim($aa[$j][11])==5){
				$lot=trim($aa[$j][13])."D".substr(trim($aa[$j][6]),-3);
			}
			elseif(substr(trim($aa[$j][6]),-3)<>'000' and substr(trim($aa[$j][6]),-3,1)=='0' and trim($aa[$j][11])==5){
...
//			echo $query."<BR>";
			...
			if($row[0]==0){	
				...
				$query="INSERT INTO OUT_PLAN_FROM_FILE (OAF_CUST_ORDER_NO, OAF_DEP_NO , OAF_PROD_NAME, OPM_ORDER_NO,  OAF_SERIAL_NO, OAF_ETA_DATE, PDD_PROD_NO, OAF_DELI_CUST_NO, OAF_DELI_CUST_NAME, OAF_ORDER_CUST_NO, OAF_ORDER_CUST_NAME) VALUES ('".$aa[$cc[$x]][5]."',2,'".get_prod_name($aa[$cc[$x]][3])."','".$aa[$cc[$x]][2]."', 1, '".gtm($aa[$cc[$x]][9])."', '".$aa[$cc[$x]][6]."', '".$aa[$cc[$x]][3]."', '".get_cust_sname($aa[$cc[$x]][3])."', '".$aa[$cc[$x]][4]."', '".get_cust_sname($aa[$cc[$x]][4])."')";
 		//		echo $query."<BR>";
				$result=mssql_query($query);
			}
			...
			$lkg=$pdd->lkg;
			$skg=$aa[$cc[$x]][7]*$lkg;
			if(strtoupper($aa[$cc[$x]][8])=='KG'){$OPD_QTY_KG=$OPD_REAL_QTY_KG=$aa[$cc[$x]][7];$OPD_QTY_LITER=$aa[$cc[$x]][7]/$lkg;}
			if(strtoupper(trim($aa[$cc[$x]][8]))=='L'){$OPD_QTY_KG=$OPD_REAL_QTY_KG=$skg;$OPD_QTY_LITER=$aa[$cc[$x]][7];}
	/*		
	...
	*/
			...
			if($pdd->type=='DM' or $pdd->type=='BTL'){$OPD_QTY_DRUM=$OPD_QTY_LITER/($pdd->drum_liter);}else{$OPD_QTY_DRUM=0;}
			
			$query="INSERT INTO OUT_PRODUCT
					   (OAF_PACKAGE, COPCONFIRMSERIES, OPD_ACC_UNIT, OPM_ORDER_NO, OPD_SERIAL_NO, OTD_NO, PDD_PROD_NO, OPD_LOT_NO, OPD_QTY_DRUM, OPD_QTY_KG,OPD_QTY_LITER, OPD_COA_NO, OPD_MEMO, OPD_REAL_QTY_KG, OPD_INWARD_DATE)	VALUES  
					   ('".$aa[$cc[$x]][19]."', '".strtoupper($aa[$cc[$x]][8])."', '".$aa[$cc[$x]][2]."',".$nn.",'".$aa[$cc[$x]][16]."','".$aa[$cc[$x]][18]."','".$aa[$cc[$x]][13]."',".round($OPD_QTY_DRUM).",".round($OPD_QTY_KG).",".round($OPD_QTY_LITER).",'".$aa[$cc[$x]][10]."','".$aa[$cc[$x]][12]."','".round($aa[$cc[$x]][7],0)."','".date("Ymd")."')";
			//echo $query;
			$result=mssql_query($query);
	//		echo '<BR>';
...
class get_pdd_literkg
{
	...
	function liter_kg(){
		$query="SELECT  PDD_TYPE, PDD_DRUM_KG, PDD_LITER_KG, PDD_UNIT, PDD_DRUM_LITER FROM PRODUCT_DATA WHERE (PDD_PROD_NO = '".$this->pid."') ";	
		$result=mssql_query($query);
		$row=mssql_fetch_row($result);
		$this->type=$row[0];
		$this->drum_kg=$row[1];
