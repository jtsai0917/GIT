...
<option value="Co" <?php if($_POST['work']=='Co') echo 'selected';?>>Co</option>
 ...
 if($_POST['work']=='Co'){echo '<a href=/cal/RR/'.$_GET['pdd_chemical'].'_Co.xls target="new">原始範本</a>';}
  ...
  if(isset($_POST['printrr0'])){
 	...
 	$queryo="select distinct top 3 first,min(create_time) create_time from analyze_first where lot_no like '".$lot_no."%' group by first order by create_time";
//	echo $queryo;
	$times=0;
...
//	echo "<BR>".$u1.get_uname($u1)."#".$u2.get_uname($u2)."#".$u3.get_uname($u3)."#".
...
//		echo "<BR>".$query."<BR>";
		...
		{ 
			...
			/*
			if($u1==$first){
				echo $x."U1:#".$u1."#First:#".$first."#NA:#".$row['Na']."<BR>";
			}
			if($u2==$first){
				echo $x."U2:#".$u2."#First:#".$first."#NA:#".$row['Na']."<BR>";
			}
			if($u3==$first){
				echo $x."U3:#".$u3."#First:#".$first."#NA:#".$row['Na']."<BR>";
			}
			$x++;
			*/
//			echo "#".$first."#-U1#".$u1."#U2#".$u2."#U3#".$u3."#SN#".$sn."#<BR>";
	...
	$objPHPExcel->getActiveSheet()->getStyle('D7:P55')->getFill()->applyFromArray(array('type' => PHPExcel_Style_Fill::FILL_SOLID,'startcolor' => array('rgb' =>'F0F0F0')));
	setbg($objPHPExcel,13);
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
	if (unlink("../tmp/tentimes.xlsx")) {
    echo "檔案成功刪除。";
} else {
    echo "無法刪除檔案。";
}
 ...
 function setbg($objPHPExcel,$n){
 	for ($i=1;$i<= $n;$i++){
 		$objPHPExcel->setActiveSheetIndex($i);	
 		$objPHPExcel->getActiveSheet()->getStyle('D6:H20')->getFill()->applyFromArray(array('type' => PHPExcel_Style_Fill::FILL_SOLID,'startcolor' => array('rgb' =>'F0F0F0')));
 	}
 	
 }
 function rnw($objPHPExcel,$location,$na,$mg,$al,$k,$ca,$cr,$mn,$fe,$ni,$co,$cu,$zn,$pb){
 //	echo $location."<BR>";
 		if($na=='1.0E-6'){$na='0.000001';}
 		if($mg=='1.0E-6'){$mg='0.000001';}
 		if($al=='1.0E-6'){$al='0.000001';}
 		if($k=='1.0E-6'){$k='0.000001';}
 		if($ca=='1.0E-6'){$ca='0.000001';}
 		if($cr=='1.0E-6'){$cr='0.000001';}
 		if($mn=='1.0E-6'){$mn='0.000001';}
 		if($fe=='1.0E-6'){$fe='0.000001';}
 		if($ni=='1.0E-6'){$ni='0.000001';}
 		if($co=='1.0E-6'){$co='0.000001';}
		if($cu=='1.0E-6'){$cu='0.000001';}
 		if($zn=='1.0E-6'){$zn='0.000001';}
 		if($pb=='1.0E-6'){$pb='0.000001';}
...
///////橫的列印 藥品上到下
	...
	{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/RR/tsmc GRR.xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
