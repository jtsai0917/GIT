...
elseif($_GET['lot_no']){$_SESSION['lot_no']=$_GET['lot_no'];}
	...
	$_SESSION['items']=item_all($bb->cid,$bb->pid);
...
<label for="urgent">先行COA：
  <input type="checkbox" name="and_coa_fst" id="and_coa_fst" <?php 
	  ...
	  if($_SESSION['and_total']<9){$_SESSION['and_total']=9;}
...
<input name="new" type="button" id="new" value="新增/修改" readonly="readonly" />
</span></td>
    </tr>
     <tr>
       <td><input type="checkbox" name="deliver_time" id="deliver_time" />
		 ...
		 if($_GET['outdate']<>''){$_SESSION['outdate']=$_GET['outdate'];}
...
?> onClick="return confirm('確定取消Lot：<?php echo $_SESSION['lot_no']?> 的依賴?')"/>
	...
	if($_SESSION['class_name']<>'受入')
	{
		if($numRows<1)
		{
			$query="INSERT INTO dbo.AnalyzeDesign
								(AND_COA_FST, AND_RESULT,AND_RESULT_DATETIME,AND_GET_DATETIME,AND_GET_QTY,AND_MARK,AND_VALUE,AND_ANA_ID,AND_APPLY_DATE, AND_NEED_NO, AND_LOT_NO, AND_GOODS, AND_BEFORE, 
					 ...
					 VALUES          ('".$and_coa_fst."','','','','0','".$urgent."','','','".date("Ymd")."',".$_SESSION['needno'].",'".$_POST['lot_no']."','".$_POST['pdd_chemical1']."','".$before."','".$outtime."','".
		...
		else
		{
			$query="UPDATE        dbo.AnalyzeDesign
					SET                  	   AND_APPLY_DATE = '".dod($_POST['datepicker4'])."',AND_GOODS = '".$_POST['pdd_chemical1']."',AND_TOTAL = '".$_POST['sample_number']."', AND_BEFORE = '".$before."', AND_SMP_DATETIME = '".$smptime."', AND_NEED_NO='".$_POST['class']."',
	...
	else
	{
		...
		else
		{
			$query="UPDATE        dbo.AnalyzeDesign
					SET                  	  AND_APPLY_DATE = '".dod($_POST['datepicker4'])."',AND_GOODS = '".$_POST['pdd_chemical1']."',AND_TOTAL = '".$_POST['sample_number']."', AND_BEFORE = '".$before."', AND_SMP_DATETIME = '".$smptime."', AND_NEED_NO='".$_POST['class']."',
							  AND_ITEM = '".$itm."', AND_NOTE = '".$_POST['remark']."', AND_NOTE2 = '".$_POST['remark2']."', AND_OUT_DATETIME = '".$outtime."', AND_COA_FST = '".$and_coa_fst."', 
							  AND_REPORT_DATETIME = '".$ordertime."', AND_MARK = ".$urgent."  	WHERE         (AND_LOT_NO = '".$_POST['lot_no']."')";
		}
...
if($numRows>0){
				...
				$asd->lid=$_POST['lot_no'];
				$asd->cid();
				$_SESSION['asdcid']=$asd->cid;
				$_SESSION['AND_GOODS']=$row['AND_GOODS'];
				$_SESSION['items']=item_all($asd->cid,$row['AND_GOODS']);
				$_SESSION['testitems']=$row['AND_ITEM'];
				$_SESSION['smptime']=$row['AND_SMP_DATETIME'];	
				$_SESSION['outtime']=$row['AND_OUT_DATETIME'];
				$_SESSION['ordertime']=$row['AND_REPORT_DATETIME'];	
...
function item_all($cid,$pid)
{
	$ID='';
	$queryp="select Monthly_first FROM Analyze_Rulls WHERE CTD_CUST_NO LIKE '%".$cid."%' and PDD_PROD_NO='".$pid."'";
	...
	if(trim($ID)==',')
	{///$numrowp=0
	$queryp="select Monthly_first FROM Analyze_Rulls WHERE CTD_CUST_NO LIKE '%C00001%' and PDD_PROD_NO='".$pid."'";
	$resultp = mssql_query($queryp);
	$numrowp=mssql_num_rows($resultp);
	if($numrowp>0){
		while($rowp = mssql_fetch_row($resultp))
