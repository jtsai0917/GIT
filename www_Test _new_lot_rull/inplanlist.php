...
mssql_close($dbhandle);
    ...
    <tr>
      ...
      入荷預定日:
          <input type="text" name="datepicker1" id="datepicker1" size="10" value="<?php $d=strtotime("-1 Days"); echo date("m/d/Y",$d);?>">
~
<input type="text" name="datepicker2" id="datepicker2" size="10" value="<?php $d=strtotime("+3 Days"); echo date("m/d/Y",$d);?>">      
      </span></td>
...
$titles='<table width="2360" border="1">
    <td width="45">入荷預定日</td>
    <td width="189">P/O NO</td>
    <td width="152">Invoice NO</td>
    <td width="156">入荷先</td>
    <td width="102">料號</td>
    <td width="176">品名</td>
    <td width="158">荷姿</td>
    <td width="45">單位</td>
    <td width="189">包裝型態</td>
    <td width="152">預交量</td>
    <td width="156">數量</td>
    <td width="102">入貨日</td>
    <td width="176">類別</td>
    <td width="158">入荷計畫書</td>
    <td width="102">目前狀況</td>
    <td width="176">原始COA數據</td>
    <td width="158">轉檔日期</td>';
	echo $titles;
include('connections/conn.php'); 
//mysql_select_db($database_connection, $connection);
$query = "SELECT IN_PLAN.IPA_ETA_DATE, IN_PLAN.IPA_PO_NO, IN_PLAN.IPA_INVOICE_NO, CUSTOMER_DATA.CTD_CUST_NAME, IN_PLAN_FROM_FILE.PDD_PROD_NO, IN_PLAN_FROM_FILE.IAF_PROD_NAME, PRODUCT_DATA.PDD_STYLE, IN_PLAN_PRODUCT.IAP_UNIT, PRODUCT_DATA.PDD_PACKAGE, IN_PLAN_PRODUCT.IAP_ORDER_QTY, IN_PLAN_PRODUCT.IAP_QTY, IN_PLAN.IPA_IN_DATE,IN_PLAN.IPA_TYPE, IN_PLAN_FROM_FILE.IPA_PO_NO AS Expr1, IN_PLAN_PRODUCT.IAP_STATE, IN_PLAN_PRODUCT.IAP_LOT_NO, IN_PLAN_PRODUCT.IAP_INWARD_DATE, CUSTOMER_DATA.CTD_SUPPLIER
...
if ($_POST['pono']<>""){
$query=$query." and (IN_PLAN.IPA_PO_NO LIKE '".$_POST['pono']."')";
}
if ($_POST['t1']<>""){
...
//format and display results
	...
	$ts1=$ts[0]."-".$ss[0]."-".$ss[1];
    echo ("<td>$ts1</td>");
	echo '<td><a target="_blank" href="list_po.php?po_no='.$t2.'">'.$t2.'</a></td>';
//    echo ("<td>$t2</td>");
    echo ("<td>$t3</td>");
	echo ("<td>$t4</td>");
	echo ("<td>$t5</td>");
