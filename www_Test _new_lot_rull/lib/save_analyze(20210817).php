...
if($pid=='P001-200' || $pid=='P002-190' || $pid=='P006-330' || $pid=='P050-330')
	...
	if($numRows<1 and $dun==0){
	  ...
	  VALUES          (".$_GET['sample_metal_f'].",".$_GET['sample_particle_f'].",'".$cid."','','','',0,'".'0'."','','','".date("Ymd")."',1,'".$_GET['lot_no']."','".$_GET['pid']."','N','".$_GET['datepicker1']."','".
	...
	elseif($numRows<1 and $dun==1){
		...
		echo $query;
		 ...
		 $result = mssql_query($query);
			...
			$query="INSERT INTO dbo.AnalyzeDesign
								(".$_GET['sample_metal_f'].",".$_GET['sample_particle_f'].",sample_metal_f, sample_particle_f, CTD_CUST_NO,AND_RESULT,AND_RESULT_DATETIME,AND_GET_DATETIME,AND_GET_QTY,AND_MARK,AND_VALUE,AND_ANA_ID,AND_APPLY_DATE, AND_NEED_NO, AND_LOT_NO, AND_GOODS, AND_BEFORE, AND_SMP_DATETIME, AND_ITEM, AND_NOTE, AND_TOTAL, AND_OUT_QTY, AND_OUT_DATETIME, AND_REPORT_DATETIME, AND_PERSON, AND_SELECT_TYPE, AND_ITEM_ORI)
