...
//echo "FID:*".$fid."*<BR>";
$lorry_no=trim($lorry_no);
// break;
if(trim($lorry_no)<>trim($lorry_no1))
{
...
WHERE   (FDM_LOT_NO = '".$_POST['lid']."')";
	...
	echo '<input type="submit" name="submit1" id="submit" value="  下 一 步  " style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;"/></br></font></form>';
	}
...
//		if($trim($_POST['em3'])==''){$_POST['em3']=bm3($_POST['lid'],$etq);}
	...
	if($_POST['btk']=='' or trim($_POST['btk'])=='N/A'){$btk='NULL';}else{$btk="'".$_POST['btk']."'";}
		$query="Insert Into LORRY_FILL_CHECK 	(FDM_LOT_NO,  LFC_W_LY_NO,  LFC_B_REMAIN_QTY,  LFC_B_TROUGH_TOP,  LFC_B_CHK_COUP_CLR,  LFC_B_CHK_COUP,  LFC_B_CHK_COUP_LINK,  LFC_B_CHK_EXHAUST,  
				LFC_B_SET_FILL,  LFC_F_FLOW,  LFC_F_SAM_COUNT,  LFC_F_RESISTANCE,  LFC_E_OPER_FILL,  LFC_B_TROUGH_QTY,  LFC_E_CHK_AIR_SEAL,  LFC_E_CHK_FILL_CLOSE,  LFC_E_CHK_EXHAUST,  LFC_E_CHK_VALVE,  
				LFC_E_CHK_COUP_CLR,  LFC_E_CHK_COUP,  LFC_E_CHK_PIPE_PICKUP,  LFC_FILLER,  LFC_AIR_SEAL,  LFC_AIR_SEAL_START,  LFC_AIR_SEAL_END,  LFC_E_CHK_MEGA_CHECK,  LFC_E_CHK_SURFACE,  LFC_TANK,  LFC_E_TROUGH_TOP, LFC_E_TROUGH_QTY) 
...
function tank(){
	...
	echo '<input type="hidden" name="purge_qty" id="qty" value="'.$_POST['purge_qty'].'" />';
	echo '<input type="hidden" name="flow_speed" value="'.$_POST['flow_speed'].'" />';
	echo '來源 LOT NO:<input type="text" style="font-size:20" size="15" name="source_lot" value="" /><BR>';
	...
	echo '<BR>選擇充填路徑<table width="600" border="1" bgcolor="#CCCCCC"><tr><td width="100">系別</td><td width="100">TANK</td><td width="100">Pump</td><td width="100">TANK1</td></tr><tr><td>';
	select_series($pid);
	echo '</td><td>';
	...
	echo '</td></tr><tr>';
	echo '<td width="100">Pump1</td><td width="100">Filter 1</td><td width="100">Filter 2</td><td width="100">Filter 3</td></tr><td>';
	select_Pump1($pid);
	echo '<br><input type="text" size="5" name="p1">kg/cm^2';
	echo '</td><td>';
	select_Filter1($pid);
	...
	echo '</td><td>';
	select_Filter3($pid);
	echo '<br><input type="text" size="5" name="f3">kg/cm^2';
	echo '</td></tr><tr><td>充填站</td><tr><td>';
	select_Spot($pid);
	echo '</td></tr></table>';	
	echo '</br></br><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="finish_1" value="   暫存並離開   " /><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="finish" value="   下 一 步   " /><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="submit3" value="上一步" /></br>';
}
...
//	echo "<BR>".$query."<BR>";
	...
	if($numr==0){
		...
		$query="INSERT INTO lorry_fill_tmp (lot_no, remnant, m3, btk, [mega-check], purge_qty, tank_no, LFC_TANK, flow_speed, smp_cnt, update_time, lorry_no, qty, bm3)
			VALUES  (N'".$_SESSION['lid']."', ".$_POST['remnant'].", ".$_POST['m3'].", '".$_POST['btk']."', ".$_POST['mega_check'].",".$_POST['purge_qty'].", N'".trim($_POST['tank_no'])."', N'".trim($_POST['tank_no'])."', ".$_POST['flow_speed'].", ".$_SESSION['smp_cnt'].", N'".date("YmdHis")."', '".trim($_SESSION['lorry_no'])."',".$_POST['qty'].",".$_POST['bm3'].")";
		$result=mssql_query($query);
	}
	else{
		while($row=mssql_fetch_array($result)){
