...
echo 'ID <select name="id" onchange="set_date_session(this.name,this.value)"><option value=""></option>';

foreach($aa as $value) {
	$bb=explode(",",$value);
	if($value==$_SESSION['id']){$s1='selected';}else{$s1='';}
  echo '<option value="'.$value.'" '.$s1.'>'.$bb[0].'</option>';
}
echo '</select><BR>';
...
echo '<input type="submit" name="add" value="'.iconv("utf-8","big5",'新增').'"> &nbsp;<br>';
echo '</form>';
if(isset($_POST['sub1'])){
	$query="update istar_default_setting set name='".$_POST['item']."' where [index]=".$cc[0];
	$result=mssql_query($query);
	refresh();
}
if(isset($_POST['sub2'])){
	$query="update istar_default_setting set [value]='".$_POST['value']."' where [index]=".$cc[0];
	$result=mssql_query($query);
	refresh();
}
if(isset($_POST['sub3'])){
...
if(isset($_POST['add'])){
	$query="INSERT INTO istar_default_setting
                            (name, [value], active, creator, createtime, [level], UNIT, [order])
VALUES          ('".$_POST['item']."', '".$_POST['value']."', 1, N'".$_SESSION['uid']."', '".date("Y-m-d H:i:s")."', 0, N'kg', ".$_POST['order'].")";
//echo $query."<BR>";
	$result=mssql_query($query);
	refresh();
}
