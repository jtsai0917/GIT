...
//	echo $query."<BR>";
	...
	//	echo $rows['ANI_DATAFIELD'].":".$rows['ELF_FORM'].":".$_POST['lotno']."<BR>";
		...
		if( $AA[2]<>''){
			...
			echo '<tr><td>'.$i.'</td><td>'.$rows['ANI_GROUPNAME'].'</td><td>'.$rows['ANI_NICKNAME'].'</td><td>'.$rows['ItemName'].'</td><td>'.$rows['ELF_FORM'].'</td><td>'.$rows['ANI_DATAFIELD'].'</td><td>'.$AA[2].'</td>
			<td>'.$val1.'</td><td>'.$rows['ItemUnit'].'</td><td>'.$AA[3].'</td><td>'.$AA[0].'</td></tr>
			
			<input type="hidden" name="lno'.$i.'" value="'.$_POST['lotno'].'"><input type="hidden" name="testdate'.$i.'" value="'.sta($AA[0]).'"><input type="hidden" name="sample_no'.$i.'" value="'.$AA[2].'"><input type="hidden" name="item'.$i.'" value="'.$rows['ANI_NICKNAME'].'"><input type="hidden" name="value'.$i.'" value="'.$val1.'"><input type="hidden" name="ELF_FORM'.$i.'" value="'.$rows['ELF_FORM'].'"><input type="hidden" name="ANI_DATAFIELD'.$i.'" value="'.$rows['ANI_DATAFIELD'].'"><input type="hidden" name="ELF_FORM'.$i.'" value="'.$rows['ELF_FORM'].'"><input type="hidden" name="itemname'.$i.'" value="'.$rows['ItemName'].'"><input type="hidden" name="itemname'.$i.'" value="'.$rows['ItemName'].'"><input type="hidden" name="itemunit'.$i.'" value="'.$rows['ItemUnit'].'"><input type="hidden" name="ok'.$i.'" value="'.$AA[4].'"><input type="hidden" name="Tester'.$i.'" value="'.$AA[5].'"><input type="hidden" name="Operator'.$i.'" value="'.$AA[6].'"><input type="hidden" name="anatime'.$i.'" value="'.$AA[0].'">
			';
			$i++;
		}
	}
...
if(isset($_POST['trans'])){
	for($i=1;$i < $_POST['count'];$i++){
			$query="delete from QC_LotData where LotNo='".$_POST['lno'.$i]."'";
			$result=mssql_query($query);
	}
	echo '轉檔中..';
	for($i=1;$i < $_POST['count'];$i++){
