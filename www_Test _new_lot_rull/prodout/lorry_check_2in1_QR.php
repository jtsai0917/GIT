...
//// Starting excel
	...
	for($i=0;$i<$num;$i++)
	{
		$query="SELECT          FILLPLAN_OUT_DECIDE.PDD_PROD_NO, FILLPLAN_OUT_DECIDE.FDM_LOT_NO, 
                            ...
                            FILL_INDICATE.FID_FILL_BEGIN_DATE, CUSTOMER_PRODUCTS.CTD_CUST_NO, 
                            CUSTOMER_PRODUCTS.PDD_PROD_NO AS Expr1
FROM              FILLPLAN_OUT_DECIDE INNER JOIN
                            CUSTOMER_PRODUCTS ON 
                            FILLPLAN_OUT_DECIDE.PDD_PROD_NO = CUSTOMER_PRODUCTS.PDD_PROD_NO AND 
                            FILLPLAN_OUT_DECIDE.CTD_CUST_NO = CUSTOMER_PRODUCTS.CTD_CUST_NO INNER JOIN
                            FILL_INDICATE ON FILLPLAN_OUT_DECIDE.FDM_LOT_NO = FILL_INDICATE.FDM_LOT_NO 
		...
		while($row = mssql_fetch_array($result))
			{
				...
				$produce_date=substr(trim($row['FID_FILL_BEGIN_DATE']),0,8);
				$barcode=$row['CTP_BIGHOSEBAR'];
			...
			//	echo "<BR>19115151<BR>";	
				...
				//	echo "<BR>19115151<BR>";	
					...
					$objPHPExcel->setActiveSheetIndex(0)
								...
								->mergeCells('F'.($n+12).':F'.($n+13))
								->mergeCells('C'.($n+4).':C'.($n+5))
								->mergeCells('F'.($n+4).':F'.($n+5))
								->mergeCells('C'.($n+6).':C'.($n+7))
								->mergeCells('F'.($n+6).':F'.($n+7))
								->mergeCells('C'.($n+8).':C'.($n+9))
								->mergeCells('F'.($n+8).':F'.($n+9))
								->mergeCells('C'.($n+10).':C'.($n+11))
								->mergeCells('F'.($n+10).':F'.($n+11))
								;	
					...
					$objPHPExcel->setActiveSheetIndex(0)->getStyle('C'.($n+4).':F'.($n+13))->getBorders()->getAllborders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);	
...
function getqrl($id){
    ...
    return $filename = $PNG_TEMP_DIR.$_REQUEST['data'].'.png';
}
