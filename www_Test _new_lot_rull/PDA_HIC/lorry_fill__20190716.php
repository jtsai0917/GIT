...
if($short_name<>trim($fid))
	{
		$_SESSION['lid']=$_SESSION['fid']=$_SESSION['lorry_no1']=$_POST['lid']='';
		my_msg("充填口錯誤，請檢查原因","fill.php");
	}
...
if(isset($_POST['end'])) 
{
	...
	$smp_cnt=samp_count(trim($_SESSION['lid']));
 	$query="select count(*) as cnt from  LORRY_FILL_CHECK where FDM_LOT_NO='".trim($_SESSION['lid'])."'"; 
	echo $query."<BR>";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	if($row[0]==0){		
	// 1. Insert into lorry_fill_check
		...
		$query="Insert Into LORRY_FILL_CHECK 	(FDM_LOT_NO,  LFC_W_LY_NO,  LFC_B_REMAIN_QTY,  LFC_B_TROUGH_TOP,  LFC_B_CHK_COUP_CLR,  LFC_B_CHK_COUP,  LFC_B_CHK_COUP_LINK,  LFC_B_CHK_EXHAUST,  
				...
				VALUES ('".$_SESSION['lid']."','".trim($lorry_no)."',".$remnant.",".$m3.",'Y','Y','Y','Y',".$qty.",".$flow_speed.",".$smp_cnt.",'".$btk."',".$qty.",'".$_POST['m32']."','Y','Y','Y','Y','Y','Y','Y','".$_SESSION['uid']."',".$_POST['sti0'].",'".$_POST['sti1']."','".$_POST['sti2']."','1','Y','".$tank_no."')";
		$result1 = mssql_query($query);
...
function sm1(){
	...
	echo '控制室(m^3)(%)： <input type="text"  autocomplete="off" name="m3" id="m3"  style="font-size:'.($_SESSION['font_size']).'px" size="12"   autofocus="autofocus" /></br>';
...
function tank(){
	...
	echo '<BR>選擇充填路徑<table width="400" border="1" bgcolor="#CCCCCC"><tr><td width="100">系別</td><td width="100">TANK</td><td width="100">Pump</td><td width="100">TANK1</td></tr><tr><td>';
	select_series($pid);
	echo '</td><td>';
	...
	select_Spot($pid);
	echo '</td></tr></table>';	
	echo '</br></br><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="finish" value="   下 一 步   " /><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="submit3" value="上一步" /></br>';
}
...
VALUES  ('".trim($_SESSION['lid'])."', '".$pid."', '".$c."', '".date("Ymd")."', 100, 0, 0, 0, 0)";
	...
	if($result6){}else{break;}
	
	// 7. 設定充填路徑
	$query="INSERT INTO dbo.Fill_Flow_Chart (Lot_No, flow, creator, date) VALUES ('".$_SESSION['lid']."','".$flow."','".$_SESSION['uid']."','".date("YmdHis")."')";	
//	echo "<BR>".$query."<BR>";
	$result=mssql_query($query);
/*	
...
//	echo "<BR>".$query."<BR>";
	...
	if($result5){;}else{break;}
	
	$query="select * from lorry_fill_tmp where lot_no='".$_SESSION['lid']."'";
	$result=mssql_query($query);
	$numr=mssql_num_rows($result);
	if($numr==0){
		if(trim($_POST['btk'])=='N/A'){$_POST['btk']=NULL;}
		$query="INSERT INTO lorry_fill_tmp (lot_no, remnant, m3, btk, [mega-check], purge_qty, tank_no, LFC_TANK, flow_speed, smp_cnt, update_time, lorry_no, qty)
			VALUES  (N'".$_SESSION['lid']."', ".$_POST['remnant'].", ".$_POST['m3'].", '".$_POST['btk']."', ".$_POST['mega_check'].",".$_POST['purge_qty'].", N'".trim($_POST['tank_no'])."', N'".trim($_POST['tank_no'])."', ".$_POST['flow_speed'].", ".$_SESSION['smp_cnt'].", N'".date("YmdHis")."', '".trim($_SESSION['lorry_no'])."',".$_POST['qty'].")";
//		echo "<BR>".$query."<BR>";
		$result=mssql_query($query);
	}
	else{
	...
	echo '(m^3)： <input type="text"  autocomplete="off" name="m32" id="m32"  style="font-size:'.($_SESSION['font_size']).'px" size="12"  autofocus="autofocus" value="90" /></br>';
