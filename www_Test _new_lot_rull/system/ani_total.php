...
<table border="1" width="720"><tr>
  <td align="center"><input type="submit" name="reg" id="reg" value="常規" />
    <input type="submit" name="total" id="total" value="全項" /> 
    項目: <?php echo $_SESSION['disc']?></td></tr></table></br>
<table border="1" width="720"><tr>  
    <td>
      項目簡稱：
      ...
      <input type="submit" name="add1" id="add1" value="確定"  />
	  <input type="submit" name="clear" id="clear" value="清除所有項目"  /> 
    
      <input type="submit" name="quit" id="quit" value="離開"  />
    </tr>
  </table>
...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_INDEX'] ?>" <?php echo testnotonick($row['ANI_INDEX'],$_GET['pid'],$_SESSION['ani_total'].",")?>/></td></tr>
<?php
}
?>
</table>
...
//////////////////////////////////////////////////////////
if(isset($_POST['reg'])){
$_SESSION['disc']="常規"; $column="Rull_Reg";
refresh();
}
if(isset($_POST['total'])){
$_SESSION['disc']="全項"; $column="Rull_Total";
refresh();
}
...
if(isset($_POST["normal"])){
	...
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result)){
	$_SESSION['ani_total'].= $row['ANI_INDEX'].",";
	}

$url="index.php?url=ani_total&id=".$_GET['id']."&pid=".$_GET['pid'];
jumpto($url);
}
...
if (isset($_POST["add1"])){
	if($_SESSION['disc']=="常規"){$column='ani_regular';}
	if($_SESSION['disc']=="全項"){$column='ani_all';}
    $_SESSION[$column]=substr($_SESSION['ani_total'],0,-1);
}
if (isset($_POST["quit"])){
