...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_NICKNAME']; ?>" <?php echo testnotonick($row['ANI_FULLNAME'],$_SESSION['prod_no'],$_SESSION['items'].",")?>/></td></tr>
<?php
}
?>
</table>
...
if(isset($_POST["normal"])){
	...
	$result = mssql_query($query);
		$numRows = mssql_num_rows($result);
		while($row = mssql_fetch_array($result)){
		$_SESSION['items'].= $row['ANI_FULLNAME'];}
		echo $_SESSION['items'];
		$url="index.php?url=add_testitems2";
		jumpto($url);
