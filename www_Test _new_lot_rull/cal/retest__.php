...
if((isset($_POST['search']) and $_POST['EMP_NO']=='0') or $_GET['page']<>'NULL'){	
	...
	$query="SELECT          LotNo, PDD_NO, ani_group,Analyzetime  
FROM              (SELECT          *, row_number() OVER (ORDER BY lotno) AS rn
FROM              RE_TEST where  (Analyzetime >= '".dod(trim($_SESSION['datepicker1']))."000000' AND Analyzetime <= '".dod(trim($_SESSION['datepicker2']))."235959')
";

	if($_POST['selected_group']<>''){ $query.= "and ani_group='".trim($_POST['selected_group'])."' ";}
	if($_SESSION['pid']<>''){ $query.= "and PDD_NO='".trim($_SESSION['pid'])."' ";}
$query.= "
) RE_TEST
WHERE         ( rn BETWEEN 50 AND 99 )  
";
