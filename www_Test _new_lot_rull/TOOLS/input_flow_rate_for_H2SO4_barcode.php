...
session_start();
?>
<form action="" method="post" name="123">區間：
...
FROM              AnalyzeDesign INNER JOIN
                            PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO   WHERE (AND_APPLY_DATE >= '".dod($_POST['datepicker1'])."') and (AND_APPLY_DATE <= '".dod($_POST['datepicker2'])."') AND (AnalyzeDesign.AND_ITEM LIKE '%,272%') AND (PRODUCT_DATA.PDD_CHEMICAL = 'H2SO4')";
echo $query."<br>";					
...
//	echo $query."<br>";
	...
	if($createdate==''){$createdate=$apply_date."000000";}
	/*
...
VALUES          (CONVERT(DATETIME, '".sta($createdate)."', 102), N'".$lotno."', 1, '".$smp."', ".$flow_speed.", N'1', N'adm', 'adm', 'adm', 
                            N'".$createdate."')";
//	echo $query."<BR>";		
	$result=mssql_query($query);
}