...
<table border="1" width="720">
    ...
    <td>
      ...
      <input type="submit" name="add1" id="add1" value=" 儲 存 "  />
      <input type="submit" name="leave" id="leave" value=" 離 開 "  />      </tr>
  </table>
  <table border="1" width="720">
  <tr>
  <td>  序號  </td>
...
WHERE          (dbo.PRODUCT_DATA.PDD_PROD_NO = '".$_GET['pid']."')";
  if($_POST['group_name']<>""){
...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_INDEX'] ?>" <?php echo $_SESSION['tmp']=testnotonick($row['ANI_INDEX'],$_GET['pid'],trim($_SESSION['ani_m_fst']).",")?>/></td></tr>
<?php
}
?>
</table>
...
if(isset($_POST["normal"])){
	...
	WHERE          (dbo.RegularAnalyzeItem.PROD_NO = '".$_GET['pid']."')";
	$result = mssql_query($query);
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result)){
	$_SESSION['ani_m_fst'].= $row['ANI_INDEX'].",";
	}

$url="index.php?url=ani_m_fst&id=".$_GET['id']."&pid=".$_GET['pid'];
jumpto($url);
}
...
FROM              dbo.ELEMENT_FORM INNER JOIN
                            dbo.PRODUCT_DATA ON dbo.ELEMENT_FORM.PDD_CHEMICAL = dbo.PRODUCT_DATA.PDD_CHEMICAL INNER JOIN
                            dbo.AnalyzeItem ON dbo.ELEMENT_FORM.ELM_ID = dbo.AnalyzeItem.ANI_INDEX
WHERE          (dbo.PRODUCT_DATA.PDD_PROD_NO = '".$_GET['pid']."') AND ((dbo.AnalyzeItem.ANI_GROUPNAME = 'M13') or (dbo.AnalyzeItem.ANI_GROUPNAME = 'M21') or (dbo.AnalyzeItem.ANI_GROUPNAME = 'TT') or (dbo.AnalyzeItem.ANI_GROUPNAME = 'TM'))";
			$result = mssql_query($query);
		$numRows = mssql_num_rows($result);
		while($row = mssql_fetch_array($result)){
		$_SESSION['ani_m_fst'].= $row['ANI_INDEX'].",";}
...
if (isset($_POST["add1"])){
	if($_GET['id']<>''){
	$aa=$_POST['chkbox'];
    $_SESSION['ani_m_fst']=implode(",", $aa);
	$query="UPDATE        dbo.Analyze_Rulls
	SET                  Monthly_first ='".$_SESSION['ani_m_fst']."'
	WHERE         ([index] = ".$_GET['id'].")";
$result = mssql_query($query);
	}
...
if(isset($_POST["m13"])){
	...
	$query="SELECT DISTINCT dbo.PRODUCT_DATA.PDD_PROD_NO, dbo.AnalyzeItem.ANI_INDEX, dbo.AnalyzeItem.ANI_ID, 
                            dbo.AnalyzeItem.ANI_NICKNAME, dbo.AnalyzeItem.ANI_FULLNAME, dbo.AnalyzeItem.ANI_UNIT, 
                            dbo.AnalyzeItem.ANI_ANR_ID, dbo.AnalyzeItem.ANI_GROUPNAME, dbo.AnalyzeItem.ANI_DATAFIELD, 
                            dbo.AnalyzeItem.ANI_ORDER
			FROM              dbo.ELEMENT_FORM INNER JOIN
                            dbo.PRODUCT_DATA ON dbo.ELEMENT_FORM.PDD_CHEMICAL = dbo.PRODUCT_DATA.PDD_CHEMICAL INNER JOIN
                            dbo.AnalyzeItem ON dbo.ELEMENT_FORM.ELM_ID = dbo.AnalyzeItem.ANI_INDEX
			WHERE          (dbo.PRODUCT_DATA.PDD_PROD_NO = '".$_GET['pid']."') AND (dbo.AnalyzeItem.ANI_GROUPNAME = 'M13')";
	
		$result = mssql_query($query);
		$numRows = mssql_num_rows($result);
		while($row = mssql_fetch_array($result)){
		$_SESSION['ani_m_fst'].= $row['ANI_INDEX'].",";}
		echo $_SESSION['ani_m_fst'];
		$url="index.php?url=ani_m_fst&id=".$_GET['id']."&pid=".$_GET['pid'];
		jumpto($url);
}
