...
mssql_close($dbhandle);
      ...
      <td width="100" class="centerutton">&nbsp;</td>
    </tr>
    <tr>
...
$titles='<table border="1">
    ...
    <td width="60">轉檔日期</td>
	<td width="60">ContainnerNO</td>';
	echo $titles;
include('../connections/conn.php'); 
//mysql_select_db($database_connection, $connection);
$query = "SELECT distinct IN_PLAN.IPA_ETA_DATE, IN_PLAN.IPA_PO_NO, IN_PLAN.IPA_INVOICE_NO, CUSTOMER_DATA.CTD_CUST_NAME, IN_PLAN_FROM_FILE.PDD_PROD_NO, IN_PLAN_FROM_FILE.IAF_PROD_NAME, PRODUCT_DATA.PDD_STYLE, IN_PLAN_PRODUCT.IAP_UNIT, PRODUCT_DATA.PDD_PACKAGE, IN_PLAN_PRODUCT.IAP_ORDER_QTY, IN_PLAN_PRODUCT.IAP_QTY, IN_PLAN.IPA_IN_DATE,IN_PLAN.IPA_TYPE, IN_PLAN_PRODUCT.PRA_SERIAL_NO,IN_PLAN_FROM_FILE.IPA_PO_NO AS Expr1, IN_PLAN_PRODUCT.IAP_STATE, IN_PLAN_PRODUCT.IAP_LOT_NO, IN_PLAN_PRODUCT.IAP_INWARD_DATE, CUSTOMER_DATA.CTD_SUPPLIER
...
$query=$query." and (IN_PLAN.IPA_TYPE ='A')";
if ($_POST['pono']<>""){
$query="SELECT  distinct  IN_PLAN.IPA_ETA_DATE, IN_PLAN.IPA_PO_NO, IN_PLAN.IPA_INVOICE_NO, CUSTOMER_DATA.CTD_CUST_NAME, 
                            ...
                            IN_PLAN_PRODUCT.IAP_QTY, IN_PLAN.IPA_IN_DATE, IN_PLAN.IPA_TYPE, IN_PLAN_PRODUCT.PRA_SERIAL_NO,
...
//format and display results
	...
	$ts1=$ts[0]."-".$ss[0]."-".$ss[1];
    echo ("<td>$ts1</td>");
	echo '<td><a target="_self" href="index.php?url=list_po&po_no='.$t2.'&t13=A">'.$t2.'</a></td>';
//    echo ("<td>$t2</td>");
    echo ("<td>$t3</td>");
	echo ("<td>$t4</td>");
	echo ("<td>$t5</td>");
