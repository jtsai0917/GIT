...
if(isset($_POST['submit1']))
{
	...
	for($j=1;$j<=$c;$j++)
	{
		...
		if($str[$j][1]==1)
		{
				...
				if($rowre['DHN_DISCARD_DATE']=='')
				{	
				
					if($rowre['DHN_USED_COUNT']==0)
					{
						echo '<td>'."無出貨紀錄".'<td>';
					}
					
					
					if($rowre['DHN_USED_COUNT']==1 and $rowre['DHN_OUT_DATE1']<>'' and $rowre['DHN_WL_RCV_DATE1']=='' )
					{
					$up1="update DRUM_HISTORY_NORMAL
					set DHN_WL_RCV_DATE1='".$str[$j][2]."',DHN_TYS_RCV_DATE1='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
					$resultup1 = mssql_query($up1);
					echo '<td>'."第一次出貨回收,客戶:".$rowre['C1'].'</td>';
					}
					elseif($rowre['DHN_USED_COUNT']==1 and $rowre['DHN_OUT_DATE1']=='' ){echo '<td>'."無第一次出貨紀錄".'</td>';}
					elseif($rowre['DHN_USED_COUNT']==1 and $rowre['DHN_WL_RCV_DATE1']<>'' ){echo '<td>'."已有回收紀錄".'</td>';}
					
					
					if($rowre['DHN_USED_COUNT']==2 and $rowre['DHN_OUT_DATE2']<>'' and $rowre['DHN_WL_RCV_DATE2']=='')
					{
					$up1="update DRUM_HISTORY_NORMAL
					set DHN_WL_RCV_DATE2='".$str[$j][2]."',DHN_TYS_RCV_DATE2='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
					$resultup1 = mssql_query($up1);
						if($rowre['CTD_CUST_NO2']<>'')
						{
					...
					elseif($rowre['DHN_USED_COUNT']==2 and $rowre['DHN_OUT_DATE2']==''){echo '<td>'."無第二次出貨紀錄".'</td>';}
					elseif($rowre['DHN_USED_COUNT']==2 and $rowre['DHN_WL_RCV_DATE2']<>'' ){echo '<td>'."已有回收紀錄".'</td>';}
					
					
					if($rowre['DHN_USED_COUNT']==3 and $rowre['DHN_OUT_DATE3']=='') 
					{
						echo '<td>'."此桶無第三次出貨紀錄".'</td>';
					}
					if($rowre['DHN_USED_COUNT']==3 and $rowre['DHN_OUT_DATE3']<>'' and $rowre['DHN_WL_RCV_DATE3']=='')
					{
					$up1="update DRUM_HISTORY_NORMAL
					set DHN_WL_RCV_DATE3='".$str[$j][2]."',DHN_TYS_RCV_DATE3='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
					$resultup1 = mssql_query($up1);
						if($rowre['CTD_CUST_NO3']<>'')
						{
						...
						if($rowre['CTD_CUST_NO3']=='' and $rowre['CTD_CUST_NO2']=='')
						{
							echo '<td>'."第三次出貨回收,客戶:".$rowre['C1'].'</td>';
						}
						
					}
					elseif($rowre['DHN_USED_COUNT']==3 and $rowre['DHN_OUT_DATE3']==''){echo '<td>'."無第三次出貨紀錄".'</td>';}

					elseif($rowre['DHN_USED_COUNT']==3 and $rowre['DHN_WL_RCV_DATE3']<>'' ){echo '<td>'."已有回收紀錄".'</td>';}
		...
		if($str[$j][1]==3)
		{
			...
			{	
				...
				if($rowre['DHN_USED_COUNT']==0  )
				{
					...
					set DHN_DISCARD_DATE='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
					echo '<td>'."".$str[$j][0]."已登記廢棄".'</td>';
				}
				elseif($rowre['DHN_TYS_RCV_DATE1']!='' and $rowre['DHN_USED_COUNT']==1)
				{
						$up2="update DRUM_HISTORY_NORMAL 
					set DHN_DISCARD_DATE='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
				...
				elseif($rowre['DHN_USED_COUNT']==2 and $rowre['DHN_TYS_RCV_DATE2']<>'')
				{
						$up2="update DRUM_HISTORY_NORMAL 
					set DHN_DISCARD_DATE='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
				...
				elseif($rowre['DHN_USED_COUNT']==3 and $rowre['DHN_TYS_RCV_DATE3']<>'')
				{
						$up2="update DRUM_HISTORY_NORMAL 
					set DHN_DISCARD_DATE='".$str[$j][2]."'
					where DHN_DRUM_NO='".$str[$j][0]."'";
