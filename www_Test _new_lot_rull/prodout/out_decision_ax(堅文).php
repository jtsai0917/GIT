...
//			echo $query."<BR>";
			...
			if($row[0]==0){	
				...
				$query="INSERT INTO OUT_PLAN_FROM_FILE (OAF_CUST_ORDER_NO, OAF_DEP_NO , OAF_PROD_NAME, OPM_ORDER_NO,  OAF_SERIAL_NO, OAF_ETA_DATE, PDD_PROD_NO, OAF_DELI_CUST_NO, OAF_DELI_CUST_NAME, OAF_ORDER_CUST_NO, OAF_ORDER_CUST_NAME, OAF_PACKAGE_DESC) VALUES ('".$aa[$cc[$x]][5]."',2,'".get_prod_name($aa[$cc[$x]][3])."','".$aa[$cc[$x]][2]."', 1, '".gtm($aa[$cc[$x]][9])."', '".$aa[$cc[$x]][6]."', '".$aa[$cc[$x]][3]."', '".get_cust_sname($aa[$cc[$x]][3])."', '".$aa[$cc[$x]][4]."', '".get_cust_sname($aa[$cc[$x]][4])."', '".$package."')";
 				//echo $query."<BR>";
				$result=mssql_query($query);
			}
	...
	*/
			...
			$query="INSERT INTO OUT_PRODUCT
					   (COPCONFIRMSERIES, OPD_ACC_UNIT, OPM_ORDER_NO, OPD_SERIAL_NO, OTD_NO, PDD_PROD_NO, OPD_LOT_NO, OPD_QTY_DRUM, OPD_QTY_KG,OPD_QTY_LITER, OPD_COA_NO, OPD_MEMO, OPD_REAL_QTY_KG, OPD_INWARD_DATE, OAF_PACKAGE)	VALUES  
					   ('".$aa[$cc[$x]][19]."', '".strtoupper($aa[$cc[$x]][8])."', '".$aa[$cc[$x]][2]."',".$nn.",'".$aa[$cc[$x]][16]."','".$aa[$cc[$x]][18]."','".$aa[$cc[$x]][13]."',".round($OPD_QTY_DRUM).",".round($OPD_QTY_KG).",".round($OPD_QTY_LITER).",'".$aa[$cc[$x]][12]."','".$aa[$cc[$x]][10]."','".round($aa[$cc[$x]][7],0)."','".date("Ymd")."','".$package."')";
			//echo $query;
			$result=mssql_query($query);
	//		echo '<BR>';
