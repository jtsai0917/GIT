...
//			echo "<BR>";
	...
	$queryo="select distinct top 2 first from analyze_first where lot_no like '".$_GET['lot_no']."%' and first!='' ";
	//echo $queryo;
	...
	where lot_no like '".$_GET['lot_no']."%' and first = '".$rowo['first']."' and AF.ani_group='M13' order by TL.lotno";
	//echo $query;
	$n=0;
	$count=0;
	$result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			for($i=0;$i<count($a1);$i++)
			{
					...
					if($times==0)
					{
						$objPHPExcel->getActiveSheet()->setCellValue($a2[$n].$s,$row[$a1[$i]]);
					}
					if($times==1)
					{
						$objPHPExcel->getActiveSheet()->setCellValue($a3[$n].$s,$row[$a1[$i]]);
...
if($_GET['ani_groupname']=='P' )//特別列印
{
	...
	$query="select top 40 * from ".$_GET['efm']." where lotno like '".substr($_GET['lot_no'],0,-1)."%' order by lotno,Tester,serialno";
	//echo $query;
		$result = mssql_query($query);
		$eng='D';$num1='5';$i=0;$OK=0;
		while($row = mssql_fetch_array($result))
		{		
				if(($i==0 or $i==1) and $OK==0 )
				{
				$objPHPExcel->getActiveSheet()->setCellValue('B5',iconv("big5","utf-8",$row['Tester']));
				$objPHPExcel->getActiveSheet()->setCellValue('B6',iconv("big5","utf-8",$row['Tester']));

				}
				if(($i==2 or $i==3) and $OK==0 )
				{
				$objPHPExcel->getActiveSheet()->setCellValue('B8',iconv("big5","utf-8",$row['Tester']));
				$objPHPExcel->getActiveSheet()->setCellValue('B9',iconv("big5","utf-8",$row['Tester']));
				}
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['PP02']);$num1++;
				if($i==1){$num1++;}
				$i++;
				if($i==4){$i=0;$eng++;$num1='5';$OK=1;}
	...
	//echo $query;
		...
		{		
				...
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['PP05']);$num1++;
				if($i==1){$num1++;}
				$i++;
				if($i==4){$i=0;$eng++;$num1='5';$OK=1;}
...
if($_GET['ani_groupname']=='A' )
{
	...
	//echo $query;
		...
		{		
				...
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['Average']);$num1++;
				if($i==1){$num1++;}
				$i++;
				if($i==4){$i=0;$eng++;$num1='5';$OK=1;}
