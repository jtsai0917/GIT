...
function dmdy_dymd($ds){ //   2022-12-31 => 12/31/2022
	$dat=substr($ds,5,2)."/".substr($ds,8,2)."/".substr($ds,0,4);
	return $dat;
}
...
function gts($ds){  //day-style  09/02/2015=>2015/09/02
$dat=(substr($ds,6))."/".substr($ds,0,2)."/".substr($ds,3,2);
return $dat;
}

function des($ds){  //day-style  2017/11/06=>20171106
$dat=substr($ds,0,4).substr($ds,5,2).substr($ds,8,2);
return $dat;
}
...
function dod1($ds){  //day-style  106/08/30=>20170830
$dat=(substr($ds,0,3)+1911).substr($ds,4,2).substr($ds,7,2);
return $dat;
}

function dod2($ds){  //day-style  20170830=>106/08/30

$dat=(substr($ds,0,4)-1911)."/".substr($ds,4,2)."/".substr($ds,6,2);
return $dat;
}
...
function stc($ds){   //daytime-style  20150902010203=>2015 年 09 月 02 日
if($ds<>NULL){$dat=substr($ds,0,+4)."年".substr($ds,4,+2)."月".substr($ds,6,+2)."日";}
return $dat;
}
...
function sts($ds){   //daytime-style  20150902010203=>01:02:03
if($ds<>NULL){$dat=substr($ds,8,+2).":".substr($ds,10,+2).":".substr($ds,12,+2);}
return $dat;
}

function sta($ds){   //daytime-style  20150902010203=>2015/09/02 01:02:03
if(substr($ds,12,+2)==''){$ss='00';}else{$ss=substr($ds,12,+2);}

if($ds<>NULL){$dat=substr($ds,0,+4)."/".substr($ds,4,+2)."/".substr($ds,6,+2)." ".$dat=substr($ds,8,+2).":".substr($ds,10,+2).":".$ss;}
return $dat;
}
function stab($ds){   //daytime-style  20150902010203=>2015/09/02 01:02:03
$mon=intval(substr($ds,4,+2)) ;
$day=intval(substr($ds,6,+2)) ;
if($ds<>NULL){$dat=substr($ds,0,+4)."/".$mon."/".$day;}
return $dat;
}
function stabb($ds){   //daytime-style  20150902010203=>02/09/2015
$mon=intval(substr($ds,4,+2)) ;
$day=intval(substr($ds,6,+2)) ;
if($ds<>NULL){$dat=$day."/".$mon."/".substr($ds,0,+4);}
return $dat;
}
...
<head>
  ...
  function add_date_session1(nam,val){
	window.open("../backend1.php?name="+nam+"&value="+val)  
	window.location.href="'.$_SESSION['urln1'].'";
  }
  function checkbox_change(nam,val){
	window.open("../checkbox_change.php?name="+nam+"&value="+val)  
	window.location.href="'.$_SESSION['urln1'].'";
  }
</script>
</head>';
...
if($_SESSION['pid']<>NULL){$query.=" AND (PRODUCT_DATA.PDD_PROD_NO='".$_SESSION['pid']."') ";}
	if($_SESSION['search']=='1' or '2'){$query.=" AND AND_SMP_DATETIME<='".dod($_SESSION['datepicker2'])."2359' and AND_SMP_DATETIME>='".dod($_SESSION['datepicker1'])."0000' ";}
...
function testnotonick($index,$pdd_no,$str){
	$str=trim($str);
$_SESSION['INDEX']=$index=",".trim($index).',';
$str=",".$str;
...
function analyzereport($str,$lot_no,$operator,$pdd_chemical,$pdd_prod_no,$report_datetime){
...
/*
*/
if($report_datetime>='20200923111544'){
	$OOKK=ani_result_group($lot_no,$row['ANI_GROUPNAME']);
	$oku=upload_ok($lot_no,$pdd_prod_no,$row['ANI_GROUPNAME'],$pdd_chemical);
	if($OOKK==1 and $oku=='blue'){$ok="blue";$ok1="white";}
	elseif($OOKK==1 and $oku=='red'){$ok='purple';$ok1='white';}
	elseif($OOKK=='x'){$ok="red";$ok1="white";}
	elseif($OOKK=='0'){$ok="white";$ok1="red";}
}
else{
	$oku=upload_ok($lot_no,$pdd_prod_no,$row['ANI_GROUPNAME'],$pdd_chemical);
	...
	if ($oka=='green' or $oka=='red'){$_SESSION['passornot']=0;}
}
if($row['ANI_GROUPNAME']=='PMS' or $row['ANI_GROUPNAME']=='FPMS' or $row['ANI_GROUPNAME']=='TM'){$_SESSION['passornot']=1;}
if($row['ANI_GROUPNAME']=='TM'){
	echo '<a target="_self" href=index.php?url=input_report_TM&lot_no='.$lot_no."&pdd_prod_no=".$pdd_prod_no."&ani_groupname=".$row['ANI_GROUPNAME']."&pdd_chemical=".$pdd_chemical."&operator=".$operator.' '.'><span style="background-color: '.$ok1.'"><font color="'.$ok.'">'.$row['ANI_GROUPNAME'].',  </font></span> </a>';
}
else{
	echo '<a target="_self" href=index.php?url=input_report&lot_no='.$lot_no."&ani_groupname=".$row['ANI_GROUPNAME']."&pdd_chemical=".$pdd_chemical."&pdd_prod_no=".$pdd_prod_no."&operator=".$operator.' '.'><span style="background-color: '.$ok1.'"><font color="'.$ok.'">'.$row['ANI_GROUPNAME'].',  </font></span> </a>';}
//	echo iconv("big5","utf-8",$row['ANI_GROUPNAME']).', ';
}
echo '</td>';
}

function ani_result_group($lid,$group){
	$query="SELECT top(1) ani_result_group.* FROM ani_result_group WHERE (lot_no = N'".$lid."') AND (ani_group = N'".$group."') ORDER BY [index] DESC ";	
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	if(mssql_num_rows($result)==0){return "x";}
	else{return $row[3];}
}
...
$query1.=" FROM             dbo.".$table;
 if($needno<=150){  $query1.=" INNER JOIN dbo.Sample_All ON dbo.".$table.".LotNo = dbo.Sample_All.SMA_LOT 
							AND dbo.".$table.".SampleNo+'%' like dbo.Sample_All.SMA_ID+'%' ";}
...
// if($ani_groupname=='TT'){$_SESSION['tmp']=$query1;}
//echo $query1."<BR>";
$result = mssql_query($query1);
$numRows = mssql_num_rows($result);
if($numRows==0){return "red";}
...
function my_msg_close($msg,$redirect){
    echo "<SCRIPT Language=javascript>";
    echo "window.alert('".$msg."')";
    echo "</SCRIPT>";
    echo "<script language=\"javascript\">";
...
function _confirm_($msg,$fail,$true){
	echo "<script type='text/javascript'>
	if(confirm('$msg')==true){
		window.open('".$fail."','_self')
	}
...
function get_table_A($groupname,$pdd_chemical,$lotno){
include ('../connections/conn.php');
	$query="SELECT AnalyzeDesign.AND_ITEM, PRODUCT_DATA.PDD_CHEMICAL FROM AnalyzeDesign INNER JOIN PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO WHERE (AND_LOT_NO = '".$lotno."')";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	$chemical=$row[1];
	$aa=explode(",",$row[0]);
	$an=count($aa);
	for($i=0;$i<$an;$i++){
		$str.= '(ANI_INDEX='. $aa[$i].") or " ;	
	}
	$query="SELECT DISTINCT ELEMENT_FORM.ELF_FORM FROM AnalyzeItem INNER JOIN ELEMENT_FORM ON AnalyzeItem.ANI_INDEX = ELEMENT_FORM.ELM_ID    where   (ELEMENT_FORM.PDD_CHEMICAL = '".$chemical."') and (ANI_GROUPNAME = 'A') AND ( ". substr($str,0,-3).")";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
 	$table=$row[0];
return $table;
...
function get_table_($groupname,$pdd_chemical){
include ('../connections/conn.php');
	$query="SELECT DISTINCT ELEMENT_FORM.ELF_FORM
FROM              ELEMENT_FORM INNER JOIN
                            AnalyzeItem ON ELEMENT_FORM.ELM_ID = AnalyzeItem.ANI_INDEX
...
{
//	echo "Showmod:".$showmode."<BR>";
	$stdu=trim($stdu);
//	echo "TABLE:".$table."</br>";
if($item=='Na'){$_SESSION['showmod']=$showmode;}
$query="SELECT     TOP (1)    LotNo, SampleNo, Ok, Tester, Operator, AnalyzeTime,[".$item."]
FROM              dbo.".$table."
WHERE          (LotNo = '".$lot_no."') order by AnalyzeTime desc";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
	
//	if($table=='TLLQA9100301'){echo $table." : ".$query."</br>";}
...
{
//		echo "type:1  USL=".$usl."#LSL=".$lsl."#Report=".$report."<BR>";
		if($report>$lsl)
		{
			echo "type:1-1  USL=".$usl."#LSL=".$lsl."<BR>";
			$ok="合";
		}
		else
		{
			echo "type:1-2  USL=".$usl."#LSL=".$lsl."<BR>";
			$ok="否";
		}
...
{
//	echo "type:2  USL=".$usl."#LSL=".$lsl."#Report=".$report."<BR>";
	if(($report>=$lsl) and ($report<=$usl))
	{
		$ok="合";
		echo "type:2-1  USL=".$usl."#LSL=".$lsl."<BR>";
	}
	else
	{
		$ok="否";
		echo "type:2-2  USL=".$usl."#LSL=".$lsl."<BR>";
	}
}

if(($showmode=='<DL') or ($showmode=='<USL'))
{
	if(substr($spc,0,1)=='<' or substr($spc,0,1)=='>'){$spc1=substr($spc,1);};
//	echo "</BR>".$table."#item".$item."#spc".$spc1."#dl".$dl."#usl".$usl."#report".$report."#stdu".$stdu."#</BR>";
	if(($report<=$stdu and $stdu<>'') or ($report<=$usl and $stdu==''))
	{
		$ok="合";
	}
	else
...
return array(trim($report),trim($SampleNo),trim($Tester),trim($AnalyzeTime),trim($ok),trim($Ok_value));
}
...
class pdd_type
{
	...
	function get_ab(){
		$query="SELECT          PDD_CLASS, PDD_PROD_NO
FROM              PRODUCT_DATA
...
public $cid,$unit,$cid3;
public $pid,$numrows,$pdd_prod_name,$lyno,$showname;
public $cname,$csname;
public $pdd_chemical,$pdd_prod_short_name;
public $needno,$and_coa_fst;
public $prodstyle;
public $qty,$items;
public $pdd_type,$lorry_no1,$carry_type;
public $qty_drum,$note,$note2;
public $spc,$cancel,$cid2,$foduni;
public $tmp,$smptime,$outdate,$testdate;

function chk_car_type(){
	$query="SELECT BIG_DRUM_VOLUME.CARRY_TYPE FROM BIG_DRUM_VOLUME WHERE (PDD_PROD_SHORT_NAME = '".$this->lorry_no1."')";
//	echo $query."<BR>";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	$this->carry_type=$row[0];
}
...
//	echo "<BR>".$query."<BR>";
	...
	while($row=mssql_fetch_array($result)){
		...
		$this->pdd_prod_name=$row['PDD_PROD_NAME'];
		$this->unit=$row['PDD_UNIT'];
	}
}

function cid(){
	if(substr($this->lid,-1)=="_"){$this->lid=substr($this->lid,0,-1);}
	include("../connections/conn.php");
...
//echo $query."<BR>";
	$result = mssql_query($query);
	$this->numrows =$numRows= mssql_num_rows($result);
	if($numRows>0){
	while($row = mssql_fetch_array($result))
	{	
...
function cid2()
{
	...
	$query="SELECT          FILLPLAN_DRUM_CUSTOMER.CTD_CUST_NO, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, FDMC_DRUM_COUNT
			...
			WHERE          (FDM_LOT_NO = '".$this->lid."' and  FOD_UNI = '".$this->foduni."') ";	
//	echo $query;
	$result = mssql_query($query);
	while($row = mssql_fetch_array($result)){
...
{
	include("../connections/conn.php");
	$query="SELECT          '(' + CUSTOMER_DATA.CTD_CUST_NO + ')' + CUSTOMER_DATA.CTD_CUST_SHORT_NAME + '-' + CAST(OUT_PRODUCT.OPD_QTY_DRUM AS VARCHAR(10)) 
                            AS CombinedColumn
FROM              OUT_PLAN INNER JOIN
                            OUT_PRODUCT ON OUT_PLAN.OPM_ORDER_NO = OUT_PRODUCT.OPM_ORDER_NO INNER JOIN
                            CUSTOMER_DATA ON OUT_PLAN.CTD_CUST_NO = CUSTOMER_DATA.CTD_CUST_NO
WHERE          (OUT_PRODUCT.OPD_LOT_NO = '".$this->lid."') ";	
// echo $query."<BR>";
	$result = mssql_query($query);
	while($row = mssql_fetch_array($result)){
		$this->cid3.=$row['CombinedColumn'].",";
	}
	$this->cid3=substr($this->cid3,0,-1);
}
...
function checkanalizestatus($lot){
	include("../connections/conn.php");
	$query1="SELECT          AND_LOT_NO,AND_ITEM
	FROM              dbo.AnalyzeDesign
	WHERE          (AND_LOT_NO = '".$lot."')";	

	$result1 = mssql_query($query1);
	$numRows1 = mssql_num_rows($result1);
	$a=mssql_fetch_array($result1);
	$_SESSION['ANALYZE_ITEMS']=$a[1];
	return $numRows1;	
}
...
$uid=$_SESSION['MM_Username'];
	...
	$textfield4=$_SESSION['textfield4'];
	$cid=$_SESSION['cid'];
	...
	$_SESSION['textfield4']=$textfield4;
	$_SESSION['cid']=$cid;
...
function auth_PDA($aid,$emp_aut_group)
{
	$chk=0;
	if($emp_aut_group=='')
	{
...
function lock_head(){
	...
	echo '<script type="text/javascript" src="/css/jquery.min.js"></script>
...
function get_dep_name($dep_no){
	$query="select DEP_NAME from  DEPARTMENT_DATA where DEP_NO='".$dep_no."'";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	return $row[0];
}

function lotno_to_date($lotno){
	$r=substr(date("Y"),0,3);	
	$query="SELECT AND_NEED_NO FROM AnalyzeDesign WHERE (AND_LOT_NO = '".$lotno."')";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	$needno=$row[0];
	if($needno<=150){
		$ret=$r.substr($lotno,3,1)._exmonth(substr($lotno,4,1)).substr($lotno,5,2);	
	}
	elseif($needno>=270){
		$ret=$r.substr($lotno,1,1)._exmonth(substr($lotno,2,1)).substr($lotno,3,2);	
	}
	return $ret;
}

function create_drum($dm_no,$pid,$lid)
{
	$query="Insert Into DRUM_HISTORY_NORMAL 	(DHN_DRUM_NO, DHN_USED_COUNT, CTD_CUST_NO1, PDD_PROD_NO1,DHN_LOT_NO1) 	VALUES ('".$dm_no."',  0 ,'','".$pid."', '".$lid."'  )";	
	$result=mssql_query($query);
}

function select_cust($pid,$cid)
{
	if(trim($_SESSION['cid'])==''){$_SESSION['cid']=$cid;}
	echo '<select name="cid" id="cid" onchange="set_date_session(this.name,this.value)">';
	echo '<option value="'.$cid.'" >'.$cid."-".get_cust_sname($cid).'</option>';
	$query="SELECT  CUSTOMER_PRODUCTS.CTD_CUST_NO AS CN, CUSTOMER_DATA.CTD_CUST_SHORT_NAME as CCN
FROM      CUSTOMER_PRODUCTS INNER JOIN
                   CUSTOMER_DATA ON CUSTOMER_PRODUCTS.CTD_CUST_NO = CUSTOMER_DATA.CTD_CUST_NO
WHERE   (CUSTOMER_PRODUCTS.PDD_PROD_NO = '".$pid."')";
	$result=mssql_query($query);
	while($row=mssql_fetch_array($result))
	{
		if($row['CN']==$_SESSION['cid']){$str='selected';}else{$str='';}
  		echo '<option value="'.$row['CN'].'" '.$str.' >'.$row['CN']."-".$row['CCN'].'</option>';
	}
	echo '</select>';
}
function fill_drum($pdd)
{
	$query1="select PDD_TYPE from PRODUCT_DATA where PDD_PROD_NO LIKE '%".$pdd."%' and (PDD_TYPE='DM' or PDD_TYPE='BTL' or PDD_TYPE='TOTO')";
	$result1=mssql_query($query1);
	$numRows1=mssql_num_rows($result1);
	return $numRows1;
}
...
function keep($pid,$cid,$type){
	$query="SELECT  CTP_BEF_SMP_PE as m1, CTP_BEF_SMP_TFA as p1 FROM CUSTOMER_PRODUCTS WHERE (CTD_CUST_NO = '".$cid."') AND (PDD_PROD_NO = '".$pid."')";	
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	if($type=='M'){
		return $row[0];
...
function rt_ani_group($list){
	$aa=explode(",",$list);
	$n=count($aa);
	$query="SELECT DISTINCT ANI_GROUPNAME FROM AnalyzeItem where ";
	for($i=0;$i<$n;$i++){
		$query.="ANI_INDEX ='".$aa[$i]."' or ";
	}
...
function insert_into_mapping_table($table_name){
	$query="SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME = '".$table_name."') AND (COLUMN_NAME <> 'index') AND (DATA_TYPE <> 'timestamp')";
	$result=mssql_query($query);
	while($row=mssql_fetch_array($result)){
		$str1=$str1.$row['COLUMN_NAME'].",";
		$str2=$str2."'".$_POST[$row['COLUMN_NAME']]."',";
	}
...
function signatory_empname($lv,$lotno){
	$query="SELECT Signatory_Name FROM Signatory WHERE (LV = ".$lv.") AND (cancel <> 1) and LotNo='".$lotno."'";
//	echo $query."<BR>";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	return $row[0];
