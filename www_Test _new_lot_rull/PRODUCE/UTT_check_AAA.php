...
<form name="form1" method="post" enctype="multipart/form-data" action="<?php echo $loginFormAction; ?>">
  <label for="textfield"></label>
        ...
        <input name="pid" type="text" id="pid" size="10" onchange="set_date_session(this.name,this.value)" value="
<?php 
		if ($_GET['pid']){
			echo $_GET['pid'];
...
if($_GET['remark']<>'' and ($_SESSION['pass']==1 or $_SESSION['pass']==2) or isset($_POST['cancel1']) or isset($_POST['cancel2']))  //??πÊ¨°Á∞ΩÊ†∏??áÂ??Ê∂àÁ????®Â??
{
	$remark=$_GET['remark'];
	...
	my_msg('Á∞ΩÊ†∏ÂÆåÊ??!',$url);
}
	if($_SESSION['sign']<>'')////??ÆÊ¨°Á∞ΩÊ†∏??ÑÈÉ®???
	{
...
if(isset($_POST['search']) or isset($_POST['morning']) or isset($_POST['afternoon']) or isset($_POST['night']) or $_SESSION['pass']<>'')
{
	...
	if($_SESSION['pid']<>'')
	{
		...
		echo '<tr bgcolor="#CCCCCC"><td>TAG_NO</td><td>??ÖÁõÆ??çÁ®±</td><td>??∏ÂÄ?</td><td>SPEC</td><td>Â∑°Ê™¢??•Ê????ÇÈ??</td><td>Â∑°Ê™¢‰∫∫Âì°</td><td>??àÊ†º/‰∏çÂ?????</td></tr>';
...
//		echo $query."<BR>";
		...
		{
		$pass=trim($row1['pass']);
					if($pass=='O'){$pass='??àÊ†º';$BG='';}
					elseif($pass=='???'){$pass='';$BG='';}
					else{$pass='‰∏çÂ?????';$BG='bgcolor="#FFFF66"';}
...
//		echo $query."<BR>";
		...
		if($Numrows==0){my_msg('??•ÁÑ°Ë≥áÊ??');}
		$sign1=hr_duteies($_SESSION['uid']);   ////??πÊ¨°Á∞ΩÊ†∏??®Â??
		/////////??πÊ¨°??ñÊ????®Â??
		...
		if(substr($_SESSION['uid'],1)=='319' or substr($_SESSION['uid'],1)=='029' or substr($_SESSION['uid'],1)=='525'  or substr($_SESSION['uid'],1)=='903' or substr($_SESSION['uid'],1)=='066' or substr($_SESSION['uid'],1)=='1048' or substr($_SESSION['uid'],1)=='1263' or substr($_SESSION['uid'],1)=='900' or substr($_SESSION['uid'],1)=='1038' or substr($_SESSION['uid'],1)=='1044' or substr($_SESSION['uid'],1)=='1103' or substr($_SESSION['uid'],1)=='1100' or substr($_SESSION['uid'],1)=='1065')
		{
			if($signcount==$Numrow4){
				...
				elseif($signcount1==0){$sign1[0]='<input type="submit" name="cancel1" value="??πÊ¨°??ñÊ??" style="background-color:red;">';}
				else{
				$sign1[0]='<input type="submit" name="cancel1" value="??πÊ¨°??ñÊ??" style="background-color:red;">';}
			}
			if($signcount1==$Numrow4){$sign1[1]='<input type="submit" name="cancel2" value="??πÊ¨°??ñÊ??" style="background-color:red;">';$sign1[0]='';}
		}
		///////////////
		
		$morning='<input type="submit" name="morning" value="???">';
		$afternoon='<input type="submit" name="afternoon" value="‰∏?">';
		$night='<input type="submit" name="night" value="???">';
		...
		*/
		echo '<tr  bgcolor="#CCCCCC"><td colspan="4"  align="center" valign="center"><strong>Á∞ΩÊ†∏‰ΩúÊ•≠</strong></td><td align="center">'.$morning.' '.$afternoon.' '.$night.'</td><td colspan="2"><td align="center">'.$sign1[0].'</td><td align="center">'.$sign1[1].'</td></tr>';
		$query2="SELECT distinct Date from UTT_CHECK_DATA UD left join UTT_TAGNO_DATA UN ON UD.TAG_NO=UN.TAG_NO where ".$Datetime." and ".$form."=".$_SESSION['form']." order by date";
//		echo $query2."<BR>";
		$result2=mssql_query($query2);
...
//		$result=mssql_query($query);
		...
		if($_SESSION['director']==1 or $_SESSION['director']==2 or $director<>'')
		{
			...
			{
				$SIGN1='<input type="submit" value="Á∞ΩÊ†∏" name="sigA'.$sign1.'_'.trim($row2[0]).'" style="background-color:green;">';
				$SIGN2='';
			...
			if(trim($row1['SIGN1'])<>'' and trim($row1['SIGN2'])=='')
			{
				...
				$SIGN1='<input type="submit" value="??ñÊ??Á∞ΩÊ†∏" name="canA'.$sign1.'_'.trim($row2[0]).'" style="background-color:red;">';
				$_SESSION['SIGN'][$count]='canA'.$sign1.'_'.trim($row2[0]);
				$count++;
				}
				if($_SESSION['director']==1 or $director==1){
				$SIGN2='<input type="submit" value="Á∞ΩÊ†∏" name="sigB'.$sign1.'_'.trim($row2[0]).'" style="background-color:green;">';
			...
			if(trim($row1['SIGN1'])<>'' and trim($row1['SIGN2'])<>'')
			{
				...
				$SIGN2='<input type="submit"  value="??ñÊ??Á∞ΩÊ†∏" name="canB'.$sign1.'_'.trim($row2[0]).'" style="background-color:red;">';
		...
		echo '<tr bgcolor="#CCCCCC"><td>TAG_NO</td><td>??ÖÁõÆ??çÁ®±</td><td>??∏ÂÄ?</td><td>SPEC</td><td>Â∑°Ê™¢??•Ê????ÇÈ??</td><td>Â∑°Ê™¢‰∫∫Âì°</td><td>??àÊ†º/‰∏çÂ?????</td><td>‰∏ª‰ªªÁ∞ΩÊ†∏:'.$SIGN1.'</td><td>‰∏ªÁÆ°Á∞ΩÊ†∏:'.$SIGN2.'</td></tr>';
		$query="select FD.Form,FD.filename,FD.link,FD.sn,FD.sn1,FD.way,TD.TAG_NO,TD.project,TD.spec,rs.Data,rs.pass,rs.Date,rs.saveuid from UTT_FORM_DATA FD LEFT JOIN UTT_TAGNO_DATA TD ON   FD.[index]=TD.".$form." left join UTT_record_spec rs on  TD.TAG_NO=rs.TAG_NO  where FD.[index]='".$_SESSION['form']."' AND Date ='".trim($row2[0])."'";
//		echo $query."<BR>";
		$result=mssql_query($query);
		while($row1=mssql_fetch_array($result))
...
if(isset($_POST['print']))
{
	if($_SESSION['pid']<>'')  //??áÂ??Ê™¢È????ÖÁõÆ
	{
		...
		$objPHPExcel->getActiveSheet()->getStyle('B'.$Num)->getAlignment()->setWrapText(true);//??™Â????õË??
	...
	else   //‰∏çÊ??ÂÆöÊ™¢È©óÈ?????
	{
		...
		//	echo $query2."<BR>";
			...
			//PHPExcel_Cell::columnIndexFromString  Â∞áËã±??áË????∫Êï∏Â≠?
			//PHPExcel_Cell::stringFromColumnIndex  Â∞áÊï∏Â≠óË????õÁÇ∫??±Ê??
			
			$col_start=PHPExcel_Cell::columnIndexFromString(substr($sn,0,1));  // EXCEL??ãÂ??ËÆÄ??ñÊ?? ‰æãÂ?? 1
			$col_write_start=$col_start+3;    //??±Ë©≤??óÈ??ÂßãÂØ´??? ??∏Â??
		...
		//	$reader = PHPExcel_IOFactory::createReader('Excel2007'); 
			...
			// ËÆÄ??ñÊ??Ê°àÁ??TAG_NO, ??? $sn ??ãÂ??ÂØ´Âà∞array tagno  
		...
		//		ÂØ´ÂÖ•Á∞ΩÊ†∏title
			echo "COL:".($col_write_start-1)."ROW:".($highestRow+1)."<BR>";
	...
	}
{
	...
	unset($_SESSION['time1'],$_SESSION['time2'],$objPHPExcel);
}  //end print
