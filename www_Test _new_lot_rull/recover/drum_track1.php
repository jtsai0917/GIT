...
//          include("../connections/conn.php");
  ...
  <tr class="GridviewScrollHeader">';
  		echo '<td>count</td><td>LOT NO</td><td>D/M NO</td><td>客戶編號</td><td>用戶</td><td>料號</td><td>
		 品名</td><td>'."LOTNO".'</td><td>'."出荷日".'</td><td>'."出荷檢查日".'</td>';
			...
			$query="SELECT DISTINCT 
                            OUT_DECISION.OPM_ORDER_NO, OUT_DECISION.CTD_CUST_NO, CUSTOMER_DATA.CTD_CUST_NAME, 
                            CUSTOMER_DATA.CTD_CUST_SHORT_NAME, OUT_DECISION.OPM_ETA_DATE, OUT_PRODUCT.PDD_PROD_NO, 
                            PRODUCT_DATA.PDD_PROD_NAME, OUT_PRODUCT.OPD_LOT_NO, OUT_CHECK_DRUM.OTD_NO AS Expr1, 
                            OUT_CHECK_DRUM_DETAIL.OCD_DRUM_NO, OUT_CHECK_DRUM.OCM_CHK_DATE, DRUM_HISTORY_NORMAL.DHN_TYS_RCV_DATE1 as rcv_date
FROM              DRUM_HISTORY_NORMAL INNER JOIN
                            OUT_CHECK_DRUM_DETAIL ON 
                            DRUM_HISTORY_NORMAL.DHN_DRUM_NO = OUT_CHECK_DRUM_DETAIL.OCD_DRUM_NO RIGHT OUTER JOIN
                            OUT_CHECK_DRUM ON OUT_CHECK_DRUM_DETAIL.OTD_NO = OUT_CHECK_DRUM.OTD_NO RIGHT OUTER JOIN
                            CUSTOMER_DATA INNER JOIN
                            OUT_DECISION ON OUT_DECISION.CTD_CUST_NO = CUSTOMER_DATA.CTD_CUST_NO INNER JOIN
                            ...
                            OUT_CHECK_DRUM.OTD_NO = OUT_PRODUCT.OTD_NO AND 
                            OUT_CHECK_DRUM.OTD_NO = OUT_DECISION.OTD_NO ";
			if($_POST['cust']=='' and $_POST['pdd_chemical1']==''){
				$query.=" WHERE   
                (OUT_CHECK_DRUM.OCM_CHK_DATE >= '".$date1."' AND OUT_CHECK_DRUM.OCM_CHK_DATE <= '".$date2."') ";
			}
			if($_POST['cust']<>'' or $_POST['pdd_chemical1']<>''){   
				$query.=" WHERE   
				(DHN_TYS_RCV_DATE3 >= '".$date1."' AND DHN_TYS_RCV_DATE3 <= '".$date2."' and OUT_PRODUCT.PDD_PROD_NO  like '".$_POST['pdd_chemical1']."%' and OUT_DECISION.CTD_CUST_NO like '".$_POST['cust']."%' )";
			}
			$query.=" ORDER BY   OUT_DECISION.OPM_ETA_DATE,OUT_DECISION.OPM_ORDER_NO,OUT_PRODUCT.OPD_LOT_NO ";
//			echo $query."<BR>";
...
//			echo "SS";
			...
			{		
				if(substr(trim($row['OPD_LOT_NO']),-4,1)=='D' or substr(trim($row['OPD_LOT_NO']),-4,1)=='B' ){$lotno=substr(trim($row['OPD_LOT_NO']),0,-4);}else{$lotno=trim($row['OPD_LOT_NO']);}
				if($row['rcv_date']-substr(trim($row['OPM_ETA_DATE']),0,8)>0){$rcv='Y';}else{$rcv='N';}
					echo '<tr class="GridviewScrollItem">';
					echo  '<td>'.$rr.'</td>';
					echo  '<td>'.$row['OPD_LOT_NO'].'</td>';
			  		...
			  		echo  '<td>'.std($row['rcv_date']).'</td>';      
			   		echo  '<td>'.$rcv.'</td>';      
			  		echo  '<td>'.std($dm->aa[$i][6]).'</td>';      
			    	echo  '<td>'.std($dm->aa[$i][7]).'</td>';
					echo '</tr>';					
				$rr++;	
		}//ifpostsubmit
		 echo '</table>'; 		
...
//          include("../connections/conn.php");
  ...
  <tr class="GridviewScrollHeader">';
		 ...
		 品名</td><td>'."LOTNO".'</td><td>'."出荷日".'</td><td>'."出荷檢查日".'</td>';
			...
			$date2=dod($_SESSION['datepicker2']);			
			$query="SELECT DISTINCT 
                            [index], DHN_DRUM_NO, DHN_USED_COUNT, CTD_CUST_NO1, PDD_PROD_NO1, DHN_WASHED_DATE1, 
                            DHN_FILLED_DATE1, DHN_LOT_NO1, DHN_OUT_DATE1, DHN_VALIDATE1, DHN_TYS_RCV_DATE1, 
                            DHN_RCV_DECIDE1, DHN_DISCARD_W_DATE, DHN_DISCARD_W_MAN, DHN_DISCARD_DATE, 
                            DHN_DISCARD_R_MAN, DISABLE, DHN_CAP_W_DATE, DHN_CAP_W_MAN, DHN_TEAR_DATE, 
                            DHN_TEAR_MAN
FROM              DRUM_HISTORY_NORMAL ";
			if($_POST['cust']=='' and $_POST['pdd_chemical1']==''){
				$query.=" WHERE   
				(DHN_TYS_RCV_DATE1 >= '".$date1."' AND DHN_TYS_RCV_DATE1 <= '".$date2."') OR
                (DHN_TYS_RCV_DATE2 >= '".$date1."' AND DHN_TYS_RCV_DATE2 <= '".$date2."') OR
                (DHN_TYS_RCV_DATE3 >= '".$date1."' AND DHN_TYS_RCV_DATE3 <= '".$date2."') ";
			}
			if($_POST['cust']<>'' or $_POST['pdd_chemical1']<>''){   
				$query.=" WHERE   
				(DHN_TYS_RCV_DATE1 >= '".$date1."' AND DHN_TYS_RCV_DATE1 <= '".$date2."' and PDD_PROD_NO1 like '".$_POST['pdd_chemical1']."%' and CTD_CUST_NO1 like '".$_POST['cust']."%' ) OR
				(DHN_TYS_RCV_DATE2 >= '".$date1."' AND DHN_TYS_RCV_DATE2 <= '".$date2."' and PDD_PROD_NO2 like '".$_POST['pdd_chemical1']."%' and CTD_CUST_NO1 like '".$_POST['cust']."%' ) OR
				(DHN_TYS_RCV_DATE3 >= '".$date1."' AND DHN_TYS_RCV_DATE3 <= '".$date2."' and PDD_PROD_NO3 like '".$_POST['pdd_chemical1']."%' and CTD_CUST_NO1 like '".$_POST['cust']."%' )";
			}
...
//			echo "SS";
			while($row=mssql_fetch_array($result))
			{						
				if(trim($row['DHN_LOT_NO1']<>'')){
					$lot_no=$row['DHN_LOT_NO1'];
					$cust_no=$row['CTD_CUST_NO1'];
					$prod_no=$row['PDD_PROD_NO1'];
					$wash_Date=$row['DHN_WASHED_DATE1'];
					$fill_date=$row['DHN_FILLED_DATE1'];
					$tys_rcv_date=$row['DHN_TYS_RCV_DATE1'];
					$outdate=substr($row['DHN_OUT_DATE1'],0,8);	
					if($tys_rcv_date-$outdate>>0){$ret='Y';}
					else{$ret='N';}
				}

				if($row['DHN_DISCARD_DATE']<>''){$discard_date=$row['DHN_DISCARD_DATE'];}
...
if(isset($_POST['submit3']))		
		{	
				...
				$objPHPExcel->getActiveSheet()->setCellValue("C".$p, iconv("big5","utf-8","桶號"));
				$objPHPExcel->getActiveSheet()->setCellValue("D".$p, iconv("big5","utf-8","客戶編號"));
				$objPHPExcel->getActiveSheet()->setCellValue("E".$p, iconv("big5","utf-8","客戶名稱"));
				$objPHPExcel->getActiveSheet()->setCellValue("F".$p, iconv("big5","utf-8","料號"));
				$objPHPExcel->getActiveSheet()->setCellValue("G".$p, iconv("big5","utf-8","品名"));
				$objPHPExcel->getActiveSheet()->setCellValue("H".$p, iconv("big5","utf-8","出荷日"));
				$objPHPExcel->getActiveSheet()->setCellValue("I".$p, iconv("big5","utf-8","出荷檢查日"));
			  $objPHPExcel->getActiveSheet()->setCellValue("J".$p, iconv("big5","utf-8","TYS回收日"));
				$objPHPExcel->getActiveSheet()->setCellValue("K".$p, iconv("big5","utf-8","已回收"));
				$objPHPExcel->getActiveSheet()->setCellValue("L".$p, iconv("big5","utf-8","D/M廢棄出廠日"));
				$objPHPExcel->getActiveSheet()->setCellValue("M".$p, iconv("big5","utf-8","D/M廢棄再確認日"));
				$p++;
...
//			echo $query."<BR>";
			...
			{			
				if(substr(trim($row['OPD_LOT_NO']),-4,1)=='D' or substr(trim($row['OPD_LOT_NO']),-4,1)=='B' ){$lotno=substr(trim($row['OPD_LOT_NO']),0,-4);}else{$lotno=trim($row['OPD_LOT_NO']);}
					if($row1['rcv_date']-substr(trim($row1['OPM_ETA_DATE']),0,8)>0){$rcv='Y';}else{$rcv='N';}
			//	echo $row['prodno']."<BR>";
				$objPHPExcel->getActiveSheet()->setCellValue("A".$p,$rr);
				$objPHPExcel->getActiveSheet()->setCellValue("B".$p,$row1['OPD_LOT_NO']);		
				$objPHPExcel->getActiveSheet()->setCellValue("C".$p,$row1['OCD_DRUM_NO']);		
				$objPHPExcel->getActiveSheet()->setCellValue("D".$p, iconv("big5","utf-8",$row1['CTD_CUST_NO']));
				$objPHPExcel->getActiveSheet()->setCellValue("E".$p,iconv("big5","utf-8",$row1['CTD_CUST_SHORT_NAME']));
				$objPHPExcel->getActiveSheet()->setCellValue("F".$p,$row1['PDD_PROD_NO']);
				$objPHPExcel->getActiveSheet()->setCellValue("G".$p,iconv("big5","utf-8",$row1['PDD_PROD_NAME']));
				$objPHPExcel->getActiveSheet()->setCellValue("H".$p,std($row1['OPM_ETA_DATE']));
				$objPHPExcel->getActiveSheet()->setCellValue("I".$p,std($row1['OCM_CHK_DATE']));
				$objPHPExcel->getActiveSheet()->setCellValue("J".$p,std($row1['rcv_date']));
				$objPHPExcel->getActiveSheet()->setCellValue("K".$p,$rcv);
				$objPHPExcel->getActiveSheet()->setCellValue("L".$p,std($dm->aa[$i][6]));
				$objPHPExcel->getActiveSheet()->setCellValue("M".$p,std($dm->aa[$i][7]));
				$p++;
				$rr++;
...
//			echo "SS";
			...
			{						
				...
				$objPHPExcel->getActiveSheet()->setCellValue("D".$p, iconv("big5","utf-8",$cust_no));
				$objPHPExcel->getActiveSheet()->setCellValue("E".$p,iconv("big5","utf-8",get_cust_name($cust_no)));
				$objPHPExcel->getActiveSheet()->setCellValue("F".$p,$prod_no);
				$objPHPExcel->getActiveSheet()->setCellValue("G".$p,iconv("big5","utf-8",get_prod_name($prod_no)));
				$objPHPExcel->getActiveSheet()->setCellValue("H".$p,std($outdate));
				$objPHPExcel->getActiveSheet()->setCellValue("I".$p,std(substr($dm->aa[$i][4],0,8)));
				$objPHPExcel->getActiveSheet()->setCellValue("J".$p,std($tys_rcv_date));
				$objPHPExcel->getActiveSheet()->setCellValue("K".$p,$ret);
				$objPHPExcel->getActiveSheet()->setCellValue("L".$p,std($discard_date));
