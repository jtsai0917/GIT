...
<form name="form1" method="post" action="<?php echo $loginFormAction; ?>">
  ...
  <table width="1240" border="1">
    <tr>
      <td width="480"><span class="re">LOT充填日期</span>
...
FROM              dbo.AnalyzeDesign INNER JOIN
                            ...
                            dbo.EMPLOYEE_DATA ON AnalyzeDesign.AND_PERSON = EMPLOYEE_DATA.EMP_NO INNER JOIN
...
if ($_POST['datepicker1']){
	$_SESSION['datepicker1']=$_POST['datepicker1'];
    $query=$query." AND (dbo.AnalyzeDesign.AND_SMP_DATETIME >='".dod($_POST['datepicker1'])."0000')";
}
elseif($_SESSION['datepicker1'])
{
	$query=$query." AND (dbo.AnalyzeDesign.AND_SMP_DATETIME >='".dod($_SESSION['datepicker1'])."0000')";
}

if ($_POST['datepicker2']){
	$_SESSION['datepicker2']=$_POST['datepicker2'];
	$query=$query." AND (dbo.AnalyzeDesign.AND_SMP_DATETIME <='".dod($_POST['datepicker2'])."2359')";
}
elseif($_SESSION['datepicker2'])
{
	$query=$query." AND (dbo.AnalyzeDesign.AND_SMP_DATETIME <='".dod($_SESSION['datepicker2'])."2359')";
}
...
$query.=" ORDER BY dbo.FILLPLAN_OUT_DECIDE.FOD_YEAR_MONTH,dbo.FILLPLAN_OUT_DECIDE.FOD_DAY ";
...
FROM              AnalyzeDesign INNER JOIN
                            PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO INNER JOIN
                            EMPLOYEE_DATA ON AnalyzeDesign.AND_PERSON = EMPLOYEE_DATA.EMP_NO
WHERE          (AnalyzeDesign.AND_LOT_NO LIKE '%".trim($_SESSION['lid'])."%') ORDER BY AND_APPLY_DATE DESC";
}
