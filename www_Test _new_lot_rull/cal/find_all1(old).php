...
FROM              ".$table1." AS TL INNER JOIN
			...
			*/
				$query1="SELECT DISTINCT 
                            TL.*, Sample_All.SMA_DRUMNO AS Expr1, Sample_All.SMA_DRUMNO, 
                            AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, AF.first 
FROM              ".$table1." AS TL INNER JOIN
                            Sample_All ON TL.LotNo = Sample_All.SMA_LOT INNER JOIN
                            AnalyzeDesign ON TL.LotNo = AnalyzeDesign.AND_LOT_NO LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time LEFT OUTER JOIN
                            CUSTOMER_DATA ON AnalyzeDesign.CTD_CUST_NO = CUSTOMER_DATA.CTD_CUST_NO  
	...
	//			$_SESSION['OOOOOOOOOOOOOOOOOOOOOO']= $query1;
			if($_POST['lot']<>''){
			$query1="SELECT DISTINCT 
                            *, TL.AnalyzeTime, Sample_All.SMA_DRUMNO AS Expr1, Sample_All.SMA_DRUMNO, 
                            AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, AF.first 
FROM              ".$table1." AS TL INNER JOIN
                            Sample_All ON TL.LotNo = Sample_All.SMA_LOT INNER JOIN
                            AnalyzeDesign ON TL.LotNo = AnalyzeDesign.AND_LOT_NO LEFT OUTER JOIN
...
//echo $query1;
			...
			while ($row1 = mssql_fetch_array($result1))
			{
				...
				for($i=0;$i<$numrows;$i++)
				{
					$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM,round($row1[trim($item[$i])],3));$ENG++;
				}
