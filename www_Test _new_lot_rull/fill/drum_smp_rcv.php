...
<form id="form1" name="form1" method="post" action="<?php echo $loginFormAction; ?>"><font size="+2">
  Lot NO : 
  <input name="lot_no" readonly="readonly" type="text" id="lot_no"  autocomplete="off" style="font-size:16px" size="12" <?php echo $auto_lotno;?> value="<?php
  	echo $_GET['lot'];	
  ?>"/>
<?php
...
// echo $query;
	...
	echo '<BR>';
	$nton=trim($_GET['sn'])."~".trim($_GET['sn']);
	echo '取樣位置：<input name="sn" style="font-size:20px" id="selected_group" value="'.$nton.'"/>';
	echo '<BR>';
?>樣品瓶號：
  <input name="sma_id" type="text"  autocomplete="off" id="sma_id" <?php echo $auto_sma_id;?> style="font-size:20px" size="6" />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<BR />
<input type="submit" name="submit" id="submit" value=" 確 定 " style="width:100px;height:30px;border:2px orange double;" />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
</font>
  <font size="+2">
  <input type="submit" name="submit2" id="submit2" value=" 離 開 " style="width:100px;height:30px;border:2px orange double;" />
  </font>
</form>
</body>
</html>
		...
		WHERE          (SMP_ID = '".$_POST['sma_id']."')"; 
			...
			if($numRows>0)
			{
				...
				$query="INSERT INTO dbo.Sample_All
											...
											VALUES          ('".$_POST['sma_id']."',".$smp_times.",3,'".$_POST['lot_no']."',
