...
if($row['ANI_GROUPNAME']=='TM'){
	...
	echo '<a target="_self" href=index.php?url=input_report&lot_no='.$lot_no."&pdd_prod_no=".$pdd_prod_no."&ani_groupname=".$row['ANI_GROUPNAME']."&pdd_chemical=".$pdd_chemical."&operator=".$operator.' '.'><span style="background-color: '.$ok1.'"><font color="'.$ok.'">'.$row['ANI_GROUPNAME'].',  </font></span> </a>';}
//	echo iconv("big5","utf-8",$row['ANI_GROUPNAME']).', ';
}
echo '</td>';
}
...
if($table=='TLAQA9110900'){$_SESSION['tmp']=$query1;}
$result = mssql_query($query1);
$numRows = mssql_num_rows($result);
if($numRows==0){return "red";}
else {
