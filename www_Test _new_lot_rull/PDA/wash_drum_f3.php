...
// 是否已經廢棄
$overthreeyears=date("YmdHis");
 // my_msg($overthreeyears);
$od = $overthreeyears-30000000000;

$query="SELECT COUNT(EL_WASH_DRUM.EWD_DRUM) AS cnt FROM FILL_INDICATE RIGHT OUTER JOIN EL_WASH_DRUM ON FILL_INDICATE.FDM_LOT_NO = EL_WASH_DRUM.FDM_LOT_NO
 WHERE          (EL_WASH_DRUM.EWD_DRUM = '".$_POST['dm_no']."') AND (FILL_INDICATE.FID_FILL_BEGIN_DATE > '".$od."') and EL_WASH_DRUM.DISABLE <> 1"; 
// echo $query."<BR>";
// break;
$result=mssql_query($query);
$row=mssql_fetch_row($result);
if($_SESSION['pidx']=='P001-200' or $_SESSION['pidx']=='P002-190'){
	$used_times=3;
}
else{
	$used_times=1;
...
$used_cnt=$row[0];
if($used_cnt > $used_times){
	my_msg($_POST['dm_no']."  已經使用超過 ".$used_times." 次，請勿再次使用 ： ","wash_drum_f2.php");
}
...
$query="SELECT count(FDM_LOT_NO) as cnt FROM EL_WASH_DRUM WHERE (FDM_LOT_NO = '".$_POST['lid']."' and EWD_DRUM= '".$_POST['dm_no']."') and DISABLE <> 1"; 
// echo $query."<BR>";
$result=mssql_query($query);
$row=mssql_fetch_row($result);
$used_cnt=$row[0];
// echo $used_cnt.":".$used_times."<BR>";
if($used_cnt > 0){
	my_msg($_POST['dm_no']."  已有紀錄 ： ","wash_drum_f2.php");
}
...
$sa->lid=$_POST['lid'];
$sa->ani();
echo $pdd_prod_short_name=trim($sa->pdd_prod_short_name);

if(substr($_POST['dm_no'],0,2)<>$pdd_prod_short_name)
{
	echo substr($pdd_prod_short_name,0,1);
	my_msg(" 桶號錯誤 ： ".$_POST['dm_no'],"wash_drum_f2.php"); 
}

if($_SESSION['uid']==''){jumpto("login.php");} 
if(substr(trim($_POST['dm_no']),0,1)<>substr(trim($_SESSION['lid']),0,1)){my_msg("桶號不符合規則!!","wash_drum_f2.php");} 
?><head>
