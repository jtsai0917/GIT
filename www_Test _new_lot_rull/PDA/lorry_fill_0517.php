...
?>
    ...
    <input type="text"  autocomplete="off" name="lid" id="lid"  style="font-size:<?php echo $_SESSION['font_size'];?>px" size="12" <?php echo ' value="'.$_SESSION['lid'].'"'.$fo1; ?>  onchange="set_date_session(this.name,this.value)"/><BR /><BR />
     Lorry NO (槽車)：
    <input type="password" autocomplete="off" name="lorry_no1" id="lorry_no1"  style="font-size:<?php echo $_SESSION['font_size'];?>px" size="12" <?php echo ' value="'.$_SESSION['lorry_no1'].'"'.$fo2;; ?>  onchange="set_date_session(this.name,this.value)"/><?php  echo $lorry_no1=base64_decode(trim($_SESSION['lorry_no1'])) ;$lorry_no1=substr($lorry_no1,0,6);?><BR /><BR />
    PASSBOX NO (充填口)：
    <input type="password"  autocomplete="on" name="fid" id="fid"  style="font-size:<?php echo $_SESSION['font_size'];?>px" size="12" <?php echo ' value="'.$_SESSION['fid'].'"'.$fo3; ?>  onchange="set_date_session(this.name,this.value)"/><?php  echo $fid=base64_decode($_SESSION['fid']) ;$fid=trim($fid);?>
...
//	echo "FID:".$_SESSION['fid']."ss<BR>";
//	echo "lorry_no1:".$_SESSION['lorry_no1']."ss<BR>";
	// 檢查有無充填指示
	$query="SELECT  COUNT(*) AS Numbers FROM FILL_INDICATE WHERE (FDM_LOT_NO = '".trim($_POST['lid'])."')";
//	echo $query;
...
//	echo "<BR>#".$row[0]."#<BR>";
	...
	$query="SELECT  FILL_INDICATE.FID_FILL_STATUS, FILL_INDICATE.FID_FILL_BEGIN_DATE, FILLPLAN_OUT_DECIDE.FDM_LY_NO
FROM      FILL_INDICATE INNER JOIN
                   FILLPLAN_OUT_DECIDE ON FILL_INDICATE.FDM_LOT_NO = FILLPLAN_OUT_DECIDE.FDM_LOT_NO 
			WHERE   (FILL_INDICATE.FDM_LOT_NO ='".$_POST['lid']."') ";
//			echo $query."<BR>";
	$result=mssql_query($query);
	$row1=mssql_fetch_row($result);
	$_SESSION['lorry_no']=$row1[2];
	if($row1[0]==1){$_SESSION['str']=substr($row1[1],8,2).":".substr($row1[1],10,2)."開始充填，充填進行中.......<BR>";
	...
	$query="SELECT  FILL_INDICATE.FID_FILL_BEGIN_DATE, FILL_INDICATE.FID_FILL_END_DATE, LORRY_FILL_CHECK.FDM_LOT_NO FROM LORRY_FILL_CHECK INNER JOIN
		...
		WHERE   (LORRY_FILL_CHECK.FDM_LOT_NO = '".$_POST['lid']."')";
//	echo $query;
	$result=mssql_query($query);
...
if($lorry_no<>trim($lorry_no1))
	...
	$query="SELECT lorry_fill_tmp.* FROM lorry_fill_tmp WHERE (lot_no = N'".$_POST['lid']."') ";
//	echo $query;
	$result=mssql_query($query);
...
WHERE          (FDM_LOT_NO = '".$_POST['lid']."')";
	...
	$numRows = mssql_num_rows($result);
	if($numRows==0){
	$query1="UPDATE  FILL_INDICATE
SET           FID_FILL_BEGIN_DATE = '".date("YmdHis")."'
WHERE   (FDM_LOT_NO = '".$_POST['lid']."')";
	...
	echo '充填前殘液量 ： '.'<input type="text" name="remnant" id="remnant"   autocomplete="off" style="font-size:'.($_SESSION['font_size']).'px" size="8" autofocus="autofocus" onKeyPress="return SubmitEnter(this,event)" />';
	echo '<BR><input type="submit" name="submit1" id="submit" value="  下 一 步  " style="width:100px;height:20px;border:2px orange double;"/></br></font></form>';
	}
...
//		if($trim($_POST['bm3'])==''){echo "OOP";$_POST['bm3']=bm3($_POST['lid'],$m3);}
//		if($trim($_POST['em3'])==''){$_POST['em3']=bm3($_POST['lid'],$etq);}
	if($_POST['bm3']==''){$_POST['bm3']=0;}
	if($_POST['em3']==''){$_POST['em3']=0;}
	if($_POST['mega_check']==''){$_POST['mega_check']='NULL';}else{$_POST['mega_check']="'".$_POST['mega_check']."'";}
	if($_POST['btk']=='' or trim($_POST['btk'])=='N/A'){$btk='NULL';}else{$btk="'".$_POST['btk']."'";}
		$query="Insert Into LORRY_FILL_CHECK 	(FDM_LOT_NO,  LFC_W_LY_NO,  LFC_B_REMAIN_QTY,  LFC_B_TROUGH_TOP,  LFC_B_CHK_COUP_CLR,  LFC_B_CHK_COUP,  LFC_B_CHK_COUP_LINK,  LFC_B_CHK_EXHAUST,  
				...
				VALUES ('".$_SESSION['lid']."','".trim($lorry_no)."',".$remnant.",".$m3.",'Y','Y','Y','Y',".$qty.",".$flow_speed.",".$smp_cnt.",".$btk.",".$qty.",'".$_POST['etq']."','Y','Y','Y','Y','Y','Y','Y','".$_SESSION['uid']."',".$_POST['sti0'].",'".$_POST['sti1']."','".$_POST['sti2']."',".$_POST['mega_check'].",'Y','".$tank_no."',".$_POST['bm3'].",".$_POST['em3'].")";
		$result = mssql_query($query);
		
		
		$query="INSERT INTO TLAQAFLOWSPEED (TestDate, LotNo, SampleNo, speed, Ok, Tester, AnalyzeTime) VALUES (CONVERT(DATETIME, '".date("Y-m-d H:i:s")."', 102), N'".trim($_SESSION['lid'])."', 'F9', ".$flow_speed.", N'1', N'".$_SESSION['uid']."', N'".date("YmdHis")."')";
		$remnant=mssql_query($query);
		if($result){echo "1. Writting LORRY_FILL_CHECK .....<BR>";}else{// my_msg("LORRY_FILL_CHECK重複輸入",'lorry_fill.php')
		;}
	$query2="select FDM_LOT_NO from LORRY_FILL_CHECK where FDM_LOT_NO='".$_SESSION['lid']."'";
	$result2=mssql_query($query2);
	$numrows2=mssql_num_rows($result2);
	if($numrows2==0){
		
		jumpto("Lorry_check.php");
	}
	}
	...
	$cid=trim($jj->cid);
	$jj->cid();
	$_SESSION['chemical']=trim($jj->pdd_chemical);
...
if(isset($_POST['finish_1'])) {
	finish();
	$query="Update FILL_INDICATE Set FID_FILL_STATUS=1 where FDM_LOT_NO= '".$_SESSION['lid']."'";
//	echo $query;
	$result=mssql_query($query);
//	break;
	jumpto("index.php");
...
function sm3(){
	...
	if($cnt[0]>=$smp_cnt){
		echo '<input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="sm2" value="上一步" /><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="smp_fin" autofocus="autofocus" value="  下 一 步  " /></br>';	
	}
}
...
function sm2(){
	...
	echo '充填<br>' ;
	echo '流量(必須'._flow($lot).')： <input type="text"  autocomplete="off" name="flow_speed" value=""></br>比抵抗 (IPA>5.0GΩcm)：<input type="text"  autocomplete="off" name="btk" id="btk"  style="font-size:'.($_SESSION['font_size']).'px" autofocus="autofocus" value="N/A"/>';
...
// 	echo '<input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="submit3" id="submit3" value="  下 一 步  " /><input type="submit" name="sm1" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" value="上一步" /></br>';
	...
	if($numrows>0){
		...
		$_SESSION['smp_cnt']=$cnt[0];
		if($cnt[0]>=$smp_cnt){
			echo '<input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="smp_fin" autofocus="autofocus" value="  下 一 步  " /><input type="submit" name="sm1" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" value="上一步" /></br>';	
		}	
	}
function sm1(){
	$_SESSION['remnant']=$_POST['remnant'];
	...
	echo '控制室(%)： <input type="text"  autocomplete="off" name="m3" id="m3"  style="font-size:'.($_SESSION['font_size']).'px" size="10"   autofocus="autofocus" /><BR>';
	echo '控制室(m^3)： <input type="text"  autocomplete="off" name="bm3" id="bm3"  style="font-size:'.($_SESSION['font_size']).'px" size="12"   autofocus="autofocus" /></br>';
	...
	echo '充填量設定： <input type="text"  autocomplete="off" name="qty" id="qty"  style="font-size:'.($_SESSION['font_size']).'px" size="12"  onKeyPress="return SubmitEnter("this",event)"/></br>';
	echo '<input type="submit" name="submit2" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" id="submit2" value="  下 一 步  "/></br>';
...
function tank(){
	...
	echo '<input type="hidden" name="purge_qty" id="qty" value="'.$_POST['purge_qty'].'" />';
	echo '<input type="hidden" name="flow_speed" value="'.$_POST['flow_speed'].'" />';
	echo '來源 LOT NO:<input type="text" style="font-size:'.($_SESSION['font_size']).'px" size="10" name="source_lot" value="" /><BR>';
	echo '充填來源：<select name="tank_no" style="font-size:15" id="tank_no"><option value=""></option>';
	...
	echo '<BR>選擇充填路徑<table width="640" border="1" bgcolor="#CCCCCC"><tr><td width="160">系別</td><td width="160">TANK</td><td width="160">Pump</td><td width="160">Filter1</td></tr><tr bgcolor="white"><td>';
	select_series($pid);
	echo '</td><td>';
	...
	select_Pump($pid);
	//p1
	echo '<br><input type="text" size="5" name="p0" value="'.$_SESSION['p0'].'" onchange="set_date_session(this.name,this.value)">kg/cm^2';
	echo '</td><td>';
	select_Filter1(trim($jj->pid));
	//f1
	echo '<br><input type="text" size="5" name="f1" value="'.$_SESSION['f1'].'" onchange="set_date_session(this.name,this.value)">kg/cm^2';
	echo '</td></tr><tr>';
	echo '<td width="100">Filter2</td><td width="100">充填口</td><td width="100">充填壓力</td><td width="100"></td></tr><tr bgcolor="#FFFFFF"><td>';
	select_Filter2($pid);
	//f2
	echo '<br><input type="text" size="5" name="f2" value="'.$_SESSION['f2'].'"onchange="set_date_session(this.name,this.value)">kg/cm^2';
	echo '</td><td>';
	select_Spot($pid);
	echo '</td><td>';
	echo $_SESSION['p0'].'kg/cm^2';
	echo '</td><td>';
	
	echo '</td></tr></table>';	
	echo '</br></br><input type="submit" style="width:100px;border:2px orange double;" name="finish_1" value="   暫存並離開   " /><input type="submit" style="width:100px;border:2px orange double;" name="finish" value="   下 一 步   " /><input type="submit" style="width:100px;border:2px orange double;" name="submit3" value="上一步" /></br>';
}
...
function finish(){
	...
	$query="SELECT leave FROM Tank_batch where active=1 and tank_no='".$_POST['tank_no']."' ORDER BY [index] DESC";
//	echo $query."<BR>";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	$leave=$row[0];
...
//	echo $query."<BR>";
/*	
		if(($leave*1000)<=$_POST['qty']){
		echo '充填量:'.$_POST['qty']."<BR>";
		echo 'Tank '.' 殘量:'.$row[0]." M^3 <BR>";
		echo '<font color="#FF0000">存量不足</font>' 	;
	}
*/
// 8. 封槽管理項目
	$query="SELECT *  FROM Tank_batch where active=1 and tank_no='".$_POST['tank_no']."'";
	$result=mssql_query($query);
	$numrows=mssql_num_rows($result);
	if($numrows>0){
			$row=mssql_fetch_row($result);
...
//			echo $query."<BR>";
			$result=mssql_query($query);
			$query="update Tank_batch set active=0  where active=1 and tank_no='".$_POST['tank_no']."'";
			$result=mssql_query($query);
...
//	echo "<BR>".$query."<BR>";
	...
	if($numr==0){
		if($_SESSION['btk']=='N/A'){$_SESSION['btk']=0;}
		if(trim($_POST['btk'])=='N/A'){$_POST['btk']=0;}
		...
		$query="INSERT INTO lorry_fill_tmp (lot_no, remnant, m3, btk, [mega-check], purge_qty, tank_no, LFC_TANK, flow_speed, smp_cnt, update_time, lorry_no, qty, bm3)
			VALUES  (N'".$_SESSION['lid']."', ".$_SESSION['remnant'].", ".$_SESSION['m3'].", '".$_SESSION['btk']."', ".$_SESSION['mega_check'].",".$_SESSION['purge_qty'].", N'".trim($_SESSION['tank_no'])."', N'".trim($_POST['tank_no'])."', ".$_SESSION['flow_speed'].", ".$_SESSION['smp_cnt'].", N'".date("YmdHis")."', '".trim($_SESSION['lorry_no'])."',".$_SESSION['qty'].",".$_SESSION['bm3'].")";
		$result=mssql_query($query);
	...
	echo '充填路徑： <input type="text"  autocomplete="off" name="flow" value="'.$flow.'"  style="font-size:'.($_SESSION['font_size']).'px" size="12"  readonly="readonly"/></br>';
	echo '<input type="hidden" name="remnant" id="remnant" value="'.$_POST['remnant'].'"/>';
	echo '<input type="hidden" name="m3" id="m3" value="'.$_POST['m3'].'" />';
	echo '<input type="hidden" name="bm3" id="m3" value="'.$_POST['bm3'].'" />';
	echo '<input type="hidden" name="qty" id="qty" value="'.$_POST['qty'].'" />';
	...
	echo '</br></br><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="end" value="  完成/結束  " /><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="smp_fin" value="上一步" /></br>';
	echo '</form>';	
}
...
function finished()
{
	...
	$query="select * from lorry_fill_tmp where lot_no='".$_POST['lid']."'";
	$result = mssql_query($query);
	while($row=mssql_fetch_array($result))
	{
		$remnant=$row['remnant'];
...
WHERE          (FDM_LOT_NO = '".$_POST['lid']."')";
	...
	echo '<input type="checkbox" name="d1" id="c1" checked="checked" />HOSE外觀檢查<br>
		...
		<input type="checkbox" name="d6" value="1" id="c3"  checked="checked" />Coupler/O-ring狀態確認<br>
		<input type="checkbox" name="d7" value="1" id="c4"  checked="checked" />軟管收納<br>';
	echo '</br></br><input type="submit" style="width:100px;height:'.($_SESSION['font_size']+10).'px;border:2px orange double;" name="end" value="  完成/結束  " /></br>';
	echo '</form>';	
}
...
function select_Tank($pid){
	if($_SESSION['pidx']=='P050-000'){
		echo '<select name="select_Tank" id="select_Tank"  onchange="set_date_session(this.name,this.value)">';
		echo '<option value=""></option>';
		$query="SELECT nextitem FROM FILL_FLOW_ORDER WHERE (chemical = N'H2SO4') AND (itemname = N'".trim($_SESSION['select_series'])."')";
		$result = mssql_query($query);
		while($row=mssql_fetch_array($result)){		
			if(trim($row['nextitem'])==$_SESSION['select_Tank']){$select='selected';}
			else{$select='';}
			echo '<option value="'.$row['nextitem'].'" '.$select.'>'.$row['nextitem'].'</option>';
		}
		echo '</select>';
...
function select_Pump($pid){
	if($_SESSION['pidx']=='P050-000'){
		...
		echo '<option value=""></option>';
		$query="SELECT nextitem FROM FILL_FLOW_ORDER WHERE (chemical = N'".$_SESSION['chemical']."') AND (itemname = N'".trim($_SESSION['select_Tank'])."')";
		$result = mssql_query($query);
		while($row=mssql_fetch_array($result)){		
			if(trim($row['nextitem'])==$_SESSION['select_Pump']){$select='selected';}
			else{$select='';}
			echo '<option value="'.$row['nextitem'].'" '.$select.'>'.$row['nextitem'].'</option>';
		}
		echo '</select>';
...
function select_Filter1($pid){
	if($_SESSION['pidx']=='P050-000'){
		...
		echo '<option value=""></option>';
		$query="SELECT nextitem FROM FILL_FLOW_ORDER WHERE (chemical = N'".$_SESSION['chemical']."') AND (itemname = N'".trim($_SESSION['select_Pump'])."')";
		$result = mssql_query($query);
		while($row=mssql_fetch_array($result)){		
			if(trim($row['nextitem'])==$_SESSION['select_Filter1']){$select='selected';}
			else{$select='';}
			echo '<option value="'.$row['nextitem'].'" '.$select.'>'.$row['nextitem'].'</option>';
		}
		echo '</select>';
...
function select_Filter2($pid){
	if($_SESSION['pidx']=='P050-000'){
		...
		echo '<option value=""></option>';
		$query="SELECT nextitem FROM FILL_FLOW_ORDER WHERE (chemical = N'".$_SESSION['chemical']."') AND (itemname = N'".trim($_SESSION['select_Filter1'])."')";
		$result = mssql_query($query);
		while($row=mssql_fetch_array($result)){		
			if(trim($row['nextitem'])==$_SESSION['select_Filter2']){$select='selected';}
			else{$select='';}
			echo '<option value="'.$row['nextitem'].'" '.$select.'>'.$row['nextitem'].'</option>';
		}
		echo '</select>';
...
function select_Spot($pid){
	if($_SESSION['pidx']=='P050-000'){
		...
		echo '<option value=""></option>';
		$query="SELECT nextitem FROM FILL_FLOW_ORDER WHERE (chemical = N'".$_SESSION['chemical']."') AND (itemname = N'".trim($_SESSION['select_Filter2'])."')";
		$result = mssql_query($query);
		while($row=mssql_fetch_array($result)){		
			if(trim($row['nextitem'])==$_SESSION['select_Spot']){$select='selected';}
			else{$select='';}
			echo '<option value="'.$row['nextitem'].'" '.$select.'>'.$row['nextitem'].'</option>';
		}
		echo '</select>';
...
function samp_count($lid){
	$query="select count(SMA_LOT) as aaaa from Sample_All where SMA_LOT = '".$lid."'";
//	echo "<BR>".$query."<BR>";
	$result=mssql_query($query);
