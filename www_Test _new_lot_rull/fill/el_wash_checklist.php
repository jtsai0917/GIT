...
where		SUBSTRING(FI.FID_FILL_BEGIN_DATE, 0, 9)>='".$date1."' and SUBSTRING(FI.FID_FILL_BEGIN_DATE, 0, 9)<='".$date2."'";
// echo $query;
	 ...
	 echo '<table border="1" width="60%">';
		...
		{		
			while($row=mssql_fetch_array($result))
			{
				...
				echo '<td> <input type="button" name="rull" id="rull" value="列印" onClick="'."window.open('./index.php?url=testb&id=".$row['FDM_LOT_NO']." ', '_self');".'"/>';
