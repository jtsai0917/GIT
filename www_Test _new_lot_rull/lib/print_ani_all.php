	...
	while($row = mssql_fetch_array($result))
	{
		...
		while($row1 = mssql_fetch_array($result1))
		{
			...
			if(!isset($this->Fix_AnalyzeTime)){
				...
				if($rowname=='TestDate'){$value="西曆 ".substr($this->Fix_AnalyzeTime,0,4)." 年 ".substr($this->Fix_AnalyzeTime,4,2)." 月 ".substr($this->Fix_AnalyzeTime,6,2)." 日";}	
				if($rowname=='AnalyzeTime'){$value="西曆 ".substr($this->Fix_AnalyzeTime,0,4)." 年 ".substr($this->Fix_AnalyzeTime,4,2)." 月 ".substr($this->Fix_AnalyzeTime,6,2)." 日";}	
				$document->setValue($rowname.$i,$value);	
				$_SESSION[$rowname.$i]=$value;		
			}
		}
