...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_INDEX'] ?>" <?php echo testnotonick($row['ANI_INDEX'],$_SESSION['prod_no'],trim($_SESSION['items']).",")?>/></td></tr>
<?php
}
?>
</table>
...
if(isset($_POST["normal"])){
	...
	if($_SESSION['needno']>150 and $_SESSION['needno']<500){$cid='C00001';
	$query="select Rull_Reg,CTD_CUST_NO from  Analyze_Rulls where PDD_PROD_NO='".$_SESSION['prod_no']."' and CHARINDEX('".$cid."',CTD_CUST_NO)>=1";
	...
	if($numRows>0){
		...
		$url="index.php?url=add_testitems&cust_no=".$cid;
		jumpto($url);
		}
	else{
		$query="SELECT          dbo.AnalyzeItem.ANI_INDEX
	...
	elseif($_SESSION['needno']<=150){
	$query="SELECT          CTD_CUST_NO,FDM_SPECIFIC
			FROM              FILLPLAN_OUT_DECIDE
			WHERE          (FDM_LOT_NO = '".$_SESSION['lot_no']."')";
	$result = mssql_query($query);
	$row = mssql_fetch_row($result);
	$cid=$row[0];$p_type=$row[1];
	if($p_type=='DM')
	{
		$query="SELECT          CTD_CUST_NO, FDM_LOT_NO
...
if(isset($_POST["all"])){
	$_SESSION['items']="";
	if($_SESSION['needno']>=150 and $_SESSION['needno']<270){$cid='C00001';
	$query="select Rull_Total,CTD_CUST_NO from  Analyze_Rulls where PDD_PROD_NO='".$_SESSION['prod_no']."' and CHARINDEX('".$cid."',CTD_CUST_NO)>=1";
	$result = mssql_query($query);
	$numRows = mssql_num_rows($result);
	if($numRows>0){
		$row = mssql_fetch_row($result);
	...
	elseif($_SESSION['needno']<=150 or $_SESSION['needno']>=500 and $cid==''){
	$query="SELECT           CTD_CUST_NO,FDM_SPECIFIC
			FROM              FILLPLAN_OUT_DECIDE
			WHERE          (FDM_LOT_NO = '".$_SESSION['lot_no']."')";
...
if (isset($_POST["add1"])){
	...
	if($_SESSION['items1']==''){$_SESSION['items1']='0';}
