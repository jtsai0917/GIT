  ...
  <tr class="GridviewScrollHeader">';
		 ...
		 品名</td><td>'."LOTNO".'</td><td>'."出荷日".'</td><td>'."出荷檢查日".'</td>';
			...
			$query="SELECT DISTINCT 
                            ...
                            DRUM_HISTORY_NORMAL.DHN_TYS_RCV_DATE1
FROM              OUT_CHECK_DRUM_DETAIL INNER JOIN
                            ...
                            DRUM_HISTORY_NORMAL ON OUT_DECISION.CTD_CUST_NO = DRUM_HISTORY_NORMAL.CTD_CUST_NO1 AND 
                            OUT_CHECK_DRUM_DETAIL.OCD_DRUM_NO = DRUM_HISTORY_NORMAL.DHN_DRUM_NO AND 
                            OUT_CHECK_DRUM_DETAIL.OCD_LOT_NO = DRUM_HISTORY_NORMAL.DHN_LOT_NO1 ";
			...
			{		
				...
				if(substr($row['OPM_ETA_DATE'],0,8) < $row['DHN_TYS_RCV_DATE1']){$rcv='Y';}else{$rcv='N';}
	//			  echo "EAT:" .substr($row['OPM_ETA_DATE'],0,8) . "RCV:".$row['DHN_TYS_RCV_DATE1']."RCV: ".$rcv."<BR>";
					...
					echo  '<td>'.$row['OCD_LOT_NO'].'</td>';
			  		...
			  		echo  '<td>'.std($row['DHN_TYS_RCV_DATE1']).'</td>';      
			   		echo  '<td>'.$rcv.'</td>';      
...
//			echo $query."<BR>";
			...
			{			
				if(substr(trim($row1['OPD_LOT_NO']),-4,1)=='D' or substr(trim($row1['OPD_LOT_NO']),-4,1)=='B' ){$lotno=substr(trim($row1['OPD_LOT_NO']),0,-4);}else{$lotno=trim($row1['OPD_LOT_NO']);}
				if(substr($row1['OPM_ETA_DATE'],0,8) < $row1['DHN_TYS_RCV_DATE1']){$rcv='Y';}else{$rcv='N';}
			//	echo $row['prodno']."<BR>";
				$objPHPExcel->getActiveSheet()->setCellValue("A".$p,$rr);
				$objPHPExcel->getActiveSheet()->setCellValue("B".$p,$row1['OCD_LOT_NO']);		
				...
				$objPHPExcel->getActiveSheet()->setCellValue("J".$p,std($row1['DHN_TYS_RCV_DATE1']));
