...
<form id="form1" name="form1" method="post" action="">
  <table width="640" border="1">
    <tr align="center">
      ...
      <td>容量 (L)</td>
      <td>輸送車別(1.LY 拖車.  2.鷗翼車</td>
      <td>動作</td>
    </tr>
    <tr align="center">
      <td><input type="text" name="id" id="id" /></td>
      <td><input type="text" name="name" id="name" /></td>
      <td>1<input type="radio" name="radio" id="place" value="1" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			2<input type="radio" name="radio" id="place" value="2" /></td>
...
if(isset($_POST["add"])){
	...
	if($row[0]==0){
		$query="INSERT INTO dbo.BIG_DRUM_VOLUME (PDD_PROD_SHORT_NAME, BDV_VOLUME, CARRY_TYPE)	VALUES ('".$_POST['id']."', ".$_POST['name'].",".$_POST['radio'].")";
		$result = mssql_query($query);
	}
	elseif($row[0]>>0){
		$query="UPDATE  BIG_DRUM_VOLUME SET BDV_VOLUME = ".$_POST['name'].", CARRY_TYPE=".$_POST['radio']." WHERE (PDD_PROD_SHORT_NAME = '".trim($_POST['id'])."')";
		$result = mssql_query($query);
	}
	refresh();
}
