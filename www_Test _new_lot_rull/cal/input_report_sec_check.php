...
<table width="1200" border="1">
    <tr class="centet" >
      ...
      <?php if($xx==1){echo "合";}else{echo "否";} ?>
      <input type="hidden" name="Ok" id="Ok" value="<?php echo $xx;?>" readonly>   &nbsp;&nbsp;&nbsp;&nbsp; 
      ...
      <?php
	  	if (isset($_POST["refresh"])){
			echo '<input type="submit" name="submit" id="submit" value=" 新  增 "/>';
		}else{
			echo '<input type="submit" name="refresh" id="refresh" value=" 檢 查 "/>';	
		}
	  ?>
...
$q2=" VALUES     (";
while($row = mssql_fetch_array($result)){
	if(($row['COLUMN_NAME']=='B') and $_GET['pdd_chemical']=='H2O' and $_GET['ani_groupname']<>'TT_B'){
