...
$query="SELECT DISTINCT 
                            ...
                            PRODUCT_DATA.PDD_TYPE, PRODUCT_DATA.PDD_PROD_NO, PRODUCT_DATA.PDD_CHEMICAL, AnalyzeDesign.CTD_CUST_NO, 
                            PRODUCT_DATA.PDD_PROD_NAME, AnalyzeDesign.AND_ITEM, AnalyzeDesign.AND_NEED_NO, AND_NOTE, 
                            PRODUCT_TYPE.department, AnalyzeDesign.rut, AnalyzeDesign.AND_SMP_DATETIME, 
                            Analyze_Urgent.Lot_No,AnalyzeDesign.sample_metal_f, AnalyzeDesign.sample_particle_f
...
if (trim($_POST['cust_no']<>'')){
	...
	$query=$query." AND (AnalyzeDesign.CTD_CUST_NO ='".$_SESSION['cust_no']."')";
}
...
<table width="1240" border="1">
  <tr bgcolor="#CCCCCC">
    ...
    <td width="28">定常</td>
    <td width="158">備註1</td>
    <td width="28">先行<p>樣品<br />Metel</p></td>
    <td width="28">先行<p>樣品<br />Partical</p></td>
    ...
    <p><font color="purple">紫色</font>表示沒有報告但是有檢驗結果‧<span style="background-color: red"><font color="white">紅底白字</font></span>表示檢驗未通過‧</p></td>
		<td width="131">簽核<BR><input type="submit" name="sign_selected" value="簽核(勾選)" <?php echo $disable2; ?>>  <br>
		<input type="submit" name="urgent" value="急件確認" <?php echo $disable1; ?>><br><input type="submit" name="urgent1" value="Pi-Run確認" <?php echo $disable1; ?>>
		</td>
  </tr>
...
//	elseif($diff->flag=='1'){$note=$diff->item.' 差異太大';echo '<tr bgcolor="#FF9900">';}
	...
	echo '<td>'.$rut.'</td>';
	echo '<td>'.$note.'</td>';
	echo '<td>'.$row['sample_metal_f'].'</td>';
	echo '<td>'.$row['sample_particle_f'].'</td>';
	echo '<td>'.$rpd.'</td>';
	...
	analyzereport($row['AND_ITEM'],trim($row['AND_LOT_NO']),$row['EMP_NAME'],$row['PDD_CHEMICAL'],$row['PDD_PROD_NO'],$row['AND_REPORT_DATETIME']);
	 ...
	 $urgent1=check_urgent1(trim($row['AND_LOT_NO']));

	 echo '<BR><input type="checkbox" name="chkbox'.$p.'"'.$urgent.' >急件';
		echo '<input type="checkbox" name="chkboxx'.$p.'"'.$urgent1.' >Pi-Run';
...
function check_urgent1($lotno){
	$query1="SELECT active FROM Analyze_pirun where canceled=0 and Lot_No='".$lotno."'";
	$result1=mssql_query($query1);
	$row=mssql_fetch_row($result1);
	if($row[0]==1){
		return ' checked="checked" ';
