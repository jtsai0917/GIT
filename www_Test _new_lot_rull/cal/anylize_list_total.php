...
<table width="1240" border="1">
  <tr>
  <td>
製品、受入檢查作業台帳  <?php echo $_GET['lot_no'];?> &nbsp;&nbsp;&nbsp;&nbsp; 只印充填口<input type="checkbox" name="isly"  /></td></tr></table>
  <table width="1240" border="1">
    <tr>
    <?php 
	$query="";
	...
	or ($_GET['pid']=='H2O2') or ($_GET['pid']=='H2SO4') or (($_GET['pid']=='HF') and ($needno<=150)) or ($_GET['pid']=='HNO3'))){
		...
		if(substr($_GET['lot_no'],-1,1)=='_'){
			$gflot->lid=substr($_GET['lot_no'],0,-1);
		}
		else{
	...
	$x=$x+1;
	  ...
	  ?>
      <td align="center">
      <input type="submit" name="print" id="print" value=" 列 印 AMP" />
      <input type="submit" name="print_1" id="print_1" value=" 列 印 AMP(21)" />
...
if((isset($_POST["print"]) and ($_GET['type']=='LY')) or ($_POST['isly']=='on') or (isset($_POST["print"]) and ($needno>=270)) or (isset($_POST["print"]) and ($_GET['pid']=='CAN'))) {
	$dat=date("YmdHis");
...
//	echo "<BR>";
	...
	$ott=list_username($table,$SampleNo,$_GET['lot_no']);
	if($ott<>''){
		$Tester=$ott;
	}
	if($table=='TLLQA9100301') {echo "<BR>".$table.":".$Ok."<BR>";}
...
/*	
	if(((trim($stdu)<>'') and (trim($stdl)<>'')) and (($report>$stdu) or ($report<$stdl)) and ($report<>'')){$Ok="否";echo 1;}
	if(((trim($stdu)<>'') and (trim($stdl)<>'')) and (($report<=$stdu) or ($report>=$stdl)) and ($report<>'')){$Ok="合";echo 2;}
*/
	if(($report<=$usl and $report>$stdu) or ($report>=$lsl and $report<$stdl)) {$Ok="否";}
	if($stdu<>'' and $stdl<>''){
		if($report>=$stdl and $report<=$stdu){$Ok="合";}	
	}
...
//	$spc='________';	$unit='________';  $dl='________';
	...
	if($ani_nick=='Au')
	{
		...
		$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue("'G".$au."'",mb_convert_encoding($Tester,"utf-8","big5"))
...
or ($_GET['pid']=='NH4OH') or ($_GET['pid']=='IPA') or ($_GET['pid']=='HNO3') or ($_GET['pid']=='HF'))){
	...
	if($prt->pdd_chemical =='HNO3'){$prt->activesheet=6;$prt->prt_3_HNO3();}
...
if(isset($_POST["print_1"])){ 
	...
	require_once "../PHPEXCEL/Classes/PHPExcel/IOFactory.php";
	$fileurl="../tmp1/T__".$_GET['lot_no'];
	$sqlstr="SELECT          FILLPLAN_OUT_DECIDE.FDM_QTY_DRUM
...
WHERE          (AnalyzeDesign.AND_LOT_NO = '".$_GET['lot_no']."')";
	$rlt=mssql_query($sqlstr);
	$rr=mssql_fetch_row($rlt);
	$dum_qty=$rr[0];

	$query="SELECT          AnalyzeDesign.AND_ITEM, AnalyzeDesign.AND_SMP_DATETIME, AnalyzeDesign.AND_TOTAL, 
                            AnalyzeDesign.AND_REPORT_DATETIME, PRODUCT_DATA.PDD_CHEMICAL
FROM              AnalyzeDesign INNER JOIN PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO 
WHERE          (AND_LOT_NO = '".$_GET['lot_no']."')";
//	echo $query."<BR>";
	$rlt=mssql_query($query);
	$rr=mssql_fetch_row($rlt);
	$beg='1~1';
	$mid=trim(floor($dum_qty/2)).'~'.trim(floor($dum_qty/2));
	$end=trim($dum_qty).'~'.trim($dum_qty);
	$chemical=trim($rr[4]);
//	echo $mid.":".$end."###<BR>";
	
	$objPHPExcel = new PHPExcel();
	$objPHPExcel = PHPExcel_IOFactory::load("./form/count_21.xlsx");	
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setCellValue('B3',iconv("big5","utf-8",$_GET['lot_no']));
	$objPHPExcel->getActiveSheet()->setCellValue('B4',iconv("big5","utf-8",$rr[2]));
	$objPHPExcel->getActiveSheet()->setCellValue('E4',iconv("big5","utf-8",ddd($rr[3])));
	$objPHPExcel->getActiveSheet()->setCellValue('E2',iconv("big5","utf-8",$cust_name));
	
	$reader = PHPExcel_IOFactory::createReader('Excel2007'); // 讀取2007 excel 檔案
	$PHPExcel = $reader->load("./form/count_21.xlsx"); // 檔案名稱 需已經上傳到主機上
	$sheet = $PHPExcel->getSheet(0); // 讀取第一個工作表(編號從 0 開始)
	$highestRow = $sheet->getHighestRow(); // 取得總列數
	// 一次讀取一列
	for ($row = 9; $row < $highestRow; $row++) {
		if($row<>8 and $row<>10 and $row<>25 and $row<>48){
		$val = $sheet->getCellByColumnAndRow(0, $row)->getValue();
		list($smpnoa,$a,$oka,$testera,$Operatora, $AnaManagera, $AnalyzeTimea,$dmnoa)=ttt($val,$chemical,$beg,$_GET['lot_no']);
		list($smpnob,$b,$okb,$testerb,$Operatorb, $AnaManagerb, $AnalyzeTimeb,$dmnob)=ttt($val,$chemical,$mid,$_GET['lot_no']);
		list($smpnoc,$c,$okc,$testerc,$Operatorc, $AnaManagerc, $AnalyzeTimec,$dmnoc)=ttt($val,$chemical,$end,$_GET['lot_no']);
		if($oka==1){$oka='合';}elseif($oka==0) {$oka='否';} else{$oka='';}
		if($okb==1){$okb='合';}elseif($oka==0) {$okb='否';} else{$okb='';}
		if($okc==1){$okc='合';}elseif($oka==0) {$okc='否';} else{$okc='';}
		$ok=$oka." / ".$okb." / ".$okc;
		$tester=$testera." / ".$testerb." / ".$testerc;
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$row,iconv("big5","utf-8",$a));
		$objPHPExcel->getActiveSheet()->setCellValue('E'.$row,iconv("big5","utf-8",$b));
		$objPHPExcel->getActiveSheet()->setCellValue('F'.$row,iconv("big5","utf-8",$c));
		$objPHPExcel->getActiveSheet()->setCellValue('G'.$row,iconv("big5","utf-8",$ok));
		$objPHPExcel->getActiveSheet()->setCellValue('I'.$row,iconv("big5","utf-8",$tester));
		$objPHPExcel->getActiveSheet()->setCellValue('J'.$row,iconv("big5","utf-8",std($AnalyzeTimea)));
		if($row==9){
			$objPHPExcel->getActiveSheet()->setCellValue('D'.($row-1),iconv("big5","utf-8",$smpnoa));
			$objPHPExcel->getActiveSheet()->setCellValue('E'.($row-1),iconv("big5","utf-8",$smpnob));
			$objPHPExcel->getActiveSheet()->setCellValue('F'.($row-1),iconv("big5","utf-8",$smpnoc));
		}
		if($row==11){
			$objPHPExcel->getActiveSheet()->setCellValue('D'.($row-1),iconv("big5","utf-8",$smpnoa));
			$objPHPExcel->getActiveSheet()->setCellValue('E'.($row-1),iconv("big5","utf-8",$smpnob));
	...
	$objPHPExcel->getActiveSheet()->getStyle("I9:I52")->getFont()->setSize(6);
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, "Excel2007");
	$objWriter->save($fileurl.".xlsx");
	echo '<script>document.location.href="http://'.$path_root.'/tmp1/'.$fileurl.'.xlsx";</script>';	
}
...
function list_username($table,$smpno,$lotno){
	$query="SELECT          TOP (1) EMPLOYEE_DATA.EMP_NAME 
FROM              analyze_first INNER JOIN
                            ".$table." ON analyze_first.lot_no = ".$table.".LotNo AND 
                            analyze_first.create_time = ".$table.".AnalyzeTime INNER JOIN
                            EMPLOYEE_DATA ON analyze_first.create_user = EMPLOYEE_DATA.EMP_NO
WHERE          (analyze_first.lot_no = '".$lotno."') AND (".$table.".SampleNo = '".$smpno."') AND (".$table.".Ok = '1')
ORDER BY   ".$table.".AnalyzeTime DESC";
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	return $row[0];
}

function ttt($itemnick,$chemical,$smp_du,$lot_no){
	$query="SELECT          ELEMENT_FORM.ELF_FORM, AnalyzeItem.ANI_DATAFIELD
FROM              ELEMENT_FORM INNER JOIN
                            AnalyzeItem ON ELEMENT_FORM.ELM_ID = AnalyzeItem.ANI_INDEX
WHERE          (ELEMENT_FORM.PDD_CHEMICAL = '".$chemical."') AND (AnalyzeItem.ANI_NICKNAME = '".$itemnick."')";
	...
	$query="SELECT          SampleNo, [".$itemfield."], Ok,Tester, Operator, AnaManager, AnalyzeTime, 
                            Sample_All.SMA_DRUMNO
FROM              ".$table." INNER JOIN
                            Sample_All ON ".$table.".LotNo = Sample_All.SMA_LOT AND 
                            ".$table.".SampleNo = Sample_All.SMA_ID
WHERE          (LotNo = '".$lot_no."' or LotNo = '".$lot_no."_') AND (Sample_All.SMA_DRUMNO = '".$smp_du."') ORDER BY AnalyzeTime DESC";	
//	echo $query."<BR>";

	$rlt=mssql_query($query);
	$rr=mssql_fetch_row($rlt);
	$num=mssql_num_rows($rlt);
	if($num > 0){
			return array(trim($rr[0]),trim($rr[1]),trim($rr[2]),trim($rr[3]),trim($rr[4]),trim($rr[5]),trim($rr[6]),trim($rr[7]));
	}
	else{
			return array('','','','','','','','');
	}
