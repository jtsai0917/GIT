...
function sql_rec($url,$querys)
{
	...
	$query="INSERT INTO PHP_LOG
			...
			VALUES          ('".$url."','', '".$_SESSION['uid']."', '".date("Y-m-d H:i:s")."') select SCOPE_IDENTITY() as id";
	$_SESSION['Q1']= $query;

	$result=mssql_query($query);
	$LastID = mssql_result( $result, 0, 'id' );
	$query='update PHP_LOG set sql_cmd="'.$querys.'" where index='.$LastID;
	$_SESSION['Q2']=$query;
	$result=mssql_query($query);
}
?>
