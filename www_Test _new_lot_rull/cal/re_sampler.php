  ...
  <table width="1238" border="1">
    <tr>
      <td width="480">分析日期
        ...
        檢驗項目：<?php select_ani_group();?>取樣人員：
        <?php select_Analyst();?> 
...
if(isset($_POST['search']) and $_POST['EMP_NO']=='0'){
	...
	while($row=mssql_fetch_array($result)){
		...
		$aa->ana_times();
		$aa->ana_re();
		echo '<tr><td>'.$i.'</td><td>'.$row['LotNo'].'</td><td>'.$row['PDD_NO'].'</td><td>'.get_prod_name($row['PDD_NO']).'</td><td width="100">'.$row['ani_group'].'</td><td>'.$aa->ani_times.'</td><td>'.$aa->ani_retest.'</td><td>'.$aa->ani_resample.'</td><td width=""></td></tr>';	
...
elseif(isset($_POST['search']) and $_POST['EMP_NO']<>'0'){  // 人員統計
	...
	$total_retest=0;
	$total_resample=0;
	echo '取樣人員：'.get_uname($_POST['EMP_NO']);
	echo '<table border="1" width="1238"/>';
	echo '<tr bgcolor="#CCCCCC"><td width="5">NO</td><td width="20">Lot No</td><td width="40">產品編號</td><td width="100">產品名稱</td><td width="100">分析項目</td><td width="100">分析次數</td><td width="100">再確認次數</td><td width="100">再取樣次數</td><td width="100"></td></tr>';
	$result=mssql_query($query);
	while($row=mssql_fetch_array($result)){
...
class resample{
	...
	function ana_times(){
		$query="SELECT sampler.* FROM sampler";
...
function select_Analyst(){
	...
	$query="SELECT EMP_NAME, EMP_NO FROM EMPLOYEE_DATA WHERE (DEP_NO = 'EP')";
	$result = mssql_query($query);
	while($row=mssql_fetch_array($result)){		
