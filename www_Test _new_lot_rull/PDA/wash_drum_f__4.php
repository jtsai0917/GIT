...
if(isset($_POST['next_step'])){ ///add drums
	...
	$query="Select DHN_USED_COUNT From DRUM_HISTORY_NORMAL Where DHN_DRUM_NO ='".$_POST['dm_no']."'";	
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	$numrow1=mssql_num_rows($result);
	if($numrow1==0){ //新增桶號
		$query="Insert Into DRUM_HISTORY_NORMAL (DHN_WASHED_DATE1, DHN_DRUM_NO, DHN_USED_COUNT, CTD_CUST_NO1, PDD_PROD_NO1,DHN_LOT_NO1) 	VALUES ('".date("Ymd")."','".$_POST['dm_no']."',  0 ,'','".$ppap."', '".$_SESSION['lid']."' )";
		echo $query;
		echo '<BR>';
	...
	{
		$a=$row[0]+1;
		$query="UPDATE  DRUM_HISTORY_NORMAL SET CTD_CUST_NO".$a."='', DHN_OUT_DATE".$a."='', DHN_LOT_NO".$a." = '".$_SESSION['lid']."', PDD_PROD_NO".$a." = '".$ppap."', DHN_WASHED_DATE".$a."='".date("Ymd")."' where DHN_DRUM_NO='".$_POST['dm_no']."'";
//		echo $query."<BR>";
		$result=mssql_query($query);
	...
	$query="INSERT INTO EL_WASH_DRUM
			...
			VALUES ('".$_SESSION['lid']."','".$_POST['dm_no']."',".$used_cnt.",".$ewd_sn.",'Y','Y','Y','Y','".$_POST['min_1']."','Y','Y','".$_SESSION['made_lot']."')";
	$result=mssql_query($query);	
