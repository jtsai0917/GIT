...
輸入要拋轉的 LotNo： <input type="text" name="lotno" size="16" value="'.$_SESSION['lotno'].'" onchange="set_date_session(this.name,this.value)" >';
	...
	$query="SELECT DISTINCT 
                            d.OPM_ORDER_NO AS OrderNo, QC_SpcData.LotNo, AnalyzeDesign.AND_LOT_NO, p.OTD_NO, d.CTD_CUST_NO, 
                            AnalyzeDesign.AND_APPLY_DATE AS pdate, PRODUCT_DATA.PDD_TYPE, 
                            AnalyzeDesign.CTD_CUST_NO AS cno
FROM              PRODUCT_DATA INNER JOIN
                            AnalyzeDesign ON PRODUCT_DATA.PDD_PROD_NO = AnalyzeDesign.AND_GOODS LEFT OUTER JOIN
                            QC_SpcData ON AnalyzeDesign.AND_LOT_NO = QC_SpcData.LotNo LEFT OUTER JOIN
                            OUT_PRODUCT AS p INNER JOIN
                            OUT_DECISION AS d ON p.OPM_ORDER_NO = d.OPM_ORDER_NO ON 
                            AnalyzeDesign.AND_LOT_NO = p.OPD_LOT_NO　WHERE　(AND_NEED_NO <>'') ";
if(trim($_SESSION['lotno'])<>''){
	...
	$query.= " ORDER BY QC_SpcData.LotNo";
...
if(isset($_POST['trans'])){
	...
	for($i=1;$i < $_POST['count'];$i++){
		$query="select count(LotNo) as num from QC_SpcData where LotNo='".$_POST['lno'.$i]."'";
		$result=mssql_query($query);
...
VALUES         
                            ('".$_POST['lno'.$i]."', '".$_POST['sample_no'.$i]."', '".trim($_POST['testdate'.$i])."', '".$_POST['value'.$i]."', '".$_POST['AND_GOOD'.$i]."', '".$_POST['cstno'.$i]."', 
                            '".$_POST['itemname'.$i]."', '".$_POST['itemunit'.$i]."', '".date("Y/m/d")."', 2)";
		echo "<BR>".$query."<BR>";
...
//			echo '<table width="800" border="1"><tr><td>序</td><td>LotNo</td><td>分析群組</td><td>項目</td><td>項目名稱</td><td>紀錄表</td><td>欄位名</td><td>樣品瓶號</td><td>報告值</td><td>單位</td><td>Test Date</td><td>報告時間</td></tr>';
		...
		$result=mssql_query($query);
		echo '資料搜尋中....<BR><form name="d2" action="" method="post">';
...
//		 echo $query2."<BR>";
		...
		echo '<BR><input type="submit" name="trans" value="拋轉"></form><BR>';
