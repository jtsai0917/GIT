...
//	echo $query."<BR>";
		...
		while ($row = mssql_fetch_array($result))
		{
			...
			if(trim($row['disc'])<>'' or trim($row['disc'])=='NULL' )
			{
		...
		$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8","取樣人員"));$ENG++;
		$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8","充填人員"));$ENG++;

				$query1="SELECT DISTINCT 
                            TL.*,
                            AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, AF.first 
FROM              CUSTOMER_DATA RIGHT OUTER JOIN
                            AnalyzeDesign ON CUSTOMER_DATA.CTD_CUST_NO = AnalyzeDesign.CTD_CUST_NO RIGHT OUTER JOIN
                            ".$table1." AS TL  LEFT OUTER JOIN
                            sampler ON TL.LotNo = sampler.lotno LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time ON AnalyzeDesign.AND_LOT_NO = TL.LotNo    
						 where 			 (AND_SMP_DATETIME >= '".dod($_POST['datepicker1'])."0000"."') and  (AnalyzeDesign.AND_GOODS = '".$_POST['pdd_chemical1']."') 
						AND (AND_SMP_DATETIME <= '".dod($_POST['datepicker2'])."9999"."') ORDER BY   TL.LotNo, TL.AnalyzeTime";

//		echo $query1."<BR>";			
					
	//			$_SESSION['OOOOOOOOOOOOOOOOOOOOOO']= $query1;
	/*		if($_POST['lot']<>''){
			$query1="SELECT DISTINCT 
                            *, TL.AnalyzeTime, 
                            AnalyzeDesign.AND_GOODS, CUSTOMER_DATA.CTD_CUST_SHORT_NAME, AF.first  
FROM              ".$table1." AS TL  INNER JOIN
                            ...
                            sampler ON TL.LotNo = sampler.lotno LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time LEFT OUTER JOIN
                            CUSTOMER_DATA ON AnalyzeDesign.CTD_CUST_NO = CUSTOMER_DATA.CTD_CUST_NO   
						 where 			 (AND_REPORT_DATETIME >= '".dod($_POST['datepicker1'])."000000"."') and  (AnalyzeDesign.AND_GOODS = '".$_POST['pdd_chemical1']."') 
						AND (AND_REPORT_DATETIME <= '".dod($_POST['datepicker2'])."999999"."') ORDER BY   TL.LotNo, TL.AnalyzeTime";	
			}
	*/
//				$_SESSION['sssss']=$query1;
				$result1 = mssql_query($query1);
				$numrows1=mssql_num_rows($result1);
			while ($row1 = mssql_fetch_array($result1))
			{
				$query2="SELECT DISTINCT Sample_All.SMA_DRUMNO, EMPLOYEE_DATA.EMP_NAME, EMPLOYEE_DATA_1.EMP_NAME
FROM              FILL_INDICATE INNER JOIN
                            EMPLOYEE_DATA AS EMPLOYEE_DATA_1 ON 
                            FILL_INDICATE.FID_OPERATOR = EMPLOYEE_DATA_1.EMP_NO RIGHT OUTER JOIN
                            Sample_All ON FILL_INDICATE.FDM_LOT_NO = Sample_All.SMA_LOT LEFT OUTER JOIN
                            EMPLOYEE_DATA INNER JOIN
                            sampler ON EMPLOYEE_DATA.EMP_NO = sampler.sampler ON Sample_All.SMA_ID = sampler.sample_id AND 
                            Sample_All.SMA_LOT = sampler.lotno  
WHERE          (Sample_All.SMA_LOT =  N'".$row1['LotNo']."') AND (Sample_All.SMA_ID = N'".$row1['SampleNo']."')";


//				$query2="SELECT Sample_All.SMA_DRUMNO, EMPLOYEE_DATA.EMP_NAME FROM Sample_All INNER JOIN EMPLOYEE_DATA ON Sample_All.SMA_SMP = EMPLOYEE_DATA.EMP_NO where SMA_LOT='".$row1['LotNo']."' and SMA_ID='".$row1['SampleNo']."'";
//				echo $query1.'<br>';
				...
				echo '<td>'.sta($row1['AnalyzeTime']).'</td>';
				...
				$NUM++;$ENG='A';
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$str1));$ENG++;
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row1['SampleNo']));$ENG++;
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row2[0]));$ENG++;
				for($i=0;$i<$numrows;$i++)
				{
					if(is_numeric($row1[trim($item[$i])])==1){
					$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM,round($row1[trim($item[$i])],5));$ENG++;
					}
					else{
					$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM,iconv("big5","utf-8",$row1[trim($item[$i])]));$ENG++;
					}
				}
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",sta($row1['AnalyzeTime'])));$ENG++;	
				...
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row2[1]));$ENG++;
				$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row2[2]));$ENG++;
...
FROM              ".$table1." AS TL INNER JOIN
						...
						AND (AnalyzeTime < '".dod($_POST['datepicker2'])."999999"."') ORDER BY   TL.LotNo, TL.AnalyzeTime";	
			}
*/			
				$result1 = mssql_query($query1);
				$numrows1=mssql_num_rows($result1);
...
if(isset($_POST['submit10']))
{
	...
	//	echo 1;
		...
		$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8","充填人員"));$ENG++;
			...
			$query1="select EMPLOYEE_DATA.EMP_NAME, TL.*,AF.first 
			from  EMPLOYEE_DATA INNER JOIN
                            Sample_All ON EMPLOYEE_DATA.EMP_NO = Sample_All.SMA_SMP RIGHT OUTER JOIN
                            ".$table1." AS TL ON Sample_All.SMA_ID = TL.SampleNo AND 
                            Sample_All.SMA_LOT = TL.LotNo LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time
  where 			 (AnalyzeTime > '".dod($_POST['datepicker1'])."000000"."')
				AND (AnalyzeTime < '".dod($_POST['datepicker2'])."999999"."') and chemical='".$chemical."' order by analyzetime";
//			echo $query1."<BR>000";
				$result1 = mssql_query($query1);
				$numrows1=mssql_num_rows($result1);
			while ($row1 = mssql_fetch_array($result1))
...
//				echo $query2."<BR>";
	...
	/////////////////////////////////////////////////////////////////////////////////////			
		...
		$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8","充填人員"));$ENG++;
			...
			$query1="select EMPLOYEE_DATA.EMP_NAME, TL.*,AF.first from ".$table."  AS TL INNER JOIN
                            Sample_All ON TL.SampleNo = Sample_All.SMA_ID AND TL.LotNo = Sample_All.SMA_LOT INNER JOIN
                            EMPLOYEE_DATA ON Sample_All.SMA_SMP = EMPLOYEE_DATA.EMP_NO LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time
  where 			 (AnalyzeTime > '".dod($_POST['datepicker1'])."000000"."') 
...
if(isset($_POST['submit3']))
{
	...
	//	echo $query;
		...
		if($_POST['lot']<>'')
		{
			...
			while ($row1 = mssql_fetch_array($result1))
			{
			$query2="select EMPLOYEE_DATA.EMP_NAME, TL.*,AF.first from ".$table1."  AS TL INNER JOIN
                            Sample_All ON TL.SampleNo = Sample_All.SMA_ID AND TL.LotNo = Sample_All.SMA_LOT INNER JOIN
                            EMPLOYEE_DATA ON Sample_All.SMA_SMP = EMPLOYEE_DATA.EMP_NO LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time  where 
			  lotno='".$row1['lotno']."' ";
	...
	//	echo $query;
		...
		if($_POST['lot']<>'')
		{
			...
			while ($row1 = mssql_fetch_array($result1))
			{
				$query2="select EMPLOYEE_DATA.EMP_NAME, TL.*,AF.first from ".$table2."  AS TL INNER JOIN
                            Sample_All ON TL.SampleNo = Sample_All.SMA_ID AND TL.LotNo = Sample_All.SMA_LOT INNER JOIN
                            EMPLOYEE_DATA ON Sample_All.SMA_SMP = EMPLOYEE_DATA.EMP_NO LEFT OUTER JOIN
                            analyze_first1 AS AF ON TL.AnalyzeTime = AF.create_time  where 
			  lotno='".$row1['lotno']."' ";
