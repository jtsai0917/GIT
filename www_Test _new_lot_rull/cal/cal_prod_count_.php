  ...
  <table width="1200" border="1">
    <tr>
      <td width="480">分析依賴填寫日期 
		...
		?>" readonly>
        容器容量：
        <input name="textfield" type="text" id="textfield" size="10" value="" />
        Lot No：
        <input type="button" name="X2" id="X2" value="X" onclick="window.open('../erase_lot.php ', '_self');" />
<input name="textfield4" type="text" id="textfield4" size="10" value="<?php if ($_GET['lid']){
			echo trim($_GET['lid']);
			$_SESSION['lid']=$_GET['lid'];
		}
		elseif($_SESSION['lid']){
			echo trim($_SESSION['lid']);
		}
		else{
		echo '';
		}
...
elseif($_SESSION['lid']){
	$query="SELECT    DISTINCT      AnalyzeDesign.AND_REPORT_DATETIME, EMPLOYEE_DATA.EMP_NAME, AnalyzeDesign.AND_MEMO, 
                            AnalyzeDesign.AND_LOT_NO, AnalyzeDesign.AND_GOODS, AnalyzeDesign.AND_APPLY_DATE, PRODUCT_DATA.PDD_TYPE, PRODUCT_DATA.PDD_PROD_NO,PRODUCT_DATA.PDD_CHEMICAL,
                            PRODUCT_DATA.PDD_PROD_NAME, AnalyzeDesign.AND_ITEM, AnalyzeDesign.AND_NEED_NO,dbo.AnalyzeDesign.CTD_CUST_NO
FROM              AnalyzeDesign INNER JOIN
                            PRODUCT_DATA ON AnalyzeDesign.AND_GOODS = PRODUCT_DATA.PDD_PROD_NO INNER JOIN
                            EMPLOYEE_DATA ON AnalyzeDesign.AND_PERSON = EMPLOYEE_DATA.EMP_NO
WHERE          (AnalyzeDesign.AND_LOT_NO LIKE '%".$_SESSION['lid']."%')";
...
{   
    ...
    echo '<td>'.substr($row['AND_REPORT_DATETIME'],0,4)."-".substr($row['AND_REPORT_DATETIME'],4,2)."-".substr($row['AND_REPORT_DATETIME'],6,2).
	" ".substr($row['AND_REPORT_DATETIME'],8,2).":".substr($row['AND_REPORT_DATETIME'],10,2).'</td>';
	if(substr($row['AND_ITEM'],-1)==','){$row['AND_ITEM']=substr($row['AND_ITEM'],0,-1);}
	analyzereport($row['AND_ITEM'],$row['AND_LOT_NO'],$row['EMP_NAME'],$row['PDD_CHEMICAL'],$row['PDD_PROD_NO'],$row['AND_REPORT_DATETIME']);
