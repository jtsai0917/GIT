...
$query="SELECT count(EWD_DRUM) as ss FROM EL_WASH_DRUM WHERE (FDM_LOT_NO = '".$_POST['lid']."') AND (EWD_DRUM = '".$_POST['dm_no']."') and DISABLE <> 1";
$result=mssql_query($query);
$row=mssql_fetch_row($result);
if($row[0]==0){
	my_msg($_POST['dm_no']."此桶號未經洗桶作業","el_drum_f2.php");
}

$query="SELECT  COUNT(FDM_LOT_NO) AS cnt FROM EL_FILLDATA_DRUM WHERE   (FDM_LOT_NO LIKE '".$_POST['lid']."') AND (EFD_DRUM_NO = '".$_SESSION['dm_no']."') and DISABLE <> 1 ";
$result=mssql_query($query);
$row=mssql_fetch_row($result);
$cnt=$row[0];
...
if(isset($_POST['next'])){  // EL_FILLDATA_DRUM
	...
	if (trim($_POST['source_lot'])==''){$_POST['source_lot']='NULL';}
	...
	$sn=$row[0];
	if($sn==''){$sn=1;}
	if(trim($_POST['weight'])==''){$_POST['weight']='NULL';}
...
echo $query."<BR>";
		...
		{
			$queryx="select * from DRUM_HISTORY_NORMAL where DHN_DRUM_NO='".$_POST['dm_no']."' and DHN_LOT_NO1= '".$_SESSION['lid']."'  and disable <>1";
