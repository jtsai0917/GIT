...
datepick();
$query="SELECT          TOP (1) cancel FROM Ani_Signatory WHERE (LotNo = '".$_GET['lot_no']."') ORDER BY sign_datetime DESC ";
...
if($row=mssql_fetch_row($result)){
	...
	if($_GET['ani_groupname']== 'FPMS' or $_GET['ani_groupname']== 'FRION' or $_GET['ani_groupname']== 'PMS' or $_GET['ani_groupname']== 'RION' or $_GET['ani_groupname']== 'TM' ){
		$input='';
	}
}
else{
...
//	echo "PDD_TYPE=".$pdd->pdd_type;
  ...
  {	
	...
	{
		if($_GET['pdd_prod_no']=='P001-200' or $_GET['pdd_prod_no']=='P002-190' or ($_GET['pdd_prod_no']=='P006-330' and substr($lotid,-1,1)=='_'))
		{
...
<table width="1200" border="1">
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <td width="600" align="center">
		<input type="submit" name="submit" id="submit" value=" 新  增 " <?php echo $input; ?>/>
...
echo '<td width="30">列印<br><input type="submit" name="printall" value="列印" ></td>';
...
while ($row=mssql_fetch_array($result))
{
	...
	if( (($row['AnalyzeTime']+3000000)>date("YmdHis"))){echo '<td width="30"><a href="index.php?url=edit_report&ani_groupname='.$_GET['ani_groupname'].'&lot_no='.$_GET['lot_no'].'&pdd_chemical='.$_GET['pdd_chemical'].'&pdd_prod_no='.$_GET['pdd_prod_no'].'&Ok='.$row['Ok'].'&ani_time='.$row['AnalyzeTime'].'&id='.$id.'&efm='.$table.'&first='.$analyze_1st->first.'&smpno='.trim($row['SampleNo']).'" target="new">編輯</a></td>';}
	else{echo '<td></td>';}
...
(樣品不合格)
<input type="submit" name="upload" id="upload" value="送出" <?php echo $input; ?>/>   上傳檔名請不要用特殊字元 (EX: # @ ! % & $)
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      </td>
    </tr>
    </table>
...
echo '</tr>';
$query="SELECT          [index], [FILE_PATH], [FILE_UPLOAD_TIME], [ANI_GROUP], [FILE_PID], [FILE_CID], [FILE_UID], [FILE_LOT_NO], [FILE_DISC], [FILE_NAME] FROM FILE_REPORTS 
...
while($row = mssql_fetch_array($result)){
	...
	echo'<td><a href="'.trim($row['FILE_PATH']).'">'.$row['FILE_NAME'].'</a></td>';
...
if(isset($_POST["upload"])){   
		$queryd="UPDATE AnalyzeGroup_Lot SET filed = 1 WHERE (Lot_No = N'".trim($_GET['lot_no'])."')  AND (Ana_Group_Name = N'".trim($_GET['ani_groupname'])."') ";
...
//		echo "<BR>".$query."<BR>";
	...
	if($_GET['ani_groupname']=='比重'){
		$ani_groupname='density';
	}
	else{
		$ani_groupname=$_GET['ani_groupname'];
	}
	$path="../reports/".$ani_groupname."/";
	$filename=$_GET['lot_no']."_".date("YmdHis").".".$ext;
	if($_GET['ani_groupname']=='比重'){
		$fps='density';
	}
	else{
		$fps=$_GET['ani_groupname'];
	}
	$fullpath="/reports/".$fps."/".$filename;
	if (file_exists($path)){
...
if(isset($_POST["delete"])){   
	if($_SESSION['uname']==$_SESSION['savename'] or $_SESSION['uid']=='E903' or $_SESSION['uid']=='E319' or $_SESSION['uid']=='E038' or $_SESSION['uid']=='E496')
	{
...
if(isset($_POST['slope_'])){
	...
	$lot_rsq=substr($_GET['lot_no'],0,-3)."RSQ";
	$lot_slop=substr($_GET['lot_no'],0,-3)."SLOP";
...
VALUES     (N'".$lot_rsq."', N'".$_GET['ani_groupname']."', '1', N'system', N'".$creat_time."')"; 

	$result=mssql_query($query);
	$query="INSERT INTO ani_result_group
                            (lot_no, ani_group, result, creator, createtime)
VALUES     (N'".$lot_slop."', N'".$_GET['ani_groupname']."', '1', N'system', N'".$creat_time."')"; 
	$result=mssql_query($query);
	...
	$lot=substr($_GET['lot_no'],0,-3).$btl;
