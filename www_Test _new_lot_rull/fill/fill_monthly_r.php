...
<script type="text/javascript" src="/css/jquery.min.js"></script>
        ...
        <tr>
          <td width="824" border="0"><font size="+1">月充填計畫表</font>
            <p><span class="d1"><font size="+1">藥品名:</font>
                ...
                <input name="prod_no" type="text" size="8" value="<?php echo $_SESSION['prod_no']?>" readonly="readonly" />
                <input type="button" name="pdd_no" id="pdd_no" value="查詢品名" onclick="window.open('../main.php?url=pdd_prod_no ', '_self');" />
                <input name="pname" type="text" size="10" value="<?php echo $_SESSION['prod_name']?>" readonly="readonly" />
            <font size="+1">客戶：</font>
            <input type="button" name="X2" id="X2" value="X" onclick="window.open('../erase_customer.php ', '_self');" />
            <input name="cust_no" type="text" size="8" value="<?php echo $_SESSION['cust_no']?>" readonly="readonly" />
            <input type="button" name="pdd_no2" id="pdd_no2" value="查詢客戶" onclick="window.open('../main.php?url=cust_no&sup=N ', '_self');" />
            <input name="cust_name" type="text" id="cust_name" size="10" value="<?php echo get_cust_name($_SESSION['cust_no']);?>" />
            </p>
            <p>
              <input type="checkbox" name="hy" id="hy"/>
              <font size="+1">顯示半年未出貨的客戶
              充填月份：</font>
              <input type="text" name="ym" id="ym" onchange="set_date_session(this.name,this.value)" value="<?php echo $_SESSION['ym'];?>" > 
  ...
  echo '<table id="GridView1" width="1300" border="1" align="left">
...
if(isset($_POST['submit'])){
	if($_POST['hy']=='on'){$_SESSION['hy']='on';}else{$_SESSION['hy']='';}
	$query="SELECT FLM.*,CTD_CUST_SHORT_NAME,CTD.CTD_CUST_NO as CUSTNO,PDD.PDD_PROD_NO as PRODNO,PDD.PDD_PROD_NAME as PRODNAME,CTP.CTP_SEL_LY_TOTO FROM CUSTOMER_PRODUCTS 
	CTP LEFT OUTER JOIN FILLPLAN_LORRY_MONTH FLM ON CTP.CTD_CUST_NO = FLM.CTD_CUST_NO AND CTP.PDD_PROD_NO = FLM.PDD_PROD_NO AND FLM.FLM_YEAR_MONTH = '".$_POST['ym']."' LEFT 
	OUTER JOIN CUSTOMER_DATA CTD ON CTD.CTD_CUST_NO = CTP.CTD_CUST_NO LEFT OUTER JOIN PRODUCT_DATA PDD ON PDD.PDD_PROD_NO = CTP.PDD_PROD_NO  where  (PDD.PDD_DRUM_KG = '' OR PDD.PDD_DRUM_KG IS NULL) and CTP.CTP_FILL = 'Y' and CTD_SUPPLIER='N' ";
	if($_POST['prod_no']<>''){$query.=" AND (CTP.PDD_PROD_NO = '".$_POST['prod_no']."')";}
	if($_POST['cust_no']<>''){$query.=" AND (CTD.CTD_CUST_NO = '".$_POST['cust_no']."') ";}
	if($_POST['hy']<>'on')
	$query.=" ORDER BY PRODNO";
//	echo $_SESSION['OOP']=$query;
	...
	$numRows = mssql_num_rows($result);
	 while($row = mssql_fetch_array($result))
	 {
		...
		for($i=1;$i<=31;$i++)
		{
			...
			echo '<td><a target="_blank" href="index.php?url=edit_FLMYM&ym='.$_POST['ym'].'&cn='.$row['CUSTNO'].'&pn='.$row['PRODNO'].'&day='.$i.'">'.$s.'</a></td>';
...
function check_6m($ym,$cid,$pid)
{
	$query="SELECT DISTINCT FILLPLAN_OUT_DECIDE.*
			FROM              FILLPLAN_OUT_DECIDE where CTD_CUST_NO='".$cid."' and FOD_O_YEAR_MONTH>='".$ym."' and PDD_PROD_NO='".$pid."'" ;
	$result = mssql_query($query);
	$numRows = mssql_num_rows($result);
	return $numRows;
}
    				...
    				for ($c = 0; $c < $highestColumns; $c++) 
						{
							...
							elseif($c==4){
								$aoe=substr(trim($this->value),0,6);
								$lo[$x]=$aoe;
							}
