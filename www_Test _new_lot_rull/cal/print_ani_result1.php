...
<form id="form1" name="form1" method="post" action="">
    ...
    <?php 
	if(trim($_GET['pdd_prod_no'])=='MF-MLI' or trim($_GET['pdd_prod_no'])=='MF-HKO' or trim($_GET['pdd_prod_no'])=='BR-NBP' or trim($_GET['pdd_prod_no'])=='BR-CRP' or trim($_GET['pdd_prod_no'])=='BR-CRBP')
	{
		echo '<input name="one" type="submit" id="one" value="單次">   <input name="more" type="submit" id="more" value=" 統計 ">';
	}
	else
	{
...
<input type="button"  value=" 離 開 " onclick="javascript:window.opener=null;window.open('','_self');window.close();" >
...
if(isset($_POST['one']))
{
	$pdd_prod_no=trim($_GET['pdd_prod_no']);
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("..//FormList/PrintAnalyze/GM/EL_ONE/".trim($_GET['pdd_prod_no']).".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	$query="select * from ".$_GET['table']." where lotno='".$_GET['lot_no']."'  order by serialno";
	$result=mssql_query($query);$ENG='B';$i=0;
	while($row=mssql_fetch_array($result))
	{
		$PP01=$row['PP01'];
		$PP03=$row['PP03'];
		$PP05=$row['PP05'];
		if($pdd_prod_no=='MF-MLI')
		{
			$Num='16';
			$objPHPExcel->getActiveSheet()->setCellValue($ENG.$Num, $PP03);$ENG++;
			$objPHPExcel->getActiveSheet()->setCellValue($ENG.$Num, $PP05);$ENG++;
		}
		elseif($pdd_prod_no=='BR-CRBP')
		{
			if($i<5){$Num='18';}
			if($i==5){$ENG='B';$Num=27;}
			$objPHPExcel->getActiveSheet()->setCellValue($ENG.$Num, $PP01);$ENG++;
			$objPHPExcel->getActiveSheet()->setCellValue($ENG.$Num, $PP05);$ENG++;
		}
		elseif($pdd_prod_no=='BR-CRP')
		{
			if($i<5){$Num='17';}
			if($i==5){$ENG='B';$Num=26;}
			$objPHPExcel->getActiveSheet()->setCellValue($ENG.$Num, $PP01);$ENG++;
...
if (isset($_POST["submit3"]) or isset($_POST['more']))
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("..//FormList/PrintAnalyze/GM/new/".trim($_GET['pdd_prod_no']).".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	if(trim($_GET['pdd_prod_no'])=='GM-CRP' or trim($_GET['pdd_prod_no'])=='BR-CRP' or trim($_GET['pdd_prod_no'])=='BR-CRBP' or trim($_GET['pdd_prod_no'])=='MF-MLI' or trim($_GET['pdd_prod_no'])=='MF-HKO' or trim($_GET['pdd_prod_no'])=='BR-NBP')
	{	$_SESSION['hohoho']='gogogo';
	$pdd_prod_no=trim($_GET['pdd_prod_no']);
	if($pdd_prod_no=='GM-CRP'){$lot='TAC';$lotend='GMP';}
	elseif($pdd_prod_no=='GM-CRBP'){$lot='TAB';$lotend='GMP';}
	elseif($pdd_prod_no=='MF-MLI'){$lot='TMF';$lotend='MLIP';}
	elseif($pdd_prod_no=='MF-HKO'){$lot='TMF';$lotend='HKOP';}
	elseif($pdd_prod_no=='BR-NBP'){$lot='TBR';$lotend='HKOP';}
	elseif($pdd_prod_no=='BR-CRP'){$lot='TBR';$lotend='CRP';}
	elseif($pdd_prod_no=='BR-CRBP'){$lot='TBR';$lotend='CRBP';}
	$NUM=4;$i=0;$times=0;$ENG='B';
		//$query="select distinct top 14 lotno from ".$_GET['table']." where lotno like '".$lot."%' and lotno like '%".$lotend."' order by lotno desc ";
		$query="SELECT distinct top 14 AND_LOT_NO as lotno,AND_APPLY_DATE from AnalyzeDesign where AND_LOT_NO like '%".$lot."%' and AND_LOT_NO like '%".$lotend."' and AND_CANCEL=0 ORDER BY AND_APPLY_DATE DESC";
		//echo $query;
		...
		{	
			$ENG=times($times);
			//echo substr($row1['AnalyzeTime'],0,8).','.$row1['PP01'].','.$row1['PPG1'].','.$row1['PP02'].','.$row1['PP03'].','.$row1['PP05'].'<br>';
			$objPHPExcel->getActiveSheet()->setCellValue($ENG."2", iconv("big5","utf-8",$row['AND_APPLY_DATE']));
