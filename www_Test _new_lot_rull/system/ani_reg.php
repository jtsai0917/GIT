...
<td><input type="checkbox" name="chkbox[]" id="1" value="<?php echo $row['ANI_INDEX'] ?>" <?php echo $_SESSION['tmp']=testnotonick($row['ANI_INDEX'],$_GET['pid'],trim($_SESSION['ani_reg']).",")?>/></td></tr>
<?php
}
?>
</table>
...
if(isset($_POST["normal"])){
	...
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result)){
	$_SESSION['ani_reg'].= $row['ANI_INDEX'].",";
	}

$url="index.php?url=ani_reg&id=".$_GET['id']."&pid=".$_GET['pid'];
jumpto($url);
}
...
if (isset($_POST["add1"])){
	if($_GET['id']<>''){
	$aa=$_POST['chkbox'];
    $_SESSION['ani_reg']=implode(",", $aa);
	$query="UPDATE        dbo.Analyze_Rulls
	SET                  Rull_Reg ='".$_SESSION['ani_reg']."'
	WHERE         ([index] = ".$_GET['id'].")";
$result = mssql_query($query);
	}
