...
{	
	...
	$query="SELECT DISTINCT QC.LotNo, QC.AnalyzeTime, QC.SampleNo, QC.ProdNo, Tester,CD.CTD_CUST_NO
			...
			AND (QC.AnalyzeTime < '".dod($_POST['datepicker2'])."999999"."') AND 
							...
							$queryre="select b.LotNo from ( ".$query;
						$queryre=$queryre."  )  b   group by  b.LotNo having count(b.LotNo)>1    ";
						//echo $query;
						//$chk=0;$NUM=1;
						//echo $queryre;				
	$resultre = mssql_query($queryre);
	$numRows=mssql_num_rows($resultre);
	$ENG='A';
		while ($rowre = mssql_fetch_array($resultre))
		{	$query1=$query."and Lotno='".$rowre['LotNo']."'";
	...
	$query="SELECT DISTINCT QC.LotNo, QC.AnalyzeTime, QC.SampleNo, QC.ProdNo, Tester,CD.CTD_CUST_NO
			...
			AND (QC.AnalyzeTime < '".dod($_POST['datepicker2'])."999999"."') AND 
						...
						if($num_str>1)
							...
							{$query=$query." and (CHARINDEX(','+OrgField+',',',".trim($_SESSION['itemsx']).",')>=1)";}
							
				//		echo $query.'<br>';//b=如果是ACID就再看看別的方法
	...
	$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row['CTD_CUST_NO']));$ENG++;
			for($i=0;$i<$num_str;$i++)
			{
				...
				$query1="select top 1 QC.TestData
					...
					$row['AnalyzeTime']."' and SampleNo='".$row['SampleNo']."' and 		
					orgfield='".$_SESSION['itmstr'][$i]."' and AI.ANI_GROUPNAME='".$strg[0]."' order by id desc ";
					if(trim($strg[0])=="ACID" or trim($strg[0])=="TN"  )
					{//echo "ACID";
					$query1="select top 1 QC.TestData
					from QC_LotData as QC
					left join AnalyzeItem as AI on QC.Itemname=AI.ANI_FULLNAME
					where LotNo='".$row['LotNo']."' and AnalyzeTime='".
					$row['AnalyzeTime']."' and SampleNo='".$row['SampleNo']."' and 		 
					AI.ANI_GROUPNAME='".$strg[0]."' order by id desc ";
					//echo "ACID";
					}
					if($_SESSION['CLCHK']=='on' or $_SESSION['SO4CHK']=='on' or $_SESSION['CO3CHK']=='on')
					{ //echo "陰離子";
						$query1="select top 1 QC.TestData
					from QC_LotData as QC
					left join AnalyzeItem as AI on QC.Itemname=AI.ANI_FULLNAME
