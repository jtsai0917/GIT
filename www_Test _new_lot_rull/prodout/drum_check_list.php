...
FROM              OUT_CHECK_DRUM AS OCM INNER JOIN
	...
	while($row = mssql_fetch_array($result))
	{
		...
		<td>'.$row['a5'].'</td><td>'.$row['a6'].'</td><td>'.$row['a7'].'</td><td>'.$row['a8'].'</td>'.'<td>'.'<input type="button" name="rull" id="rull" value="列印" onClick="'."window.open('./index.php?url=drum_check_print&id=".$row['a9']." ', '_self');".'"/>';
