...
$result = mssql_query($query);
echo '<table width="1900" border="1"><tr>';
echo '<td width="40">勾選</td>';
...
FROM              ".$table." 
WHERE          (LotNo  Like '".$_GET['lot_no']."%') order by AnalyzeTime";
...
while ($row=mssql_fetch_array($result))
{
	...
	$auth->keyin_isfit();
	echo '<tr>';	
	echo '<td width="40" align="center"><input type="checkbox"  name="'.$chkbox.'" value="'.$row['AnalyzeTime'].'"  style="vertical-align:middle;" CHECKED></td>';
	$chkbox++;
...
echo '<input type="button" name="delete" id="delete" value="上傳報告" onClick="'."window.open('./upload_tentimes.php?ani_groupname=".$_GET['ani_groupname']."&lotno=".$_GET['lot_no']."&table=".$_GET['efm']." ', '_blank');".'"/>';

echo '<input type="submit" name="printer" value="列印">';

if(isset($_POST['printer']))
{
if($_GET['ani_groupname']=='M21' or $_GET['ani_groupname']=='M13' or  $_GET['ani_groupname']=='TT_B' or  $_GET['ani_groupname']=='TT_Si')///////橫的列印 藥品上到下
{
///////////////////////////////////////////////////////////////////////////////////////////////////////	
	if($_GET['ani_groupname']=='M21')
	{
		if($_GET['pdd_prod_no']=='DLICP-A75')
		{$i1=5;$i2=24;}
		else{$i1=5;$i2=25;}
	}
	elseif($_GET['ani_groupname']=='M13')
	{
		$i1=5;$i2=17;
	}
	elseif($_GET['ani_groupname']=='TT_B' or $_GET['ani_groupname']=='TT_Si')
	{
		$i1=8;$i2=8;
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	$PHPExcel = $reader->load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
...
//			echo "<BR>";
	...
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	
	$objPHPExcel->getActiveSheet()->setCellValue("A2","DATE  ".date("Y/m/d") );
	
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		$query1="AnalyzeTime='".$_POST['0']."' ";
		for($i=1;$i<$chkbox;$i++)
		{
			if($_POST[$i]<>'')
			{
		 		$query1=$query1."or AnalyzeTime='".$_POST[$i]."' ";
			}
		}
		$query1.=" )  order by   AnalyzeTime";
		$query=$query.$query1;
	//echo $query;
	$eng='B';//312
	$result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			...
			{		$s=$i+$i1;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$s,$row[$a1[$i]]);

			}
			$eng++;
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
				
					$objPHPExcel->getActiveSheet()->setCellValue("A3",iconv("big5","utf-8","分析師: ".$row['Tester'] ));

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////				
		}
}
if($_GET['ani_groupname']=='NH4' or $_GET['ani_groupname']=='IC' or $_GET['ani_groupname']=='F-' or $_GET['ani_groupname']=='ICGA')//特別列印
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		$num2='8';
		while($row = mssql_fetch_array($result))
		{$eng='C';$times=substr($row['AnalyzeTime'],8,4);
	$objPHPExcel->getActiveSheet()->setCellValue("A2","DATE  ".substr($row['AnalyzeTime'],0,8) );

				if($_GET['ani_groupname']=='NH4')
				{
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['value1']);$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$times);
					
				}
				if($_GET['ani_groupname']=='IC')
				{
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['Cl-H2SO4']);$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$times);
				}
				if($_GET['ani_groupname']=='F-')
				{
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['F-']);$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$times);

				}
				if($_GET['ani_groupname']=='ICGA')
				{
					if($num2==18){
						$objPHPExcel->getActiveSheet()->setCellValue('C20',$row['Acetate']);
						$objPHPExcel->getActiveSheet()->setCellValue('D20',$row['Formate']);
						$objPHPExcel->getActiveSheet()->setCellValue('E20',$row['Propionate']);
						}
					elseif($num2<=17){
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num2,$row['Acetate']);$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num2,$row['Formate']);$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num2,$row['Propionate']);$eng++;
					}
...
if($_GET['ani_groupname']=='I_CL' or $_GET['ani_groupname']=='I'  and $_GET['pdd_chemical']!='H2SO4' )//特別列印
{
...
//		echo "../FormList/PrintAnalyze/RCIC/".$_GET['pid']."/".$_POST['group'].".xlsx<br>";
		...
		for ($i=$engs;$i<=$engd;$i++) 
		{
			$val = $sheet->getCellByColumnAndRow($i,6)->getValue();
			$val = iconv("utf-8","big5",$val);
//			echo $val;
//			echo "<BR>";
			array_push($a1,$val);
...
if($_GET['ani_groupname']=='ACT' or $_GET['ani_groupname']=='N-P')//特別列印
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		$result = mssql_query($query);
		$num1='6';	
		if($_GET['efm']=='TLIQA9100309')
		{
			while($row = mssql_fetch_array($result))
			{	
					$eng='C';
					$ACT=$row['ACT'];
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$ACT);
					$eng++;
					$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue('E'.$num1,iconv("big5","utf-8",$row['Tester']));
					$num1++;
			}
				$query="select * from TLIQA9100308 where lotno='".$_GET['lot_no']."' order by   AnalyzeTime ";
				$result = mssql_query($query);
				$num1='6';
				while($row = mssql_fetch_array($result))
				{
						$eng='D';
						$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['NPA']);
						$num1++;				
				}
		}
		if($_GET['efm']=='TLIQA9100308')
		{
			while($row = mssql_fetch_array($result))
			{	
					$eng='D';
			...
			$query="select * from TLIQA9100309 where lotno='".$_GET['lot_no']."' order by   AnalyzeTime ";
			$result = mssql_query($query);
			$num1='6';
			while($row = mssql_fetch_array($result))
			{
					$eng='C';
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['ACT']);
					$num1++;
...
if($_GET['ani_groupname']=='TOC' or $_GET['ani_groupname']=='TOC_H2SO4')//特別列印
{
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		while($row = mssql_fetch_array($result))
		{$eng='C';
				if($_GET['ani_groupname']=='TOC')
				{
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['TC']);
					$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num1,$row['IC']);
				}
				if($_GET['ani_groupname']=='TOC_H2SO4')
				{
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num2,$row['UPW_H2SO4']);
					$eng++;
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$num2,$row['H2SO4']);
...
if($_GET['pdd_prod_no']=='DLUV-PO4')
{
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		
		{
					...
					$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['PO4']);
					$i++;
					$a=$row['Tester'];
					$b=$row['AnalyzeTime'];
					$c=$row['LotNo'];
			...
			$objPHPExcel->getActiveSheet()->setCellValue("E5",iconv("big5","utf-8",$a));
			$objPHPExcel->getActiveSheet()->setCellValue("C8",iconv("big5","utf-8",$c));
			$objPHPExcel->getActiveSheet()->setCellValue("C5",iconv("big5","utf-8",date("Y.m.d",time($b))));
		}		
		if($_GET['pdd_prod_no']=='DLUV-NO3')
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		{
			$eng='E';
			$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Result']);
					$eng++;
					$eng++;
					$Result=$row['Result'];
			$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$Result*1000);
					$i++;
					$a=$row['Tester'];
					$b=$row['AnalyzeTime'];
					$c=$row['LotNo'];
		...
		$objPHPExcel->getActiveSheet()->setCellValue("H2",iconv("big5","utf-8",$a));
		$objPHPExcel->getActiveSheet()->setCellValue("B3",iconv("big5","utf-8",$c));
		$objPHPExcel->getActiveSheet()->setCellValue("A2",iconv("big5","utf-8","分析日期：".date("Y.m.d",time($b))));


}
if($_GET['pdd_prod_no']=='DLUV-NH4')
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	...
	$query="select top 16 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		...
		while($row = mssql_fetch_array($result))
		{
			if($eng<"K"){
				$i='6';
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Light0']);
				$i++;
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Light1']);
				$i++;
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Light2']);
				$i++;
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Level']);
				$i++;
				$objPHPExcel->getActiveSheet()->setCellValue($eng.$i,$row['Value0']);
				$eng++;
			}
			elseif($eng=="K"){
				$objPHPExcel->getActiveSheet()->setCellValue("C14",$row['Level']);
				$objPHPExcel->getActiveSheet()->setCellValue("C15",$row['Value0']);	
				$objPHPExcel->getActiveSheet()->setCellValue("C16",$_GET['lot_no']);	
			}
			$a=$row['Tester'];
			$b=$row['AnalyzeTime'];
		}
		$objPHPExcel->getActiveSheet()->setCellValue("E2",iconv("big5","utf-8","分析師：".$a));
		$objPHPExcel->getActiveSheet()->setCellValue("B2",iconv("big5","utf-8","分析日期：".date("Y.m.d",time($b))));
		
		
}
if($_GET['pdd_prod_no']=='DLIC-HPIB200' and $_GET['ani_groupname']=='I')
{
	$objPHPExcel = new PHPExcel();	
	$objPHPExcel = PHPExcel_IOFactory::load("../FormList/PrintAnalyze/TDL/".$_GET['pdd_prod_no']."/".$_GET['ani_groupname'].".xlsx");
	$objPHPExcel->setActiveSheetIndex(0);
	
	$query="select top 10 * from ".$_GET['efm']." where lotno='".$_GET['lot_no']."' and (";
		$query1="AnalyzeTime='".$_POST['0']."' ";
		for($i=1;$i<$chkbox;$i++)
		{
			if($_POST[$i]<>'')
		...
		$query1.=" )  order by   SerialNo,AnalyzeTime";
		$query=$query.$query1;
//		echo $query;
		...
		while($row = mssql_fetch_array($result))
		{
				$objPHPExcel->getActiveSheet()->setCellValue("C".$i,$row['CL']);

				$objPHPExcel->getActiveSheet()->setCellValue("D".$i,$row['NO3']);

				$objPHPExcel->getActiveSheet()->setCellValue("E".$i,$row['PO4']);

				$objPHPExcel->getActiveSheet()->setCellValue("F".$i,$row['SO4']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue("B3",iconv("big5","utf-8","".$a));
		$objPHPExcel->getActiveSheet()->setCellValue("B2",iconv("big5","utf-8",date("Y.m.d",time($b))));
