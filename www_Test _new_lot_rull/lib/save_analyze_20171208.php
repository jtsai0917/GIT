...
$loginFormAction = $_SERVER['PHP_SELF'];
	...
	$pid=$_GET['pid'];
	$cid=$_GET['cid'];
if($pid=='P001-200' || $pid=='P002-190' || $pid=='P006-330')
	{$dun=1;}else{$dun=0;}
	...
	if($numRows<1){
	 ...
	 if($dun==1){
		$query="INSERT INTO dbo.AnalyzeDesign
                            ...
                            AND_OUT_DATETIME, AND_REPORT_DATETIME, AND_PERSON, AND_SELECT_TYPE, AND_ITEM_ORI)
	  VALUES          ('".$cid."','','','',0,'".'0'."','','','".date("Ymd")."',1,'".$_GET['lot_no']."','".$_GET['pid']."','N','".$_GET['datepicker1']."','".
		Get_Regular_item($pid,$cid,$test_items)."','',".$_GET['smp_cnt'].",0,'','".$_GET['out_date']."','".$_SESSION['uid']."','".$_GET['item_style']."','".$_SESSION['item_ori']."')";
		$result = mssql_query($query);
