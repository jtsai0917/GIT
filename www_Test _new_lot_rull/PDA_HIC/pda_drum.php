...
//  echo $chksql;
	...
	while($rowsql=mssql_fetch_array($resultsql)){
		...
		$_SESSION['pidcname']=$rowsql['PDD_PROD_NAME'].":".$rowsql['PDD_PROD_NO'];
...
if((isset($_POST['next1'])) and $_POST['chk1']==1 and $_POST['chk2']==2 and $_POST['chk3']==3) 
{	
	/*
	$query="SELECT * FROM OUT_CHECK_DRUM WHERE (OTD_NO = '".$_SESSION['nono']."')";
	$rst=mssql_query($query);
	$numx=mssql_num_rows($rst);
	if($numx > 0){
		my_msg("作業已完成","product_out.php");
	}
	*/
	$query="SELECT    CUSTOMER_PRODUCTS.CTP_CUSTBAR1, CUSTOMER_PRODUCTS.CTP_CUSTBAR1_CHECK, OP.OPD_LOT_NO, OP.OPD_QTY_DRUM 
	FROM              OUT_PRODUCT AS OP INNER JOIN
	...
	WHERE          (OP.OPD_LOT_NO = '".trim($_POST['lot_id'])."') AND (OP.OTD_NO = '".$_SESSION['nono']."')";
	...
	$QTY1=$_SESSION['QTY'];
	echo "LOT NO:".'<input type="text" name="no" autocomplete="off" id="no" style="font-size:20px" value="'.$_SESSION['aa'][0].'"/></br>';
...
WHERE          (OUT_CHECK_DRUM_DETAIL.OTD_NO = '".$_SESSION['nono']."' and OUT_CHECK_DRUM_DETAIL.OCD_LOT_NO='".$_POST['lot_id']."')";

	$result=mssql_query($query);
	$all=mssql_num_rows($result);
//	echo $all.":".$numRows1."<BR>";
	/*
	if($all=$numRows1){
		my_msg("作業已完成","product_out.php");
	}
...
//	echo "SSS";
	...
	if($_POST['check_custbar1']==1){
			...
			$longString= trim($_POST['cust_prod_no']);
			  $shortString= trim($_SESSION['custbar1']);
			if (strpos($longString, $shortString) !== false) {
...
//				echo "<BR>".$query."<BR>";
		 ...
		 if($BC==2)
		 {
			 $bord2="select * from OUT_CHECK_DRUM_DETAIL where OTD_NO='".$_SESSION['nono']."' and OCD_PLT_NO='".trim($_SESSION['pa_no'])."' ";
...
FROM              OUT_PRODUCT AS OP INNER JOIN
			 ...
			 where OD.OTD_NO='".$_SESSION['nono']."' and REPLACE(OP.OPD_LOT_NO,' ','')='".trim($_POST['no'])."'";
//			echo "<BR>".$query2."<BR>";
			...
			$numRows2 = mssql_num_rows($result2);
			 while($row2 = mssql_fetch_array($result2))
			 {
				if(trim($_POST['no1'])<>'' and trim($_SESSION['pa_no'])<>''){
					...
					VALUES ('".trim($_SESSION['nono'])."','".trim($_POST['no'])."','".trim($_POST['no1'])."','".trim($_SESSION['pa_no'])."')";
			//		echo $inser."<BR>";
		...
		//			$_SESSION['code2']=$up1;
					...
					echo "LOT NO:".'<input type="text" autocomplete="off" size="10" name="no" id="no" style="font-size:20px" value="'.$_SESSION['aa'][0].'"/><br>';
					$query3="SELECT  OUT_PRODUCT.OPD_QTY_DRUM
				FROM      OUT_CHECK_DRUM_DETAIL INNER JOIN
								   OUT_PRODUCT ON OUT_CHECK_DRUM_DETAIL.OTD_NO = OUT_PRODUCT.OTD_NO AND 
								   OUT_CHECK_DRUM_DETAIL.OCD_LOT_NO = OUT_PRODUCT.OPD_LOT_NO
				WHERE   (OUT_CHECK_DRUM_DETAIL.OTD_NO = '".trim($_POST['nono'])."') AND (OUT_CHECK_DRUM_DETAIL.OCD_LOT_NO = '".trim($_POST['no'])."')";
//				echo $query3."<BR>";
					...
					if($row3 >= $row4[0]){
						...
						echo '<br>第'. ($row3+1).'/'.$row4[0].'桶';
						echo '</br>';
						if($_SESSION['check_custbar1']==1){
							echo "客戶料號:".'<input type="text" name="cust_prod_no" autocomplete="off" id="cust_prod_no" style="font-size:20px" value="" autofocus="autofocus" /></br>';
						}
...
//	jumpto("index.php");
	...
	echo '<BR /><input type="submit" name="enter" style="font-size:20px" id="enter" value="送出" /><br>';
