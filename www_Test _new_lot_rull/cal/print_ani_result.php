...
$query1="SELECT          TOP (1) Ani_Signatory.Signatory_Name,PRODUCT_DATA.PDD_CLASS
FROM              PRODUCT_DATA INNER JOIN
                            AnalyzeDesign ON PRODUCT_DATA.PDD_PROD_NO = AnalyzeDesign.AND_GOODS RIGHT OUTER JOIN
                            Ani_Signatory ON AnalyzeDesign.AND_LOT_NO = Ani_Signatory.LotNo
         where LotNo='".$_GET['lot_no']."' and cancel=0 order by sign_datetime desc";
// echo $query1;
...
if (isset($_POST["print_ACID"])) 
{
	...
	$document->setValue("signer0", $signer0);
...
if (isset($_POST["print_TT"])) 
{
	...
	//print_r($TT1);
	$query="SELECT          TOP (1) AnalyzeDesign.* from AnalyzeDesign where and_lot_no='".$_GET['lot_no']."' ";
//	echo $query;
	...
	$document->setValue("signer0", $signer0);
	$document->setValue("pdd_class0", $pdd_class);
