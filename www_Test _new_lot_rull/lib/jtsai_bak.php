	...
	function read()
	{
		...
		{	  ///read excel2007	
				...
				for($x=0;$x<4;$x++)
				{
    				for ($c = 0; $c < $highestColumns; $c++) 
						{ // 0 = A欄
							...
							if($v=='Sample Name')
								{
									$_SESSION['smpno']=brackets(trim($sheet->getCellByColumnAndRow($c,$x+1)->getValue()));
									$_SESSION['get_lot_no']=brackets_1(trim($sheet->getCellByColumnAndRow($c,$x+1)->getValue()));
    		...
    		else
			{
				...
				if($_GET['ani_group']){$query.=" AND (ani_group = '".$this->group."') ";}	
				$result=mssql_query($query);
				$numrows=mssql_num_rows($result);
				if($numrows>0)
				{	
				...
				$_SESSION['ssp']=$ssp=substr($this->value,0,1);			
				if($ssp=="=")
				{
