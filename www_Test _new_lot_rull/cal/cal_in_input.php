...
if(isset($_POST['excel']))
{
	...
	echo '<script>document.location.href="http://'.$_SERVER['HTTP_HOST'].'/cal/tmp1.xlsx";</script>';
}
if($_POST['search']){
...
$objPHPExcel = PHPExcel_IOFactory::load("../cal/tmp.xlsx");
...
WHERE          (AnalyzeDesign.AND_NEED_NO>=270 and AnalyzeDesign.AND_NEED_NO < 500) ";
if ($_GET['coa']==1){
	$query=$query. "AND (AnalyzeDesign.AND_TYPE=1) ";
}
else {
	$query=$query. " AND (AnalyzeDesign.AND_TYPE IS NULL) ";
}
...
<table width="1280" border="1">
  <tr bgcolor="#CCCCCC">
    <td height="49" width="76">日期序號</td>
    <td width="84">品名</td>
    <td width="29">容器</td>
    <td width="67">LotNO</td>
	<td width="28">定常</td>
    <td width="95">要求完成時間</td>
	<td width="200">充填路徑</td>
	<td width="76">受入<BR>開始/結束<BR>時間<BR>(08:30)</td>
	<td width="76">受入數量<BR>m3</td>
	<td width="76">受入路徑</td>
    <td width="400"><p>分析項目:<font color="#0000FF">藍字</font>表示已有報告及結果，<font color="#FF0000">紅字</font>表示沒有報告及結果，<font color="green">綠字</font>表示有報告但是無檢驗結果，</p>
    <p><font color="purple">紫色</font>表示沒有報告但是有檢驗結果‧<span style="background-color: red"><font color="white">紅底白字</font></span>表示檢驗未通過‧<p></td>
  		<td width="131">簽核<BR><input type="submit" name="sign_selected" value="簽核(勾選)" '.$disable2.'><br><BR><input type="submit" name="urgent" value="急件確認" '.$disable1.'></td>
  </tr>';
$NUM=2; 
...
$diff->status();
	$note=$row['AND_NOTE'];
	$rpd=ddd($row['AND_REPORT_DATETIME']);
	...
	$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row['PDD_PROD_NAME']));
	$objPHPExcel->getActiveSheet()->getStyle($ENG.$NUM)->applyFromArray($styleThinBlackBorderOutline);
 	$ENG++;
    echo '<td>'.$row['PDD_TYPE'].'</td>';
    echo '<td><a href="/cal/index.php?url=QSELMF3017-001A4&lotno='.$row['AND_LOT_NO'].'" target="_new" title="">'.$row['AND_LOT_NO'].'</a></td>';
	$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$row['AND_LOT_NO']));
	$objPHPExcel->getActiveSheet()->getStyle($ENG.$NUM)->applyFromArray($styleThinBlackBorderOutline);
	$ENG++;
	...
	$A=fill_flow($row['AND_LOT_NO']);
	 $D=fill_in_flow($row['AND_LOT_NO']);
	echo '<td>'.$A[0].'</td>';
	$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, iconv("big5","utf-8",$A[1]));
	$objPHPExcel->getActiveSheet()->getStyle($ENG.$NUM)->applyFromArray($styleThinBlackBorderOutline);
	$ENG++;
	$atid=QSELMF3017($row['AND_LOT_NO']);
	...
	$fill_ani=$_SESSION['fill_ani'];
	$objPHPExcel->getActiveSheet()->setCellValue($ENG.$NUM, $fill_ani);
	$objPHPExcel->getActiveSheet()->getStyle($ENG.$NUM)->applyFromArray($styleThinBlackBorderOutline);
	$NUM++;
	echo '<td><input type="checkbox" name="chkbox[]" value="'.$row['AND_LOT_NO'].'">';
	 check_status(trim($row['AND_LOT_NO']),$a2);
...
function fill_flow($lotno){
	$query="SELECT	Fill_Flow_Chart.flow FROM Fill_Flow_Chart WHERE (Lot_No = N'".$lotno."' and fill_in is NULL) order by date desc";
//	echo $query."<BR>";
...
function fill_in_flow($lotno){
	$query="SELECT	Fill_Flow_Chart.flow FROM Fill_Flow_Chart WHERE (Lot_No = N'".$lotno."' and fill_in=1) order by date desc";
	$url="edit_flow.php?fill_in=1&lot_no=".$lotno;
	$result=mssql_query($query);
	$row=mssql_fetch_row($result);
	if(trim($row[0])==''){$a='編輯';}else{$a=$row[0];}
	$A='<a target="_blank" href="'.$url.'">'.$a.'</a>;'.$a;
...
function QSELMF3017($lotno){
	$queryd="SELECT top(1) t2, a3, a4, hb, ctrl_A, ctrl_B FROM QSELMF3017 where Lot_No='".$lotno."' ORDER BY   [index] DESC";
	$resultd=mssql_query($queryd);
	$rowd=mssql_fetch_row($resultd);
	return $rowd[0].','.trim($rowd[1]).",".$rowd[3].",".$rowd[4].",".$rowd[5];
}
