...
class tble{
public $tbname;
public $lot_no,$shortlot,$signer0;
public $pdd_chemical,$pdd_prod_name,$pdd_prod_short_name;
public $ani_groupname,$ps,$prod_no;
...
echo $this->tbname."</br>";
$query="SELECT DISTINCT *, CONVERT(char(10), TestDate, 20) AS TestDate
FROM             dbo.".$this->tbname."
WHERE          (LotNo  = '".$this->lot_no."') order by AnalyzeTime ";
...
{	
	...
	elseif($this->prod_no=='TP00701-70' and $this->ani_groupname=="A"){$tname='TLNQA9100301F';}
	elseif(($this->prod_no=='IQ01-004' or $this->prod_no=='IQ01-005' or $this->prod_no=='IQ01-008') and $_SESSION['anigroup']=="I"){$tname='TLOQA9100304A1';}
	elseif(($this->prod_no=='IQ01-011' or $this->prod_no=='IQ01-012') and $_SESSION['anigroup']=="PH"){$tname='TLUQA9100345_ICE';}
	else
	{
		$tname=$this->tbname;	
	}
	echo $this->pdd_type.','.$type.','.$this->pdd_prod_short_name.','.$this->pdd_prod_name;
	$document = $PHPWord->loadTemplate('../FormList/PrintAnalyze/'.$this->pdd_chemical.'/'.$tname.'.docx');
	$type=$this->pdd_type;
	$document->setValue("type", $this->pdd_type);
	$document->setValue("signer0", $this->signer0);
				...
				if(($value<>'1.0E-6') and ($value<>'0.000001') and ($value<>'')  and ($value<>'NULL'))
					{
						...
						{
							$value=$value;}else{$value=round($value,4);
						}   //  (不須四捨五入)
					}
				if($value=='0'){$value='0';}
...
{	
	...
	$shortlot=substr($ss->lot_no,0,6);
	$lastlot=substr($ss->lot_no,-4);
	if($lastlot=='A001'){$samplename='中間桶';$spec='>3.2';}
	if($lastlot=='B001'){$samplename='T-1706';$spec='>3.2';}
...
WHERE       (QC_CustProdSpec.CustNo = '".$_SESSION['select_cust']."') AND (QC_CustProdSpec.ProdNo = '".$_GET['pdd_prod_no']."') AND 
                            (AnalyzeItem.ANI_GROUPNAME = 'M13' OR AnalyzeItem.ANI_GROUPNAME = 'TT_Si' OR AnalyzeItem.ANI_GROUPNAME = 'M21')";
//echo $query;
$result=mssql_query($query);
$tt=0;
while($row=mssql_fetch_array($result)){
	$_SESSION['cspec'][$row['ANI_DATAFIELD']]=$row;
//	echo $row['ANI_DATAFIELD'].":".$cspec[$row['ANI_DATAFIELD']]['ItemName']."<BR>";	
}
function spec($table,$column)
{
	$query="SELECT         [".$column."]
FROM              ".$table." 
...
//echo $query.'<br>';
	...
	//	 		list ($spec,$dl,$usl,$stdu,$ps,$stdl,$lsl)=get_test_spec($column_name,$_GET['pdd_prod_no'],$_SESSION['select_cust'],$_GET['ani_groupname']);
				//echo $_SESSION['spec'][$column_name].'<br>';
				$spec=$_SESSION['cspec'][$column_name]['ANI_DATAFIELD'];
				$spec=$_SESSION['cspec'][$column_name]['SpecNo'];
				$$dl=$_SESSION['cspec'][$column_name]['DL'];
				$usl=$_SESSION['cspec'][$column_name]['USL'];  //客規
				$stdu=$_SESSION['cspec'][$column_name]['STD_U'];  //再分析標準
				$lsl=$_SESSION['cspec'][$column_name]['LSL'];
				$ps=$_SESSION['cspec'][$column_name]['ItemUnit'];
				$stdl=$_SESSION['cspec'][$column_name]['STD_L'];
				$showmode=$_SESSION['cspec'][$column_name]['SM'];
				//echo $$dl.','.$usl.','.$stdu.'<br>';		
		//		if($column_name=='PP01'){$_SESSION['ss']=array($spec,$dl,$usl,$stdu,$ps,$stdl,$lsl);}		
				...
				*/
				//echo $showmode.','.$showmode.'<br>';
				if($showmode=='<DL'or $showmode=='<USL'){
					if($usl<>'' and $row[$i]>$usl){$spec='合  (否)';}
					elseif($usl<>'' and $row[$i]<$usl){$spec='(合)  否';}
					elseif($stdu<>'' and $row[$i]>$stdu){$spec='(合)  否';}
				}
				elseif($showmode=='>LSL'){
					if($lsl<>'' and $row[$i]<$lsl){$spec='合  (否)';}
					elseif($stdl<>'' and $row[$i]<$stdl){$spec='(合)  否';}
				}
				elseif($showmode=='VALUE'){
					if($row[$i]>$usl or $row[$i]<$lsl){$spec='合  (否)';}
					elseif(($row[$i]>$stdu and $row[$i]<$usl) or ($row[$i]<$stdl and $row[$i]>$lsl)){$spec='(合)  否';}
				}
			}	
			return $spec;
	}
...
$fileurl="../tmp1/TI".$this->lot_no;
$m1=$A[0]=get_table("M13",$this->pdd_chemical);
$m2=$A[1]=get_table("M21",$this->pdd_chemical);
$m3=$A[2]=get_table("TT_Si",$this->pdd_chemical);
$m4=$A[3]=get_table("TT_B",$this->pdd_chemical);
$m5=$A[4]=get_table("TT",$this->pdd_chemical);
$document->setValue('pdd_prod_name',get_pdd_name($_GET['pdd_prod_no']));
echo $this->tbname."</br>";
for($i=0;$i<5;$i++)////帶入工程名稱,作業指示者,測定日期
{
	$query1="SELECT 
                          Operator
FROM             dbo.".$A[$i]." 
WHERE         ((dbo.".$A[$i].".LotNo = '".$_GET['lot_no']."') ) ORDER BY  AnalyzeTime DESC";
//echo $query1.'<br>';
	$result1 = mssql_query($query1);
	...
	if($numRows1>0)
	{
		...
		{
			$value=trim($row[0]);
			$document->setValue('Operator0',$value);
		}
	}
	$query="SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$A[$i]."') and (COLUMN_NAME='TestDate' OR COLUMN_NAME='AnaManager' OR COLUMN_NAME='AnalyzeTime' OR COLUMN_NAME='CHK1' OR COLUMN_NAME='CHK2' OR COLUMN_NAME='CHK3' OR COLUMN_NAME='CHK4')";
	//echo $query.'<br>';
	$result = mssql_query($query);
	$numRows=mssql_num_rows($result);
	while($row=mssql_fetch_array($result))
	{
		if($numRows>0 and $ROW=='')
		{
			if((trim($row[0])=="CHK1") or (trim($row[0])=="CHK2") or (trim($row[0])=="CHK3") or (trim($row[0])=="CHK4")){$value="V";}
			if(trim($row[0])=='TestDate'){$value="西曆 ".substr($_GET['AnalyzeTime'],0,4)." 年 ".substr($_GET['AnalyzeTime'],5,2)." 月 ".substr($_GET['AnalyzeTime'],8,2)." 日";}	
			if(trim($row[0])=='AnalyzeTime'){$value="西曆 ".substr($_GET['AnalyzeTime'],0,4)." 年 ".substr($_GET['AnalyzeTime'],4,2)." 月 ".substr($_GET['AnalyzeTime'],6,2)." 日";}
		}
		else{continue;}
		$document->setValue(trim($row[0]).'0',$value);
	}
	if($numRows>0){$ROW=1;}
}
$fitch=" and COLUMN_NAME<>'Operator' and COLUMN_NAME<>'AnaManger' and COLUMN_NAME<>'AnalyzeTime'";
$query="SELECT DISTINCT 
                          dbo.".$m1.".*
FROM             dbo.".$m1." 
...
{	
	...
	while($row = mssql_fetch_array($result))
	{
		$ss=new analyze_first;
		$ss->lot_no=$this->lot_no;
		$ss->create_time=trim($row['AnalyzeTime']);
		$ss->get();
		$document->setValue("first0",get_uname($ss->first));
		...
		while($row1 = mssql_fetch_array($result1)){
			...
			$rowname1=$rowname;
			$spec=spec($m1,$rowname1);	
			//echo $m1.','.$rowname1.','.$spec.'<br>';
			if($value<>'' and $value<>'NULL'){
				$document->setValue($rowname.$i,$value);
				$document->setValue($rowname.'7',$spec);
				}	
			elseif($value=='NULL'){$document->setValue($rowname.$i,$value);$document->setValue($rowname.'7','合  否');}	
		}
		$i++;
	}
}
...
{	
	$ss=new analyze_first;
	$ss->lot_no=$this->lot_no;
	$ss->create_time=$_GET['AnalyzeTime'];
	$ss->get();
	$i=0;
		if(trim($row['Tester'])<>''){$document->setValue("first2",get_uname($ss->first));$document->setValue("first0",get_uname($ss->first));}else{$document->setValue("first0",'');$document->setValue("first2",'');}
		$document->setValue("Tester0",$row['Tester']);
		$document->setValue("Tester2",$row['Tester']);
		$query1="SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$m1."')".$fitch;
		$result1 = mssql_query($query1);
		while($row1 = mssql_fetch_array($result1)){
			$rowname=$row1['COLUMN_NAME'];
			$value='';
			...
			$document->setValue($rowname.'7','合  否');
		}
		$i++;
}
...
{	
	...
	while($row = mssql_fetch_array($result))
	{
		...
		while($row1 = mssql_fetch_array($result1)){
			...
			$rowname1=$rowname;
			if ($rowname=="As"){$rowname="AAS";}
			$spec=spec($m2,$rowname1);
			//echo $rowname1.','.$value.','.$spec.'<br>';
			if($rowname1<>'Si'){
			if($value<>'' and $value<>'NULL'){
				$document->setValue($rowname.$i,$value);
				$document->setValue($rowname.'7',$spec);
...
{	
	...
	$ss->get();
	$i=0;
		if(trim($row['Tester'])<>''){$document->setValue("first1",get_uname($ss->first));}else{$document->setValue("first1",'');}
		$document->setValue("Tester1",$row['Tester']);
		$query1="SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$m2."')".$fitch;
		$result1 = mssql_query($query1);
		while($row1 = mssql_fetch_array($result1)){
			$rowname=$row1['COLUMN_NAME'];
			$value='';
...
{	
	...
	$ss->get();
	$i=0;
		if(trim($row['Tester'])<>''){$document->setValue("first3",get_uname($ss->first));}else{$document->setValue("first3",'');}
		$document->setValue("Tester3",$row['Tester']);
		$query1="SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$m3."')".$fitch;
		$result1 = mssql_query($query1);
		while($row1 = mssql_fetch_array($result1)){
			$rowname=$row1['COLUMN_NAME'];
			$value='';
...
{	
	...
	$ss->get();
	$i=0;
		if(trim($row['Tester'])<>''){$document->setValue("first4",get_uname($ss->first));}else{$document->setValue("first4",'');}
		$document->setValue("Tester4",$row['Tester']);
		$query1="SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$m4."')".$fitch;
		$result1 = mssql_query($query1);
		while($row1 = mssql_fetch_array($result1)){
			$rowname=$row1['COLUMN_NAME'];
			$value='';
...
{	
	...
	$ss->get();
	$i=0;
		if(trim($row['Tester'])<>''){$document->setValue("first2",get_uname($ss->first));}else{$document->setValue("first2",'');}
		$document->setValue("Tester2",$row['Tester']);
		$query1="SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE (TABLE_NAME ='".$m5."')".$fitch;
		$result1 = mssql_query($query1);
		while($row1 = mssql_fetch_array($result1)){
			$rowname=$row1['COLUMN_NAME'];
			$value='';
...
function prt_3_h2so4(){
	...
	/// Assay
	$tb=get_table_A("A",$this->pdd_chemical,$this->lot_no);
	$query="SELECT DISTINCT 
...
WHERE          (dbo.Sample_All.SMA_LOT = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select0."')  ORDER BY AnalyzeTime";
...
FROM             Sample_All INNER JOIN
                          TLIQA9100320 ON Sample_All.SMA_LOT = TLIQA9100320.LotNo AND 
                          Sample_All.SMA_ID = TLIQA9100320.SampleNo
WHERE          (dbo.Sample_All.SMA_LOT = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select1."')  ORDER BY AnalyzeTime";
...
FROM             Sample_All INNER JOIN
                          TLIQA9100320 ON Sample_All.SMA_LOT = TLIQA9100320.LotNo AND 
                          Sample_All.SMA_ID = TLIQA9100320.SampleNo
WHERE          (dbo.Sample_All.SMA_LOT = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select2."') ORDER BY AnalyzeTime";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
while($row = mssql_fetch_array($result)){
	$n13=$row['SampleNo'];
...
FROM              ".$tb." INNER JOIN
                            Sample_All ON ".$tb.".LotNo = Sample_All.SMA_LOT AND 
                            ".$tb.".SampleNo = Sample_All.SMA_ID
WHERE          (dbo.".$tb.".LotNo = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select0."' ORDER BY AnalyzeTime)
ORDER BY   dbo.".$tb.".AnalyzeTime";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
//echo $query."</br>";
...
$numRows = mssql_num_rows($result);
while($row = mssql_fetch_array($result)){
	$k31=$row['PP05'];if($k31=='0'){$k31=conv("─");} ///(>0.5um)
	$k32=$row['PP03'];if($k32=='0'){$k32=conv("─");} ///(>0.3um)
	$k33=$row['PP02'];if($k33=='0'){$k33=conv("─");} ///(>0.2um)
	$k34=$row['PP01'];if($k34=='0'){$k34=conv("─");} ///(>0.1um)
	$k30=$row['SMA_ID']; ///取樣瓶號
	$tester13=$row['Tester'];
	$tt13=substr($row['AnalyzeTime'],0,-6);
}
...
FROM              ".$tb." INNER JOIN
                            Sample_All ON ".$tb.".LotNo = Sample_All.SMA_LOT AND 
                            ".$tb.".SampleNo = Sample_All.SMA_ID
WHERE          (dbo.".$tb.".LotNo = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select1."' ORDER BY AnalyzeTime)
ORDER BY   dbo.".$tb.".AnalyzeTime";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
while($row = mssql_fetch_array($result)){
...
$numRows = mssql_num_rows($result);
while($row = mssql_fetch_array($result)){
	$l31=$row['PP05'];if($l31=='0'){$l31=conv("─");} ///>0.5
	$l32=$row['PP03'];if($l32=='0'){$l32=conv("─");} ///>0.3
	$l33=$row['PP02'];if($l33=='0'){$l33=conv("─");} ///>0.2
	$l34=$row['PP01'];if($l34=='0'){$l34=conv("─");} ///>0.1
	$l30=$row['SMA_ID'];
	$tester23=$row['Tester'];
	$tt23=substr($row['AnalyzeTime'],0,-6);
}
...
FROM              ".$tb." INNER JOIN
                            Sample_All ON ".$tb.".LotNo = Sample_All.SMA_LOT AND 
                            ".$tb.".SampleNo = Sample_All.SMA_ID
WHERE          (dbo.".$tb.".LotNo = '".$this->lot_no."') AND (dbo.Sample_All.SMA_DRUMNO = '".$this->select2."' ORDER BY AnalyzeTime)
ORDER BY   dbo.".$tb.".AnalyzeTime";
$result = mssql_query($query);
$numRows = mssql_num_rows($result);
//echo $query."</br>";
...
$numRows = mssql_num_rows($result);
while($row = mssql_fetch_array($result)){
	$m31=$row['PP05'];if($m31=='0'){$m31=conv("─");}
	$m32=$row['PP03'];if($m32=='0'){$m32=conv("─");}
	$m33=$row['PP02'];if($m33=='0'){$m33=conv("─");}
	$m34=$row['PP01'];if($m34=='0'){$m34=conv("─");}
	$m30=$row['SMA_ID'];
	$tester33=$row['Tester'];
	$tt33=substr($row['AnalyzeTime'],0,-6);
}
...
$q31=$tt13."\n".$tt23."\n".$tt33."\n".$tt43;
	...
	$objPHPExcel->setActiveSheetIndex($this->activesheet);
	$objPHPExcel->setActiveSheetIndex($this->activesheet)
 							->setCellValue("c7",iconv('big5','utf-8',$this->lot_no))
	    					->setCellValue("j7",conv($this->type))
	  						->setCellValue("B9",conv($b7))
							->setCellValue("o10",conv($s8))
							->setCellValue("B10",conv($b8))
							->setCellValue("p8",conv($v6))
							->setCellValue('k15',$k15)
							->setCellValue('k16',$k16)
							->setCellValue("k17",$k17)
							->setCellValue("k18",$k18)
							->setCellValue("k19",$k19)
							->setCellValue("k20",$k20)
							->setCellValue("k21",$k21)
							->setCellValue("k22",$k22)
							->setCellValue("k23",$k23)
							->setCellValue("k24",$k24)
							->setCellValue("k25",$k25)
							->setCellValue("k26",$k26)
							->setCellValue("k27",$k27)
							->setCellValue("k28",$k28)
							->setCellValue("k29",$k29)
							->setCellValue("k30",$k30)
							->setCellValue("k31",$k31)
							->setCellValue("k32",$k32)
							->setCellValue("k33",$k33)
							->setCellValue("k34",$k34)
							->setCellValue("p17",conv($w17))
							->setCellValue("q17",$x17)
							->setCellValue("p31",conv($p31))
							->setCellValue("q31",$q31)
							...
							->setCellValue("l33",$l33)
							->setCellValue("l34",$l34)
							->setCellValue("p18",conv($w17))
							->setCellValue("q18",$x17)
							->setCellValue("p32",conv($p31))
							->setCellValue("q32",$q31)
							...
							->setCellValue("m33",$m33)
							->setCellValue("m34",$m34)
							->setCellValue("p19",conv($w17))
							->setCellValue("q19",$x17)
							->setCellValue("p33",conv($p31))
							->setCellValue("q33",$q31)
							
							->setCellValue("k14",$this->select0)
							->setCellValue("l14",$this->select1)
							->setCellValue("m14",$this->select2)
							;
	for($s=13;$s<35;$s++){
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'k".$s."'")->applyFromArray($styleThinBlackBorderOutline);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'l".$s."'")->applyFromArray($styleThinBlackBorderOutline);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'m".$s."'")->applyFromArray($styleThinBlackBorderOutline);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'p".$s."'")->applyFromArray($styleThinBlackBorderOutline);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'q".$s."'")->applyFromArray($styleThinBlackBorderOutline);
		
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'k".$s."'")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'l".$s."'")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'m".$s."'")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'p".$s."'")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'q".$s."'")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'k".$s."'")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'l".$s."'")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'m".$s."'")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'p".$s."'")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle("'q".$s."'")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	...
	$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle('p8')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	$objPHPExcel->setActiveSheetIndex($this->activesheet)->getStyle('p8')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel,"Excel2007");
	$objWriter->save($fileurl.".xlsx");
echo '<script>document.location.href="http://'.$path_root.'/tmp1/'.$fileurl.'.xlsx";</script>';	
...
function prt_3_hf(){
	...
	$tb=get_table_A("A",$this->pdd_chemical,$this->lot_no);
	list($l14,$l15,$tester11,$tt11)=location1($tb,$this->lot_no,$this->select0);
	$tb=get_table("M13",$this->pdd_chemical);
	list($l16,$tester12,$tt12,$l17,$l18,$l19,$l20,$l21,$l22,$l23,$l24,$l25,$l26,$l27,$l28,$l29,$l30,$l31,$l32)=location2($tb,$this->lot_no,$this->select0);
	$tb=get_table("P",$this->pdd_chemical);
	list($l33,$tester13,$tt13,$l34,$l35,$l36,$l37)=location3($tb,$this->lot_no,$this->select0);
	$tb=get_table("TT_B",$this->pdd_chemical);
	list($x,$tester14,$tt14,$l32)=location_ttb($tb,$this->lot_no,$this->select0);
	
	$tb=get_table_A("A",$this->pdd_chemical,$this->lot_no);
	list($m14,$m15,$tester21,$tt21)=location1($tb,$this->lot_no,$this->select1);
	$tb=get_table("M13",$this->pdd_chemical);
	list($m16,$tester22,$tt22,$m17,$m18,$m19,$m20,$m21,$m22,$m23,$m24,$m25,$m26,$m27,$m28,$m29,$m30,$m31,$m32)=location2($tb,$this->lot_no,$this->select1);
	$tb=get_table("P",$this->pdd_chemical);
	list($m33,$tester23,$tt23,$m34,$m35,$m36,$m37)=location3($tb,$this->lot_no,$this->select1);
	$tb=get_table("TT_B",$this->pdd_chemical);
	list($x,$tester24,$tt24,$m32)=location_ttb($tb,$this->lot_no,$this->select1);
	
	$tb=get_table_A("A",$this->pdd_chemical,$this->lot_no);
	list($n14,$n15,$tester31,$tt31)=location1($tb,$this->lot_no,$this->select2);
	$tb=get_table("M13",$this->pdd_chemical);
	list($n16,$tester32,$tt32,$n17,$n18,$n19,$n20,$n21,$n22,$n23,$n24,$n25,$n26,$n27,$n28,$n29,$n30,$n31,$n32)=location2($tb,$this->lot_no,$this->select2);
	$tb=get_table("P",$this->pdd_chemical);
	list($n33,$tester33,$tt33,$n34,$n35,$n36,$n37)=location3($tb,$this->lot_no,$this->select2);
	$tb=get_table("TT_B",$this->pdd_chemical);
	list($x,$tester34,$tt34,$n32)=location_ttb($tb,$this->lot_no,$this->select2);
	
	$tb=get_table_A("A",$this->pdd_chemical,$this->lot_no);
	list($o14,$o15,$tester41,$tt41)=location1($tb,$this->lot_no,$this->select3);
	$tb=get_table("M13",$this->pdd_chemical);
	list($o16,$tester42,$tt42,$o17,$o18,$o19,$o20,$o21,$o22,$o23,$o24,$o25,$o26,$o27,$o28,$o29,$o30,$o31,$o32)=location2($tb,$this->lot_no,$this->select3);
	$tb=get_table("P",$this->pdd_chemical);
	list($o33,$tester43,$tt43,$o34,$o35,$o36,$o37)=location3($tb,$this->lot_no,$this->select3);
	$tb=get_table("TT_B",$this->pdd_chemical);
	list($x,$tester44,$tt44,$o32)=location_ttb($tb,$this->lot_no,$this->select3);
...
$d_capn=$tt_capn1."\n".$tt_capn2."\n".$tt_capn3."\n".$tt_capn4;
	...
	$objPHPExcel->setActiveSheetIndex($this->activesheet)
							...
							->setCellValue("R7",conv($v6))
	    					->setCellValue("g6",conv($this->type))
	  						->setCellValue("B8",conv($b7))
							->setCellValue("B9",conv($b8))
							->setCellValue("m9",conv($s8))
							->setCellValue('R15',conv($w14))
							->setCellValue('S15',$x14)
							->setCellValue("R17",conv($R17))
							->setCellValue("S17",$x17)
							->setCellValue("R34",conv($w33))
							->setCellValue("S34",$x33)
...
function location1($tb,$lot,$select){
		$query="SELECT DISTINCT 
								".$tb.".*, Sample_All.SMA_DRUMNO, 
								Sample_All.SMA_ID
	FROM              ".$tb." INNER JOIN
								Sample_All ON ".$tb.".LotNo = Sample_All.SMA_LOT AND 
								".$tb.".SampleNo = Sample_All.SMA_ID
								WHERE          (dbo.".$tb.".LotNo = '".$lot."') AND (dbo.Sample_All.SMA_DRUMNO = '".$select."')
	ORDER BY   dbo.".$tb.".AnalyzeTime";
	$result = mssql_query($query);
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result)){
		...
		$value1=$row['Value'];
		$tester1=$row['Tester'];
		$time1=substr($row['AnalyzeTime'],0,-6);
	}	
//	return array(1,2,3,4);
	return array($smaid1,$value1,$tester1,$time1);
}

function location2($tb,$lot,$select)
{
	$query="SELECT          dbo.Sample_All.SMA_ID, dbo.".$tb.".*, dbo.Sample_All.SMA_DRUMNO
FROM              dbo.".$tb." INNER JOIN
                            dbo.Sample_All ON dbo.".$tb.".LotNo = dbo.Sample_All.SMA_LOT AND 
...
while($row = mssql_fetch_array($result))
{
	$smaid2=$row['SMA_ID'];
	$tester2=$row['Tester'];
	$time2=substr($row['AnalyzeTime'],0,-6);
	$na=$row['Na'];
	$mg=$row['Mg'];
	$al=$row['Al'];
	$k=$row['K'];
	...
	return array($smaid2,$tester2,$time2,$na,$mg,$al,$k,$ca,$cr,$mn,$fe,$ni,$co,$cu,$zn,$in,$ce,$pb,$b);
}

function location3($tb,$lot,$select)
{
	$query="SELECT          dbo.Sample_All.SMA_ID, dbo.".$tb.".*, dbo.Sample_All.SMA_DRUMNO
FROM              dbo.".$tb." INNER JOIN
                            dbo.Sample_All ON dbo.".$tb.".LotNo = dbo.Sample_All.SMA_LOT AND 
...
ORDER BY   dbo.".$tb.".AnalyzeTime";
	...
	return array($l33,$tester13,$tt13,$l34,$l35,$l36,$l37);
}

function location_ttb($tb,$lot,$select)
{
	$query="SELECT dbo.Sample_All.SMA_ID,, dbo.Sample_All.SMA_DRUMNO, dbo.".$tb.".*
FROM              dbo.".$tb." INNER JOIN
                            dbo.Sample_All ON dbo.".$tb.".LotNo = dbo.Sample_All.SMA_LOT AND 
                            dbo.".$tb.".SampleNo = dbo.Sample_All.SMA_ID
WHERE          (dbo.".$tb.".LotNo = '".$lot."') AND (dbo.Sample_All.SMA_DRUMNO = '".$select."')
ORDER BY   dbo.".$tb.".AnalyzeTime";
	...
	return array($l33,$tester15,$tt15,$l32);
}
?>