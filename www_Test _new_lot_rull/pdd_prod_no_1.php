...
<form id="form1" name="form1" method="post" action="">
  ...
  <p>產品編號 : 
   ...
   <input name="submit5" type="submit" id="submit5" value=" EL分析課 ">
         <input name="submit6" type="submit" id="submit6" value=" EL ">
              <input name="submit7" type="submit" id="submit6" value=" DP ">
...
$editFormAction = $_SERVER['PHP_SELF'];
   ...
   include_once("connections/conn.php");
	   $query="select * from PRODUCT_TYPE where TYPE_PROD='Y' order by PDD_PROD_NO ";
	...
	echo '<td><a href="setsession_prod.php?pro_no='.$row['PDD_PROD_NO'].'&return=define_flow_chart&pro_name='.get_prod_name($row['PDD_PROD_NO']).'">選取</a></td>';
	echo "</tr>";
		}
   ...
   { $_SESSION['which']='0';
   include_once("connections/conn.php");
	   $query="SELECT DISTINCT PDD_PROD_NO, TYPE_PROD, TYPE_ANALY, TYPE_RAW, department, GM_TS, PDD_INPUT, PDD_OUTPUT FROM PRODUCT_TYPE WHERE   (TYPE_ANALY = 'Y') ";
	   $result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			echo "<tr>";
   ...
   { $_SESSION['which']='0';
   include_once("connections/conn.php");
	   $query="select * from PRODUCT_TYPE where TYPE_RAW='Y' order by PDD_PROD_NO";
	   $result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			echo "<tr>";
   ...
   { $_SESSION['which']='0';
   include_once("connections/conn.php");
	   $query="select * from PRODUCT_TYPE where department='EL分析課' order by PDD_PROD_NO ";
	   $result = mssql_query($query); 
		while($row = mssql_fetch_array($result))
		{
			echo "<tr>";
   ...
   { $_SESSION['which']='0';
 	  include_once("connections/conn.php");
	   $query="select * from PRODUCT_TYPE where department='EL' order by PDD_PROD_NO ";
	   $result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			echo "<tr>";
   ...
   { $_SESSION['which']='0';
	   include_once("connections/conn.php");
	   $query="select * from PRODUCT_TYPE where department='DP' order by PDD_PROD_NO ";
	   $result = mssql_query($query);
		while($row = mssql_fetch_array($result))
		{
			echo "<tr>";
...
if(isset($_POST['submit4'])){$_SESSION['which']='all';}
	...
	if($_GET['pdd_class']=='tank'){
		$query="SELECT [PDD_PROD_NO],[PDD_PROD_NAME],[PDD_SHOW] FROM [dbo].[PRODUCT_DATA] WHERE ([PDD_TYPE]='TANK')";
	}
$result = mssql_query($query);
