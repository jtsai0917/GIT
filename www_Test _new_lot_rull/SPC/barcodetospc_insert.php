<?php
	session_start();
	include_once("../connections/conn_localhost.php");
	include("../lib/fun.php");
...
if(isset($_POST['intospc'])){  
	...
	elseif($_POST['status']==2){
		...
		{  //read TEMPMAIN
			...
			if($_POST['cust_no']<>''){
					$query.=" and (QC_CustProdSpec.CustNo = '".trim($_POST['cust_no'])."')";	
				}
				if($_POST['prod_no']<>''){
					$query.=" and (QC_CustProdSpec.ProdNo = '".trim($_POST['prod_no'])."')";	
				}
			echo $query."<BR>";
			break;
...
//			echo $query."<BR>";
		...
		include("conn_spc_localhost.php");  
		$x=0; 
...
//				echo $mid.":".$tnvsid."<BR>";
		...
		for($i=0;$i < $cnt_ani_items;$i++){ //填寫分析項目 TEMPNEWVSUB2-4			
			$query="SELECT top(1) TEMPMAIN.MID, TEMPNEWVSUB.TNVSID, TEMPNEWVSUB.ALIAS, TEMPNEWVSUB.UNIT, TEMPMAIN.GROUPNAME, TEMPMAIN.FILENAME
							...
							where TEMPMAIN.GROUPNAME='".$TEMPNEWVSUB[$i][1]."' and TEMPMAIN.FILENAME='".$TEMPNEWVSUB[$i][2]."' and TEMPNEWVSUB.CTRLITEM ='".$TEMPNEWVSUB[$i][4]."' ORDER BY   TEMPMAIN.MID DESC ";			
			$result=mssql_query($query);
			$row=mssql_fetch_row($result);
			$mid=$row[0];
			$tnvsid=$row[1];
...
if(isset($_POST['enter'])){
	$query="SELECT DISTINCT 
						...
						FROM              AnalyzeItem INNER JOIN
	                            ...
	                            CUSTOMER_DATA ON QC_CustProdSpec.CustNo = CUSTOMER_DATA.CTD_CUST_NO INNER JOIN
	                            PRODUCT_DATA ON QC_CustProdSpec.ProdNo = PRODUCT_DATA.PDD_PROD_NO                                                  
							WHERE          (QC_CustProdSpec.CustNo <>'') and (QC_CustProdSpec.CustNo <> '') AND (QC_CustProdSpec.CustNo <> 'C00001') AND 
                            (QC_CustProdSpec.CustNo <> 'V14051') AND (QC_CustProdSpec.ProdNo NOT LIKE 'WP%') ";
	if($_POST['cust_no']<>''){
		$query.=" and (QC_CustProdSpec.CustNo = '".trim($_POST['cust_no'])."')";	
	}
	if($_POST['prod_no']<>''){
