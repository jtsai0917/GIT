...
<form id="form1" name="form1" method="post" action="<?php echo $loginFormAction; ?>">
  ...
  <table border="0" width="600">
	  ...
	  <?php 
	  if (trim($row['CTD_CUST_NO'])<>''){echo $cid=$row['CTD_CUST_NO'];echo "&nbsp;&nbsp;&nbsp;".trim(get_cust_name($row['CTD_CUST_NO']));$_cust_id=trim($row['CTD_CUST_NO']);}
	  else{
		  	$query1="select CTD_CUST_NO from  FILLPLAN_DRUM_CUSTOMER where FOD_UNI='".$_GET['uni']."'";
		  	$result1 = mssql_query($query1);
			$numRows1 = mssql_num_rows($result1);
			if($numRows1==1){$row1 = mssql_fetch_row($result1);echo $cid=$row1[0];echo "&nbsp;&nbsp;&nbsp;".get_cust_name($row1[0]);$_cust_id=$row1[0];}
			if($numRows1>1){$_cust_id="C00000";echo $_cust_id."&nbsp;&nbsp;&nbsp;".get_cust_name($_cust_id);echo "(使用社內規格)";}
		  }
...
EX: 08:30</td></tr></table>
	<table border="0" width="600">
      <tr><td width="100">LOT_NO；</td><td width="100"><input name="lot_no" type="text" id="lot_no" value="<?php 
			...
			echo $lot;
	  ?>" size="12"  readonly="readonly" /></td><td width="400" align="right"> 來源TANK
      <?php
	  	$tank=new tank_data;
		$tank->pid=$_GET['pid'];
		$tank->lid=$lot;
    ...
    EX: 08:30</span></td></tr>
      ...
      <tr><td width="50">先行樣品瓶數；</td><td width="250">
   	  <input name="be_cnt" type="text" id="be_cnt" size="5" value="<?php if($_SESSION['be_cnt']==''){echo $row["FDM_SAM_BEF_CNT"];}else{echo $_SESSION['be_cnt'];}?>"  onchange="set_date_session(this.name,this.value)"/></td></tr>
	  ...
	  <input name="sam_cnt" type="text" id="sam_cnt" size="5" value="<?php if($_SESSION['sam_cnt']==''){echo $row["FDM_SAM_CNT"];}else{echo $_SESSION['sam_cnt'];}?>"  onchange="set_date_session(this.name,this.value)"/></td></tr>
      <tr><td width="50">總取樣瓶數；</td>
  <td width="250"><input name="smp_total" type="text" id="smp_total" size="5" value="<?php 
	echo $_SESSION['smp_total']=$_SESSION['be_cnt']+$_SESSION['att_cnt']+$_SESSION['sam_cnt']+2;
...
if(isset($_POST["anylize"])){
	...
	"&AND_GOODS=".$_GET['pid']."&AND_BEFORE=".$smp."&COA_BEFORE=".$coa."&rt=".$s2."&AND_LOT_NO=".$_SESSION['new_lot']."&outdate=".dod($_POST['datepicker3']);
	jumpto($url);
}
...
if(isset($_POST["save"]))
{
	...
	// SN
		...
		$query="UPDATE        dbo.FILLPLAN_OUT_DECIDE
				SET                  	  FOD_YEAR_MONTH ='".substr($_POST['datepicker1'],6,4).substr($_POST['datepicker1'],0,2)."', FOD_DAY ='".substr($_POST['datepicker1'],3,2)."', 
										  ...
										  CTD_CUST_NO ='".$cid."', PDD_PROD_NO ='".$_GET['pid']."', CTD_CUST_NO_GROUP =".$cust_group.",
	...
	if($_GET['style']<>'DM')
	{
		$query="INSERT INTO PRODUCT_STOCKS
			...
			VALUES          ('".$_POST['lot_no']."','".$_GET['pid']."','".$cid."',NULL,'".dod($_SESSION['datepicker1'])."','".$_SESSION['fill_qty']."',
...
WHERE   (FILL_INDICATE.FDM_LOT_NO = '".$_POST['lot_no']."')";
		...
		if($_GET['style']=='LY')
		{
			...
			if($numRows==0)
			{
				$query="INSERT INTO PRODUCT_RUNNING_ACCOUNT
					...
					VALUES          ('".$_POST['lot_no']."', ".$sn.", '".$_GET['pid']."', 1, '".$cid."', '".$_GET['qtyun']."', '".date("Ymd")."', 0, 0, 0, 0, 0, '".prod_out_term($_GET['pid'],$cid)."')";
				$result = mssql_query($query);
				sql_rec($_SERVER['PHP_SELF'],$query);
			...
			if($numRows>0)
			{
					$query="update PRODUCT_RUNNING_ACCOUNT set PRA_LOT_NO='".$_POST['lot_no']."', PRA_SERIAL_NO=".$sn.", PDD_PROD_NO='".$_GET['pid']."', PRA_PURPOSE=1, 
							CTD_CUST_NO='".$cid."', PRA_UNIT='".$_GET['qtyun']."', PRA_FAKE_IN_DATE1='".date("Ymd")."', PRA_IN_DM=0, PRA_REAL_IN_QTY=0, PRA_OUT_DM=0,
							PRA_REAL_OUT_QTY=0, PRA_OUT_COUNT=0, PRA_OUT_TERM='".prod_out_term($_GET['pid'],$cid)."' where PRA_LOT_NO='".$_POST['lot_no']."'";
					$result = mssql_query($query);
					sql_rec($_SERVER['PHP_SELF'],$query);
	...
	///檢查有沒有依賴				{
				...
				if($_POST['select_test_items']=='0')
					{
						$ull='../lib/save_analyze.php?datepicker1='.dod($_POST['datepicker1'])."1000&comfirm=Y&cid=".$cid."&pid=".$_GET['pid']."&out_date=".$_POST['datepicker3'].$_POST['time3'].$_POST['time32'].
						"&lot_no=".$_POST['lot_no']."&out_date=".$s1."&testitems=".$_aniitems."&smp_cnt=".($_SESSION['smp_total'])."&item_style=".$_SESSION['select_test_items'];
						jumpto($ull); 
					}
				elseif($_POST['select_test_items']=='1')
					{
						...
						$items->pid=$_GET['pid'];
						$items->get_rull_1();
						$ull='../lib/save_analyze.php?datepicker1='.dod($_POST['datepicker1'])."1000&comfirm=Y&cid=".$cid."&pid=".$_GET['pid']."&out_date=".$_POST['datepicker3'].$_POST['time3'].$_POST['time32']."&lot_no=".$_POST['lot_no']."&out_date=".$s1."&testitems=".$_aniitems."&smp_cnt=".$_SESSION['smp_total']."&item_style=".$_SESSION['select_test_items'];
						jumpto($ull); 
					}
		...
		{		
			...
			if($numRows>0)
			{
				$query="update FILLPLAN_DRUM_CUSTOMER set CTD_CUST_NO='".$cid."' where FOD_UNI=".$_GET['uni'];	
			}
			else{
				$query="INSERT INTO FILLPLAN_DRUM_CUSTOMER
								(FOD_UNI, FDM_LOT_NO, CTD_CUST_NO, FDMC_SERIAL_NO)
	VALUES          (".$_GET['uni'].",'".$_POST['lot_no']."','".$cid."',".$sn.")";	
			}
			sql_rec($_SERVER['PHP_SELF'],$query);
			$result = mssql_query($query);
		}	
