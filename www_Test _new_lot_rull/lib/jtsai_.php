...
public $sample_metal_f,$sample_particle_f;
...
function getone(){
		...
		while($row=mssql_fetch_array($result)){
			...
			$this->pdd_drum_l=$row['PDD_DRUM_LITER'];
		}
	}
}
...
function genlot($pid,$day,$lid){
	...
	if($prod->pdd_type=='LY'){
		$y=substr($day,2,2);
		$z=substr($day,4,2);
		$a=$prod->pdd_shortname.$y.exmonth($z).substr($day,6,2).substr($lid,2,4);
		return $a;
//		return "X1";
	}
	else{		
		...
		{
			$a=$prod->pdd_shortname.$y.exmonth(substr($day,4,2)).substr($day,6,2).substr($prod->pdd_type,0,1).substr($prod->pid,-3);
		}
		else
		{
			$place=1+strpos($pid,"-");
			$subpid=substr($pid,$place);
			$a=$prod->pdd_shortname.$y.exmonth(substr($day,4,2)).substr($day,6,2).substr($prod->pdd_type,0,1).$subpid;
		}
		return $a;
//		return "X2";
	}
...
class analyze_first{
	public $lot_no,$sample_no,$create_time,$create_user,$first,$sampler;
	function get(){
		include("../connections/conn.php");
		...
		$query="SELECT DISTINCT [index], lotno, ani_group, sampler, sample_id, createtime, creator FROM sampler WHERE  (lotno='".$this->lot_no."') and (sample_id='".$this->sample_no."')";
//		echo $query;
		$result=mssql_query($query);
		while($row=mssql_fetch_array($result)){
				$this->sampler=$row['sampler'];
		}
	}	
}
...
class read_report
{
	public $url,$location,$item,$efm,$value,$float,$pid,$group,$x,$y,$yy,$ft,$smpid,$lot_no ;
	function read()
	{
		session_start();
		...
		{	  ///read excel2007	
			...
			require_once("../connections/conn.php");
     		...
     		if(($this->group=='M13' or $this->group=='M21') or (($this->group=='TT' or $this->group=='TT_HG') and substr($_GET['pdd_prod_no'],-3,3)=='BLK'))
			{
				if(substr($_SESSION['lot_no'],-3)<>'CPS')
				{
					for($x=0;$x<=$highestRows;$x++)
					{
						$v = trim($sheet->getCellByColumnAndRow('A',$x)->getValue());
						if($v=='SLOPE'){$a=$x;}
						elseif($v=='RSQ'){$b=$x;}
					}
					$v = null;
					for($x=0;$x<4;$x++)
					{
						for ($c = 0; $c < $highestColumns; $c++) 
							{ // 0 = A欄
								...
								if($v=='BLANK' and $a!='' and $b!=''){$_SESSION['pass']=1;}
								if($v=='Sample Name')
									{
								...
								if(strpos($v," ".$this->item." ") and trim($this->item)!='' and $_SESSION['pass']==1)
									{
										//echo $v.','.$this->item.'<br>';
										$slop=trim($sheet->getCellByColumnAndRow($c,$a)->getCalculatedValue());
										$rsq=trim($sheet->getCellByColumnAndRow($c,$b)->getCalculatedValue());
										
										$_SESSION['SLOP'][$this->item]=round($slop);
										$_SESSION['RSQ'][$this->item]=round($rsq,5);
					...
					for($x=0;$x<4;$x++)
					{
						for ($c = 0; $c < $highestColumns; $c++) 
							{ // 0 = A欄
								...
								if(strpos($v," ".$this->item." "))
									{
										$this->value=trim($sheet->getCellByColumnAndRow($c+1,$x+2)->getValue());
									}
			...
			elseif($this->group=='TN')
			{
				for($c=37;$c<=47;$c++){
					$v = trim($sheet->getCellByColumnAndRow(8,$c)->getValue());
					if($v==$_SESSION['lot_no']){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif((substr($v,0,3)=='TO1' and substr($_SESSION['lot_no'],0,3)=='TO1') and (substr($v,-2,2)=='-R')){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,3)=='MXA' and substr($_SESSION['lot_no'],3,3)=='MXA'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,3)=='MXB' and substr($_SESSION['lot_no'],3,3)=='MXB'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,3)=='MXC' and substr($_SESSION['lot_no'],3,3)=='MXC'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					
					if(substr($v,-4,4)=='A001' and substr($_SESSION['lot_no'],-4,4)=='A001'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,-4,4)=='B001' and substr($_SESSION['lot_no'],-4,4)=='B001'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,7)=='D-1705A' and substr($_SESSION['lot_no'],0,6)=='TO175A'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,7)=='D-1705B' and substr($_SESSION['lot_no'],0,6)=='TO175B'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,7)=='D-1705C' and substr($_SESSION['lot_no'],0,6)=='TO175C'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,0,6)=='T-1705' and substr($_SESSION['lot_no'],0,6)=='TO175T'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
					elseif(substr($v,5,3)=='PER' and substr($_SESSION['lot_no'],5,3)=='PER'){
						$_SESSION['Value1']=$this->value=trim($sheet->getCellByColumnAndRow(15,$c)->getCalculatedValue());
						break;
					}
				}
				$query="SELECT         excel
				FROM             dbo.ani_excel_location
				WHERE         (efm = '".$this->efm."') AND (item = '".$this->item."') AND (item <> 'Lot_No')";
...
//						if(trim($row['excel'])=='H3'){$_SESSION['H3']=$this->value;}
//						if(trim($row['excel'])=='I3'){$_SESSION['I3']=$this->value;}
//						if(trim($row['excel'])=='J3'){$_SESSION['J3']=$this->value;}
					}
				}
...
//				if($this->item=='Color'){$_SESSION['tmpsss']=$ssp;}	
    		...
    		elseif($this->group=='COA' and $this->item=='Lot_no')
			{
				$query="SELECT         excel
				FROM             dbo.ani_excel_location
				WHERE         (efm = 'COA') AND (item = '".$this->item."') AND (ani_group = 'COA')";
				$query.=" AND (pid = '".$this->pid."') ";
				$result=mssql_query($query);
				$numrows=mssql_num_rows($result);
				if($numrows>0)
...
//				if($this->item=='Color'){$_SESSION['tmpsss']=$ssp;}	
			...
			else
			{
				$query="SELECT         excel
				FROM             dbo.ani_excel_location
				WHERE         (efm = '".$_GET['efm']."') AND (item = '".$this->item."') AND (item <> 'Lot_No')";
				$query.=" AND (pid = '".$_GET['pdd_prod_no']."') ";
				$result=mssql_query($query);
				$numrows=mssql_num_rows($result);
				if($numrows>0)
				{	
...
//			$_SESSION['OOOOOOOOO']=substr($_SESSION['lot_no'],-2);
			
			$_SESSION['START:']='TXT';
			if(substr($_GET['lot_no'],0,3)<>'TDL' and substr($_SESSION['lot_no'],-3)<>'CPS' and substr($_SESSION['lot_no'],-3)<>'BLK')
			{
//				echo 			"XXXXXODXXXXX";
				$_SESSION['START:']=$_SESSION['START:']."NOCPS";
				$myfile = fopen($this->url, "r") or die("Unable to open file!");
				while(!feof($myfile)) 
				{
					$str=fgets($myfile);
					$str0=trim(substr($str,0,13));
					$str1=trim(substr($str,3,6));
					$item_value0=trim(substr($str,14,30));
					$item_value=trim(substr($str,44,9));
			//		echo $str1.":".$item_value.":".$this->item."</br>";
					if($str0=='Sample Name')
					{
						$_SESSION['SampleNo']=trim($item_value0);
						$offst=strpos(trim($item_value0),'[');
						$offst1=strpos(trim($item_value0),']');
						if($offst=='' and $offst1=='')
						{
							$offst=strpos(trim($item_value0),'(');
							$offst1=strpos(trim($item_value0),')');
						}
						if($offst<>''){$_SESSION['SampleNo']=substr($_SESSION['SampleNo'],($offst+1),($offst1-$offst-1));}
					}
					elseif($str1==($this->item))
					{
						$_SESSION[$this->item]=$this->value=$item_value;
					}
				}
			}
			elseif(substr($_SESSION['lot_no'],-3)=='CPS' or substr($_SESSION['lot_no'],-3)=='BLK')
			{
				$_SESSION['START:']=$_SESSION['START:']."CPS";
				$myfile = fopen($this->url, "r") or die("Unable to open file!");
				while(!feof($myfile)) 
				{
					$str=fgets($myfile);
					$str0=trim(substr($str,0,13));
					$str1=trim(substr($str,3,6));
					$item_value0=trim(substr($str,14,10));
					$item_value=trim(substr($str,28,9));
		...
		//			echo $str1.":".$item_value.":".$this->item."</br>";
			...
			else
			{
				...
				while(!feof($myfile)) 
				{
					$_SESSION['START:']=$_SESSION['START:']."normal";
					$str=fgets($myfile);
					$str0=trim(substr($str,0,13));
					$str1=trim(substr($str,3,6));
					$item_value0=trim(substr($str,14,10));
...
class sample
{
	...
	function selections()
	{
		...
		{
			{
				...
				$drum_qty->get();
				$num=$drum_qty->drum_qty;
				$x=floor($num/64)+1;
				if($_SESSION['drumno2']=='0~0'){$s1='selected="selected"';}
				echo '<option value="0~0" '.$s1.'>0~0</option>';
				for($i=1;$i<=$x;$i++)
				{
					if($i<>$x){			
						$num1=($i*64-63)."~".($i*64);
					...
					if(trim($num1)==trim($_SESSION['drumno2'])){echo '<option value="'.$num1.'" selected="selected">'.$num1.'</option>';}
		...
		if(($gn->pdd_type=='DM'))
		{
			...
			elseif($gn->pdd_chemical=='HF' or $gn->pdd_chemical=='HNO3'){
				$drum_qty=new fillplan_out_decide;
				$drum_qty->lot_no=$this->lot_no;
				$drum_qty->get();
				$num=$drum_qty->drum_qty;
				...
				if($_SESSION['drumno2']=='0~0'){$s1='selected="selected"';}
				echo '<option value="0~0" '.$s1.'>0~0</option>';
				if($num1==$_SESSION['drumno2']){
					echo '<option value="1~'.$num.'" selected="selected">1~'.$num.'</option>';
				}
				else{
					echo '<option value="1~'.$num.'">1~'.$num.'</option>';
				}
			}
			else
			{
				...
				$x=round($drum_qty->drum_qty/2,0);
				$s1=$s2=$s3=$s4='';
				if($_SESSION['drumno2']=='0~0'){$s1='selected="selected"';}
				elseif($_SESSION['drumno2']=='1~1'){$s2='selected="selected"';}
				elseif($_SESSION['drumno2']==$x.'~'.$x){$s3='selected="selected"';}
				elseif($_SESSION['drumno2']==$_SESSION['drum_qty'].'~'.$_SESSION['drum_qty']){$s4='selected="selected"';}
				echo '<option value="0~0" '.$s1.'>0~0</option>';
				echo '<option value="1~1" '.$s2.'>1~1</option>';
					echo '<option value="'.$x."~".$x.'"  '.$s3.'>'.$x."~".$x.'</option>';
					echo '<option value="'.$_SESSION['drum_qty']."~".$_SESSION['drum_qty'].'" '.$s4.'>'.$_SESSION['drum_qty']."~".$_SESSION['drum_qty'].'</option>';
			}
		}
		if($gn->pdd_type=='LY')
		{
...
class out_plan_from_file
{
	...
	$query="SELECT          *
			FROM              OUT_PRODUCT
			WHERE          (OPM_ORDER_NO = '".$this->order_no."') and (PDD_PROD_NO='".$this->pdd_prod_no."') and (OPD_LOT_NO='".$this->lot_no."')";
	$result=mssql_query($query);
		$numRows1 = mssql_num_rows($result);
		while($row=mssql_fetch_array($result)){
			$this->memo =$row['OPD_MEMO'];
		}
	}
}
...
class PDD_MADE_FROM
{
	public $pid_from,$pid_target,$weight;
	function cx()
	{
		$queru="SELECT  PDD_MADE_FROM.* FROM PDD_MADE_FROM WHERE (target_pid ='".$this->pid_target."' )";
		$result=mssql_query($queru);
		$row=mssql_fetch_row($result);
		$this->pid_from=trim($row[2]);
		$this->weight=trim($row[3]);
	}
}

class board_all{
	public $bno,$btime,$begin,$out_cus,$out_date,$back_date,$discard_date,$discard_reason,$discard_uid;
	function last_boa(){
		$query="SELECT  TOP (1) BOA_NO, BOA_TIMES, BOA_BEGIN_DATE, BOA_OUT_LOC, BOA_OUT_DATE, BOA_BACK_DATE, 
                   BOA_DISCARD_DATE, BOA_DISCARD_REASON, BOA_DISCARD_UID
 FROM BOARD_ALL WHERE (BOA_NO = '".$this->bno."') ORDER BY BOA_TIMES DESC";	
		$result=mssql_query($query);
		$row=mssql_fetch_row($result);
		$this->btime=$row[1];
		$this->begin=$row[2];
		$this->out_cus=$row[3];
		$this->out_date=$row[4];
		$this->back_date=$row[5];
		$this->discard_date=$row[6];
		$this->discard_reason=$row[7];
		$this->discard_uid=$row[8];
	}
}
