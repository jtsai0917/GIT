...
//	echo $query."<BR>";
	...
	if($row[0]>0){
		...
		echo "<BR>".$query."<BR>";
		$result4 = mssql_query($query);
		if($result4){}else{my_msg("update PRODUCT_STOCKS failed!!");}
	}
	else{
		$query="INSERT INTO PRODUCT_STOCKS
                   (STK_LOT_NO, PDD_PROD_NO, CTD_CUST_NO, STK_MAKE_DATE, STK_QTY, STK_DRUM_COUNT, 
...
VALUES  ('".trim($_SESSION['lid'])."', '".$pid."', '".$c."', '".date("Ymd")."', 100, 0, 0, 0, 0)";
	...
	if($result6){}else{my_msg("update PRODUCT_RUNNING_ACCOUNT failed!!");}
...
//	echo "<BR>".$query."<BR>";
	...
	if($result5){;}else{my_msg("update FILL_INDICATE failed!!");}
