...
<?php 
	echo $_SESSION['uname']."<BR>車輛代號：";
		if($_SESSION['cartype']=='Lorry 拖車'){$s0='selected';}
		...
		echo '<select name="cartype" id="cartype" onchange="set_date_session(this.name,this.value)">
			  <option value="Lorry 拖車" '.$s0.'></option>
...
//	if($ap->spc<>'LY'){echo '<tr align="center"><td>位置</td><td>'.$_SESSION['lorry_no1'].'</td></tr>' ; }
	...
	if(substr(trim($_SESSION['lorry_no1']),0,11)==trim($_SESSION['lid']) and substr(trim($_SESSION['lorry_no1']),11,4)=='MCTW'){
...
if(isset($_POST['save'])) {
	...
	$query="SELECT TOP (1) [index], CAR_NO, DATE, PRINTED, CANCEL, DATETIME FROM YG_CAR_OUT  WHERE (CAR_NO = '".$_SESSION['cartype']."' and LotNo='".$_SESSION['lid']."') ORDER BY [index] DESC";
	echo $query."<BR>";;
	...
	if($car_cancel<>'' or $car_printed<>'' or $numrows==0){
		...
		$query1="INSERT INTO YG_CAR_OUT (CAR_NO, [DATE], DATETIME, LotNo) VALUES ( N'".$_SESSION['cartype']."', N'".date("Ymd")."', N'".$datetime."','".$_SESSION['lid']."')  select scope_identity() ";	
		echo $query1."<BR>";;
