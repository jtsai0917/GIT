	...
	*/
	...
	//echo $queryo;
		...
		while($rowo = mssql_fetch_array($resulto))
			...
			$query="select ".$_POST['work']." from analyze_first as AF 
		left join ".$_GET['efm']." as TL on AF.create_time=TL.analyzetime 
		where lot_no like '".$_SESSION['lot1']."%' and first = '".$rowo['first']."' and AF.ani_group='M13' order by TL.lotno,first";
			$result = mssql_query($query);
			//echo $query;
			while($row = mssql_fetch_array($result))
			{	
		...
		{ $T=substr($row['create_time'],0,8);
			...
			//echo $ENG.$s;
					for($i=0;$i<count($a1);$i++)
				{
					...
					$s++;
					if($ENG=='AG'){$ENG='C';}
					if($s==21 and $times==0){$ENG++;$s=8;}
					if($s==37 and $times==1){$ENG++;$s=24;}
					if($s==53 and $times==2){$ENG++;$s=40;} 	
				}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	$PHPExcel = $reader->load("../FormList/PrintAnalyze/RR/".$_GET['pdd_chemical']."/".$_GET['ani_groupname'].".xlsx");
		...
		//echo ".../FormList/PrintAnalyze/RR/".$_GET['pdd_chemical']."/".$_GET['ani_groupname'].".xlsx<br>";
		$sheet = $PHPExcel->getSheet(1);
		$a1=array();
		$a2=array('C','M');
...
//			echo "<BR>";
	...
	$queryo="select distinct top 2 first from analyze_first where lot_no like '".$_SESSION['lot1']."%' and create_user!=''";
	//echo $queryo;
		$resulto = mssql_query($queryo);
		while($rowo = mssql_fetch_array($resulto))
		{	
	...
	$query="select * from analyze_first as AF 
	left join ".$_GET['efm']." as TL on AF.create_time=TL.analyzetime 
	where lot_no like '".$_SESSION['lot1']."%' and first = '".$rowo['first']."' and AF.ani_group='M13' and SerialNo!=3 and SerialNo!=6 and SerialNo!=9 order by TL.lotno";
	//echo $query;
	$n=0;
	$count=0;
	$result = mssql_query($query);
