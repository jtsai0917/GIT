...
~
<input name="datepicker4" type="text" id="datepicker4" size="10" value="<?php echo $_SESSION['datepicker4'];?>" onChange="set_date_session(this.name,this.value)"></td>
...
echo '<tr class="GridviewScrollHeader"><td>藥品名</td><td>客戶名</td><td>荷姿 NO</td><td>LOT NO</td><td>SN</td><td>指示日期</td><td>充填日期</td><td>開始時間</td><td>結束時間</td><td>實際出庫量</td><td><input type="submit" size="4" name="remnant"  style="background-color:white" value="充填殘單"></td><td><input type="submit" size="4"  name="tys_qty"  style="background-color:#FF0" value="充填滿單"></td><td>充填來源</td><td>充填路徑</td><td>來源LOT NO</td><td>充填手</td><td>保壓測試</td><td>取樣數</td><td>送出數</td>
...
FROM              FILLPLAN_OUT_DECIDE AS FOD LEFT OUTER JOIN
			...
			WHERE          (FOD.FDM_LOT_NO <> NULL )	";
	if($_SESSION['lid']<>''){$query.= "AND (FOD.FDM_LOT_NO like '%".$_SESSION['lid']."%') ";}	
	else{
	if($_SESSION['datepicker1']<>''){$query.= "AND ((LEFT(FOD.FDM_EXPECT_DATE, 8) >= '".dod($_SESSION['datepicker1'])."') or (LEFT(FOD.FDM_EXPECT_DATE, 8) >= '".dod($_SESSION['datepicker1'])."')) ";}
	if($_SESSION['datepicker2']<>''){$query.= "AND ((LEFT(FOD.FDM_EXPECT_DATE, 8) <= '".dod($_SESSION['datepicker2'])."') or (LEFT(FOD.FDM_EXPECT_DATE, 8) <= '".dod($_SESSION['datepicker2'])."')) ";}
...
//		echo "AL=". $row['AL']; 
		...
		echo '<tr class="GridviewScrollItem"><td>'.$row['F1'].'</td><td>'.$row['F2'].'</td><td>'.$row['F3'].'</td><td>'.$row['F4'].'</td><td>'.$row['lysn'].'</td><td>'.$row['F5'].		'</td><td>'.$row['F6'].'</td><td>'.substr($row['F7'],0,2).":".substr($row['F7'],2,2).":".substr($row['F7'],4,2).'</td><td>'.substr($row['F8'],0,2).":".substr($row['F8'],2,2).":".substr($row['F8'],4,2).'</td><td>'.LEL_remnant($row['F4']).'</td><td align="center">'.$tysqty1.'</td><td align="center">'.$tysqty.'</td><td>'.$row['充填作業.充填來源TANK'].'</td><td>'.fill_flow($row['F4']).'</td><td>'.fill_source($row['F4']).'</td><td>'.get_uname($row['fill_operator']).'</td><td>'.$row['F11'].'</td><td>'.$row['取樣作業.取樣數'].'</td><td>'.$row['取樣作業.送出數'].'</td><td>'.$row['取樣作業.送出時間'].'</td><td>'.$row['分析作業.取得時間'].'</td><td>'.$row['分析作業.取得數量'].'</td><td>'.$row['F12'].'
		</td><td>'.$row['分析作業.判定'].'</td><td>'.$row['F13'].'</td><td>'.$row['F14'].'</td><td>'.$row['F15'].'</td></tr>';
		if(trim($row['TYSQTY1'])<>'' and trim($row['TYSQTY'])<>''){$qty=round(($row['TYSQTY']-$row['TYSQTY1']),2);}
		elseif(trim($row['TYSQTY1'])=='' and trim($row['TYSQTY'])==''){$qty=0;}
		if($row['PTYPE']=='DM' or $row['PTYPE']=='BTL'){$qty=round($row['F10']);}
		...
		echo '<input type="hidden" name="SCHEDEND'.$j.'" value="'.stab($row['F5']).'"/>';
...
//		echo "LKG:".$_POST['lkg'.$n]."<BR>";
		if($_POST['QTYSCHED'.$n]<>0 or $_POST['PTYPE'.$n]=='DM' or $_POST['PTYPE'.$n]=='BTL'){
			...
			if($_POST['QTYSCHED'.$n]>0  or $_POST['PTYPE'.$n]=='DM' or $_POST['PTYPE'.$n]=='BTL'){
				$query="select count(*) from TYS_PRODTABLETEMP where INVENTBATCHID='".$_POST['INVENTBATCHID'.$n]."'";	
				$result=mssql_query($query);
				$row=mssql_fetch_row($result);
				if(trim($_POST['WEIGHTS'.$n])==''){$_POST['WEIGHTS'.$n]=0;}
				if($row[0]>0){  // update 
					$query="update TYS_PRODTABLETEMP set BOMINVENTBATCHID='".$_POST['BOMINVENTBATCHID'.$n]."' , BOMITEMID='".$_POST['BOMITEMID'.$n]."',WEIGHTS=".$_POST['WEIGHTS'.$n].",CUSTACCOUNT='".$_POST['CUSTACCOUNT'.$n]."', INVENTBATCHID='".$_POST['INVENTBATCHID'.$n]."', ITEMID='".$_POST['ITEMID'.$n]."',LORRYNO='".$_POST['LORRYNO'.$n]."',QTYSCHED='".$qtyfill."',SCHEDEND='".$_POST['SCHEDEND'.$n]."',UNIT01='".$_POST['UNIT01'.$n]."' where INVENTBATCHID='".$_POST['INVENTBATCHID'.$n]."'";
//				 	echo $query."<BR>";
					echo "更新  ".$_POST['INVENTBATCHID'.$n]."<BR>";
					$result=mssql_query($query);
				}
				else{
					$query="insert into  TYS_PRODTABLETEMP (BOMINVENTBATCHID, BOMITEMID, WEIGHTS, DATAAREAID, RECID, CUSTACCOUNT,INVENTBATCHID,ITEMID,LORRYNO,QTYSCHED,SCHEDEND,UNIT01) values ('".$_POST['BOMINVENTBATCHID'.$n]."', '".$_POST['BOMITEMID'.$n]."',".$_POST['WEIGHTS'.$n].",'tys', ".$nummax.",'".$_POST['CUSTACCOUNT'.$n]."', '".$_POST['INVENTBATCHID'.$n]."', '".$_POST['ITEMID'.$n]."','".$_POST['LORRYNO'.$n]."','".$_POST['QTYSCHED'.$n]."','".$_POST['SCHEDEND'.$n]."','".$_POST['UNIT01'.$n]."') ";
//			 		echo $query."<BR>";
...
if(isset($_POST['ax_submit'])){
	...
	{   
		...
		if($_POST['UNIT01'.$n]=='L'){   /// 藥品單位為公升的
			$qtyfill=round($_POST['QTYSCHED'.$n]/$_POST['lkg'.$n],0);			
		}
...
if(isset($_POST['excel']))
{
	...
	$query="SELECT DISTINCT PDD.PDD_PROD_NAME AS F1, CTD.CTD_CUST_SHORT_NAME AS [F2], 
				   ...
				   LEL.LEL_TYS_QTY AS TYSQTY, LEL.LEL_TYS_REMNANT_QTY AS TYSQTY1, PDD.PDD_TYPE AS PTYPE
FROM      FILLPLAN_OUT_DECIDE AS FOD LEFT OUTER JOIN
                   PRODUCT_DATA AS PDD ON FOD.PDD_PROD_NO = PDD.PDD_PROD_NO LEFT OUTER JOIN
                   CUSTOMER_DATA AS CTD ON FOD.CTD_CUST_NO = CTD.CTD_CUST_NO LEFT OUTER JOIN
                   FILL_INDICATE AS FID ON FOD.FDM_LOT_NO = FID.FDM_LOT_NO LEFT OUTER JOIN
	...
	{   
		$obj
							...
							->setCellValue("D".$i,iconv('big5','utf-8',$row['F4']))
							...
							->setCellValue("p".$i,iconv('big5','utf-8',$row['取樣作業.取樣數']))
							->setCellValue("q".$i,iconv('big5','utf-8',$row['取樣作業.送出數']))
							->setCellValue("r".$i,iconv('big5','utf-8',$row['取樣作業.送出時間']))
							->setCellValue("w".$i,iconv('big5','utf-8',$row['分析作業.取得時間']))
							->setCellValue("t".$i,iconv('big5','utf-8',$row['分析作業.取得數量']))
							->setCellValue("u".$i,iconv('big5','utf-8',$row['F12']))
							->setCellValue("v".$i,iconv('big5','utf-8',$row['分析作業.判定']))
							->setCellValue("w".$i,iconv('big5','utf-8',$row['F13']))
							->setCellValue("x".$i,iconv('big5','utf-8',$row['F14']))
							->setCellValue("y".$i,iconv('big5','utf-8',$row['F15']))
							;
		$i=$i+1;
	}
