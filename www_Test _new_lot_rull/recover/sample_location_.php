...
~
<input name="datepicker2" type="text" id="datepicker2" size="10" >&nbsp;&nbsp;&nbsp;站別: 
...
if(isset($_POST['submit'])){
	...
	<td width="150" align="center">分析</td><td width="150" align="center">客戶端</td></tr>';
	...
	while($row=mssql_fetch_array($result)){
		...
		<td width="100" align="center">'.get_cust_name($cust_no).'</td><td width="100" align="center">'.$location_name.'</td>
		<td width="100" align="center">'.$createtime.'</td><td width="100" align="center">'.$time2.'</td><td width="100" align="center">'.$time4.'</td><td width="100" align="center">'.$time3.'</td></tr>';
		$i++;
	}	
	echo '</table><br>';
}
function loc_aa($smpid,$gid){
	$query="SELECT [index], SMPID, Lot_NO, CONVERT(VARCHAR, createtime, 120) AS createtime , LOCATION FROM Sample_Location WHERE (SMPID = N'$smpid') AND (gid = $gid) ORDER BY   [index]";
	$result=mssql_query($query);
	while($row=mssql_fetch_array($result)){
		if($row['LOCATION']==1){$time1=$row['createtime'];}
		elseif($row['LOCATION']==2){$time2=$row['createtime'];}
		elseif($row['LOCATION']==3){$time3=$row['createtime'];}
		elseif($row['LOCATION']==4){$time4=$row['createtime'];}
	}
	$aa=array(0 => $time1, 1 => $time2, 2 => $time3, 3 => $time4);
	return $aa;
}
