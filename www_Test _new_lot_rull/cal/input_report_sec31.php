...
while($row=mssql_fetch_array($result)){
  ...
  <table width="1200" border="1">
    <tr class="centet">
      ...
      <input name="LotNo" type="text" id="LotNo" value="<?php echo $_SESSION['getlot']?>" size="14" readonly /></td>
...
<?php 
if(($_GET['ani_groupname']=='M13') or ($_GET['ani_groupname']=='M21')){
...
?> 
      ...
      <input name="opname" type="text" value="<?php echo $_SESSION['username'];?>" size="10" readonly />
...
($row['COLUMN_NAME']!='SerialNo') && ($row['COLUMN_NAME']!='TestDate'))
{
	...
	echo '<tr class="centet">';
		...
		if($ss->value<>''){$_SESSION[$row['COLUMN_NAME']]=$ss->value ;}
		if((substr(trim($ss->value),0,1)=='<') or (substr(trim($ss->value),0,1)=='>')){$_SESSION[$row['COLUMN_NAME']]=$value=0.000001;}
...
<table width="1200" border="1">
    <tr class="centet" >
      <td width="800" align="center"><font color="#FF0000">合否判定</font>
        <?php if($xx==1){$x1="checked";}else{$x1='unchecked';} ?>
      <input type="checkbox" name="Ok" id="Ok" <?php echo $x1;?> readonly />
      ...
      <input type="submit" name="refresh" id="refresh" value=" 檢 查 "/>
...
jumpto($_SESSION['redir']."&url=input_report_sec3");
}
