...
if(date("Ymd")<=$ymd)
{
	echo "今日：".date("Ymd");
	echo "</br>";
	echo "出荷日：".$_GET['ym'].$_GET['day'];
	echo "</br>";
	{
	$_SESSION['my_msg']=0;
	$query="SELECT          EMPLOYEE_DATA.EMP_NAME
			, FILLPLAN_DRUM_MONTH.FDM_DAY".$_GET['day'].",FILLPLAN_DRUM_MONTH.FDM_WDAY".$_GET['day'].",FILLPLAN_DRUM_MONTH.FDM_CREATOR".$_GET['day'].", PRODUCT_DATA.PDD_PROD_NAME, PRODUCT_DATA.PDD_PROD_NO AS Expr1
			FROM               PRODUCT_DATA INNER JOIN
										FILLPLAN_DRUM_MONTH ON 
										PRODUCT_DATA.PDD_PROD_NO = FILLPLAN_DRUM_MONTH.PDD_PROD_NO INNER JOIN
										EMPLOYEE_DATA ON FILLPLAN_DRUM_MONTH.FDM_CREATOR".$_GET['day']." = EMPLOYEE_DATA.EMP_NO
			WHERE          (FILLPLAN_DRUM_MONTH.FDM_YEAR_MONTH = '".$_GET['ym']."') AND (PRODUCT_DATA.PDD_PROD_NO = '".$_GET['pn']."')";
			//echo $query;
	$result = mssql_query($query);
	$numRows = mssql_num_rows($result);
	while($row = mssql_fetch_array($result))
		 {
			$ym=$row['FDM_YEAR_MONTH'];
			$pn=$row['PDD_PROD_NO'];
			$day=$row["FDM_DAY".$_GET['day']];
			$wday=$row["FDM_WDAY".$_GET['day']];
			$cday=$row["FDM_CREATOR".$_GET['day']];
		 };
	?>
	  <form id="form1" name="form1" method="POST" action="<?php echo $loginFormAction; ?>">
		<p>
		<label for="ym"><?php echo $ym?></label>
	  最後修改人員：<?php echo getusername($cday);?></p>
		<p>桶數：
	  <input type="text" name="fdm_day" id="fdm_day" value="<?php echo $day?>" > 
		  EX:(17)
		  <input type="hidden" name="ym" id="ym" value="<?php echo $_GET['ym']?>"> 
		  <input type="hidden" name="pn" id="pn" value="<?php echo $_GET['pn']?>"> 
...
WHERE          (FDM_YEAR_MONTH = '".$_POST['ym']."') AND (PDD_PROD_NO = '".$_POST['pn']."')";
			$result=mssql_query($query);
			$numruw=mssql_num_rows($result);
			if($numruw==0){
				$query1="INSERT INTO FILLPLAN_DRUM_MONTH (FDM_YEAR_MONTH, PDD_PROD_NO, FDM_SPECIFIC, CTD_CUST_NO)  VALUES ('".$_POST['ym']."', '".$_POST['pn']."', 'DM','')";
